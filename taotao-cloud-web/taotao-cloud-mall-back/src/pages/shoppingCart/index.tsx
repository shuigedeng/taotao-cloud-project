...
const Index: Taro.FC = () => {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      setState(prevState => {
        return {...prevState, userMsg: userResult.data.userInfo,}
      })
    }
    init()
  }, [])
            ...
            {cartItems.length !== 0 ? (<View className='boTo'>
              ...
              <View className='rightfk'>
                <View className='YH'>
                  ...
                  <Text className='hj'>￥{(totalPrice.total || 0) / 100 - (state.discount)}/元</Text>
                </View>
                {!editable ? (
                  <View className='doubleButtonBox'>
                    <View className='fk' onClick={() => handlePay(couponId)}>
                      <Text className='payment'>付款</Text>
                    </View>
                  </View>
                ) : (
                  <View className='fk' onClick={() => deleteCart(deleteItem)}>
