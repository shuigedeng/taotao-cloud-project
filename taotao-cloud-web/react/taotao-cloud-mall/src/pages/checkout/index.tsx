...
const Index: Taro.FC<any> = (props) => {
  ...
  const getCheckoutInfo = () => {
    cartCheckout({
      ...
      userCouponId: state.userCouponId,
      grouponRulesId: state.grouponRulesId
    }).then(res => {
      setState(prevState => {
        return {
          ...prevState,
  ...
  const bindMessageInput = (e) => {
    setState(prevState => {
      return {
        ...prevState, message: e.detail.value
      }
    })
  }
  ...
  const submitOrder = () => {
    ...
    orderSubmit({
      ...
      grouponRulesId: state.grouponRulesId,
      grouponLinkId: state.grouponLinkId
    }).then(res => {
      // 下单成功，重置couponId
      try {
  ...
  const {checkedAddress, couponId, checkedGoodsList, availableCouponLength, couponPrice, message, goodsTotalPrice, freightPrice, actualPrice} = state;
  return (
    <View className='container'>
      <View className='address-box'>
        {
          checkedAddress.id > 0 ? <View className='address-item' onClick={selectAddress}>
            <View className='l'>
          ...
          </View> : <View className='address-item address-empty' onClick={selectAddress}>
            <View className='m'>
              还没有收货地址，去添加
            </View>
            <View className='r'>
      ...
      <View className='coupon-box'>
        <View className='coupon-item' onClick={selectCoupon}>
          {
      ...
      <View className='message-box'>
        <Input className='message-item' onInput={bindMessageInput} placeholder='如需要，请输入留言' value={message} />
      </View>
      ...
      <View className='order-total'>
        ...
        <View className='r' onClick={submitOrder}>去付款</View>
      </View>
    </View>
  );
}
