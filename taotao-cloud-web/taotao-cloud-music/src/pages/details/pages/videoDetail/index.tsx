...
const Page: FC = () => {
  ...
  function formatDuration(ms: number) {
    let minutes: string = formatNumber(parseInt(String(ms / 60000)))
    let seconds: string = formatNumber(parseInt(String((ms / 1000) % 60)))
          ...
          type === 'video' ?
            <View className='videoDetail__play__container'>
              ...
              <View className='videoDetail__play__desc'>
                ...
                {
                  videoInfo.videoGroup.map((videoGroupItem) => <Text className='videoDetail__play__tag'
                                                                     key={videoGroupItem.id}>{videoGroupItem.name}</Text>)
                }
              </View>
              <View className='flex videoDetail__play__numinfo'>
                <View className='flex-item'>
                  <AtIcon prefixClass='fa' value='thumbs-o-up' size='24' color='#323232'/>
                  <View className='videoDetail__play__numinfo__text'>{videoInfo.praisedCount}</View>
                </View>
                <View className='flex-item'>
                  <AtIcon value='star' size='24' color='#323232'/>
                  <View className='videoDetail__play__numinfo__text'>{videoInfo.subscribeCount}</View>
                </View>
                <View className='flex-item'>
                  <AtIcon value='message' size='24' color='#323232'/>
                  <View className='videoDetail__play__numinfo__text'>{videoInfo.commentCount}</View>
                </View>
                <View className='flex-item'>
                  <AtIcon value='share' size='24' color='#323232'/>
            ...
            <View className='videoDetail__play__container'>
              <View className='videoDetail__play__title flex flex-between'>
                ...
                <AtIcon value={showMoreInfo ? 'chevron-up' : 'chevron-down'} size='20' color='#323232'
              ...
              <View>
                {!relatedList.length ? <CLoading/> : ''}
                {
                  relatedList.map((item) => (
                    <View className='search_content__video__item' key={item.vid}
              ...
              <View>
                {!mvRelatedList.length ? <CLoading/> : ''}
                {
                  mvRelatedList.map((item) => (
                    <View className='search_content__video__item' key={item.id}
                  ...
                  <View className='flex flex-between'>
                    <View className='flex flex-align-center'>
                      ...
                      <View className='flex-item'>
                        ...
                        <View
                          className='videoDetail__comment__info__item__time'>{formatTimeStampToTime(item.time)}</View>
                      </View>
                    </View>
                    <View className={
                      classnames({
                      ...
                      <AtIcon prefixClass='fa' value='thumbs-o-up' size='12' color='#323232'/>
                    </View>
                  </View>
