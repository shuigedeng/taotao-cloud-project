...
import {SongActionType} from "../store/action/songAction";

export const getSongDetail = (payload) => (dispatch) => {
  return dispatch({
    type: SongActionType.GETSONGDETAIL,
    payload
  })
}
...
export const getPlayListDetail = (payload) => {
  ...
  return dispatch => {
    dispatch({
      type: SongActionType.RESETPLAYLIST,
    })
    api.get('/playlist/detail', {id})
      .then((res) => {
        ...
        dispatch({
          type: SongActionType.GETPLAYLISTDETAIL,
          payload: {
            playListDetailInfo,
  ...
  return dispatch => {
    api.get('/personalized')
      .then((res) => {
        ...
        dispatch({
          type: SongActionType.GETRECOMMENDPLAYLIST,
          payload: {
            recommendPlayList
          }
        })
...
export const getRecommendDj = () => {
  return dispatch => {
    api.get('/personalized/djprogram')
      .then((res) => {
        let recommendDj = res.data.result
        dispatch({
          type: SongActionType.GETRECOMMENDDJ,
          payload: {
            recommendDj
          }
        })
  ...
  return dispatch => {
    api.get('/personalized/newsong')
      .then((res) => {
        ...
        dispatch({
          type: SongActionType.GETRECOMMENDNEWSONG,
          payload: {
            recommendNewSong
          }
        })
...
export const getRecommend = () => {
  return dispatch => {
    api.get('/personalized/recommend')
      .then((res) => {
        let recommend = res.data.result
        dispatch({
          type: SongActionType.GETRECOMMEND,
          payload: {
            recommend
          }
        })
          ...
          .then((res) => {
            ...
            api.get('/lyric', {id})
              .then((res) => {
                ...
                dispatch({
                  type: SongActionType.GETSONGINFO,
                  payload: {
...
export const changePlayMode = (payload) => {
  return {
    type: SongActionType.CHANGEPLAYMODE,
    payload
  }
}
...
export const updatePlayStatus = (payload) => {
  return {
    type: SongActionType.UPDATEPLAYSTATUS,
    payload
  }
}
...
export const likeMusic = (payload) => {
  ...
  return dispatch => {
    api.get('/like', {id, like})
      .then((res) => {
        let changeFlag = res.data.code
        if (changeFlag === 200) {
          dispatch({
            type: SongActionType.UPDATELIKEMUSICLIST,
            payload: {
              like,
              id
            }
...
export const getLikeMusicList = (payload) => {
  const {id} = payload
  return dispatch => {
    api.get('/likelist', {uid: id})
      .then((res) => {
        dispatch({
          type: SongActionType.GETLIKEMUSICLIST,
          payload: {
...
export const updateCanplayList = (payload) => {
  return {
    type: SongActionType.UPDATECANPLAYLIST,
    payload
  }
}
...
export const updateRecentTab = (payload) => {
  return {
    type: SongActionType.UPDATERECENTTAB,
    payload
  }
}
