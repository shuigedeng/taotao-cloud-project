...
import static com.taotao.cloud.mq.consistency.raft.common.NodeStatus.FOLLOWER;
import static com.taotao.cloud.mq.consistency.raft.common.NodeStatus.LEADER;
...
import com.taotao.cloud.mq.consistency.raft.entity.ReplicationFailModel;
...
import com.taotao.cloud.mq.consistency.raft.rpc.DefaultRpcServiceImpl;
import com.taotao.cloud.mq.consistency.raft.rpc.Request;
import com.taotao.cloud.mq.consistency.raft.rpc.RpcClient;
import com.taotao.cloud.mq.consistency.raft.rpc.RpcService;
...
public class DefaultNode implements Node, ClusterMembershipChanges {
	...
	private LinkedBlockingQueue<ReplicationFailModel> replicationFailQueue = new LinkedBlockingQueue<>(
		2048);
	...
	public AentryResult handlerAppendEntries(AentryParam param) {
		if (param.getEntries() != null) {
			log.warn("node receive node {} append entry, entry content = {}", param.getLeaderId(),
				param.getEntries());
		}
	...
	public synchronized ClientKVAck handlerClientRequest(ClientKVReq request) {
		...
		log.info("write logModule success, logEntry info : {}, log index : {}", logEntry,
			logEntry.getIndex());
	...
	private void getRPCAppendResult(List<Future<Boolean>> futureList, CountDownLatch latch,
		...
		return RaftThreadPool.submit(() -> {
			...
			while (end - start < 20 * 1000L) {
				...
				try {
					...
					if (result.isSuccess()) {
						log.info("append follower entry success , follower=[{}], entry=[{}]", peer,
...
//                        replicationFailQueue.offer(model);
	...
	class ElectionTask implements Runnable {
		...
		public void run() {
			...
			log.info("node {} maybe become leader , success count = {} , status : {}",
				peerSet.getSelf(), success, NodeStatus.Enum.value(status));
			...
			for (Peer peer : peerSet.getPeersWithOutSelf()) {
				...
				RaftThreadPool.execute(() -> {
					try {
						...
						if (term > currentTerm) {
							log.error("self will become follower, he's term : {}, my term : {}",
								term, currentTerm);
							currentTerm = term;
							votedFor = "";
							status = FOLLOWER;
