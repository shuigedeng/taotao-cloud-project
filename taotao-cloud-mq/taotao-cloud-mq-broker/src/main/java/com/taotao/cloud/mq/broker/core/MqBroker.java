	...
	public void run() {
		...
		try {
			...
			serverBootstrap
				...
				// SO_BACKLOG：已完成三次握手但未被应用接受的连接队列长度 这个参数影响的是还没有被accept 取出的连接
				.option(ChannelOption.SO_BACKLOG, 1024)
				// TCP_NODELAY：禁用Nagle算法，减少小包延迟
				.childOption(ChannelOption.TCP_NODELAY, true)
				// // SO_KEEPALIVE：启用TCP心跳检测 这个参数只是过一段时间内客户端没有响应，服务端会发送一个 ack 包，以判断客户端是否还活着。
				...
				.childOption(ChannelOption.WRITE_BUFFER_WATER_MARK, new WriteBufferWaterMark(32 * 1024, 64 * 1024)) // 写缓冲区水位线
			;
