...
import com.taotao.cloud.bigdata.flink.doe.custom.MyTrigger;
...
public class LateData {
    public static void main(String[] args) throws Exception {
        ...
        SingleOutputStreamOperator<OrdersBean> beansWithWm = beans.assignTimestampsAndWatermarks
                (WatermarkStrategy.<OrdersBean>forBoundedOutOfOrderness(Duration.ofSeconds(1)).withTimestampAssigner(new SerializableTimestampAssigner<OrdersBean>() {
            @Override
         ...
         */
        OutputTag<OrdersBean> late_data = new OutputTag<>("late_data", TypeInformation.of(new TypeHint<OrdersBean>() {
        }));
        WindowedStream<OrdersBean, String, TimeWindow> window = keyed
                .window(TumblingEventTimeWindows.of(Time.seconds(10)))
                .allowedLateness(Time.seconds(2))
                .sideOutputLateData(late_data)
                ;

        SingleOutputStreamOperator<OrdersBean> res = window.process(new ProcessWindowFunction<OrdersBean, OrdersBean, String, TimeWindow>() {
            @Override
