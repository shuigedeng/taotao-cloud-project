...
//		DataGeneratorSource<Long> longGeneratorSource = new DataGeneratorSource<>(new GeneratorFunction<Long, Long>() {
//
//			@Override
...
//		DataStream<FlinkUser> broadcastStream = env.fromSource(dataGeneratorSource, WatermarkStrategy.noWatermarks(),  "data-generator");
		...
		OutputTag<FlinkUserContext> outputTag = new OutputTag<>("xxxx", TypeInformation.of(FlinkUserContext.class));
		SingleOutputStreamOperator<FlinkUser> process = streamOperatorMain
					...
					for (FlinkUser value : mapState.values()) {
						...
						if (currentProcessingTime > Long.MAX_VALUE - 1) {
							...
							ctx.output(outputTag,
								new FlinkUserContext(value, ctx.timerService(), fireTime));
						}
					}
				}
		...
		Pattern<FlinkUserContext, FlinkUserContext> pattern = Pattern.<FlinkUserContext>begin(
				"first")    // 以第一个登录失败事件开始
			.where(new SimpleCondition<FlinkUserContext>() {
				@Override
				public boolean filter(FlinkUserContext loginEvent) throws Exception {
		...
		patternStream.process(new PatternProcessFunction<FlinkUserContext, FlinkUserContext>() {
			@Override
			public void processMatch(Map<String, List<FlinkUserContext>> match, Context ctx,
				Collector<FlinkUserContext> out) throws Exception {
				...
				next.getTimerService().deleteProcessingTimeTimer(next.getFireTime());
			}
		}).print();
