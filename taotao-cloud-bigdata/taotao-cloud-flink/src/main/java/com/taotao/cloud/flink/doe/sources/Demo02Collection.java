...
public class Demo02Collection {
	public static void main(String[] args) throws Exception {
		 ...
		 *      @typeInfo            ： 指定返回值类型
		 * */
		DataGeneratorSource<FlinkUser> stringDataGeneratorSource = new DataGeneratorSource<>(new GeneratorFunction<Long, FlinkUser>() {
			// 定义随机数数据生成器
		...
		DataStreamSource<FlinkUser> stringDataStreamSource = see.fromSource(stringDataGeneratorSource, WatermarkStrategy.noWatermarks(), "data-generator");

		SingleOutputStreamOperator<FlinkUser> map = stringDataStreamSource.map(new RichMapFunction<FlinkUser, FlinkUser>() {
			@Override
		...
		KeyedStream<FlinkUser, Integer> flinkUserIntegerKeyedStream = map.keyBy(new KeySelector<FlinkUser, Integer>() {
			@Override
