...
public class GenerateEntityAction extends AnAction {
    ...
    public void actionPerformed(AnActionEvent e) {
        ...
        String url = String.format("jdbc:mysql://%s:%s/%s", settings.getHost(), settings.getPort(), settings.getDatabase());
    ...
    private void createAndOpenFile(Project project, DatabaseSettings settings, String className, StringBuilder classCode) {
        ...
        if (selectedPackage != null) {
            ...
            Messages.showErrorDialog(project, "No package selected. Cannot generate entity.", "Package Selection Error");
        }
    }

    private PsiPackage getSelectedPackage(Project project, DatabaseSettings settings) {
        if (settings.getSelectedPackage() != null && !settings.getSelectedPackage().isEmpty() && !"NO SELECT PACKAGE".equals(settings.getSelectedPackage())) {
    ...
    private StringBuilder generateEntityClass(DatabaseMetaData metaData, String tableName, String className, DatabaseSettings settings) throws SQLException {
    ...
    private List<FieldInfo> generateFields(DatabaseMetaData metaData, String tableName, StringBuilder classCode, DatabaseSettings settings) throws SQLException {
    ...
    private void addFieldAnnotations(StringBuilder classCode, String columnName, boolean isAutoIncrement, DatabaseSettings settings) {
        if ("JPA annotations".equals(settings.getAnnotationOption())) {
    ...
    private String mapSqlTypeToJavaType(String sqlType, DatabaseSettings settings) {
        ...
        return userTypeMappings.getOrDefault(upperCaseSqlType, DEFAULT_TYPE_MAPPINGS.getOrDefault(upperCaseSqlType, "Object"));
    }
    ...
    private void generateGettersAndSetters(StringBuilder classCode, List<FieldInfo> fields) {
        for (FieldInfo field : fields) {
            ...
            classCode.append("    }\n\n");
            // Setter
            classCode.append("    public void set").append(capitalizedName).append("(").append(field.type).append(" ").append(field.name).append(") {\n");
