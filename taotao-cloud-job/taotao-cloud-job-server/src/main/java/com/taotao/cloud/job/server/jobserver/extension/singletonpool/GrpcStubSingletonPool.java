...
package com.taotao.cloud.job.server.jobserver.extension.singletonpool;
        ...
        switch (type) {
            ...
            case RemoteConstant.NAMESERVER:
                port = Integer.parseInt(
                        TtcJobServerConfig.staticNameServerAddress.split(":")[1]);
        }
        //        int port = type.equals(RemoteConstant.SERVER) ? RemoteConstant.DEFAULT_SERVER_GRPC_PORT :
        // RemoteConstant.DEFAULT_WORKER_GRPC_PORT;
        // 通过 computeIfAbsent 来确保线程安全地创建和存储单例对象
        int finalPort = port;
