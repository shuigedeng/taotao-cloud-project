...
import com.taotao.cloud.job.server.jobserver.core.schedule.TimingStrategyService;
...
public class Consumer {
    ...
    private void createJob(MqCausa.Message message) {
        ...
        try {
            ...
            Long nextTriggerTime = timingStrategyService.calculateNextTriggerTime(
                    null,
                    TimeExpressionType.getTimeExpressionTypeByProtoBuf(jobReq.getTimeExpressionType()),
                    jobReq.getTimeExpression(),
                    lifeCycle.getStart(),
                    lifeCycle.getEnd());

            JobInfo build2 = JobInfo.builder()
    ...
    private void deleteJob(MqCausa.Message message) {
        try {
            ...
            jobInfoService.remove(new QueryWrapper<JobInfo>().lambda().eq(JobInfo::getJobId, jobReq.getJobId()));
    ...
    private void updateJob(MqCausa.Message message) {
        try {
            ...
            jobInfoService.update(
                    build2, new QueryWrapper<JobInfo>().lambda().eq(JobInfo::getJobId, build2.getJobId()));
