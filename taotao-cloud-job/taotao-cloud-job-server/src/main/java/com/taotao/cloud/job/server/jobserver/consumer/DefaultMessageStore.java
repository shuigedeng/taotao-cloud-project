...
import com.taotao.cloud.job.server.jobserver.consumer.entity.FlushRequest;
import com.taotao.cloud.job.server.jobserver.consumer.entity.Response;
...
public class DefaultMessageStore {
    ...
    static {
        try {
            String commitLogPath = Objects.requireNonNull(
                            DefaultMessageStore.class.getClassLoader().getResource("message/commitlog.dat"))
                    .toURI()
                    .getPath();
            String consumerQueuePath = Objects.requireNonNull(
                            DefaultMessageStore.class.getClassLoader().getResource("message/consumerQueue.dat"))
                    .toURI()
                    .getPath();
            // 修复路径格式
            COMMIT_LOG_FILE = commitLogPath.startsWith("/") ? commitLogPath.substring(1) : commitLogPath;
            CONSUMER_QUEUE_FILE =
                    consumerQueuePath.startsWith("/") ? consumerQueuePath.substring(1) : consumerQueuePath;
    ...
    private final AtomicLong lastProcessedOffset =
            new AtomicLong(0); // consumerQueue的buffer拉取commitLog的位置，与commitLog相比，重启时就是consumerQueue文件最后一条消息的索引位置
    ...
    public void startWatcher(Consumer consumer) {
        ...
        consumerthreadPoolExecutor = new ThreadPoolExecutor(
    ...
    private void mapFilesToMemory() throws IOException {
        ...
        try (FileChannel commitLogChannel = FileChannel.open(
        ...
        try (FileChannel consumerQueueChannel = FileChannel.open(
    ...
    public void consumeMessages() {
        ...
        while (currentConsumerPosition < currentConsumerQueuePosition.get()) {
            ...
            try {
                ...
                System.out.println(
                        "Message content: " + message1.getCreateJobReq().getJobId());
    ...
    class SyncFlushService implements Runnable {
        ...
        public SyncFlushService() {
            // 回调线程池
            final int availableProcessors = Runtime.getRuntime().availableProcessors();
            ThreadFactory callbackThreadPoolFactory = new ThreadFactoryBuilder()
                    .setNameFormat("ttcjob-callback-%d")
                    .build();
            remoteCallbackExecutor = new ThreadPoolExecutor(
                    availableProcessors * 10,
                    availableProcessors * 10,
                    120L,
                    TimeUnit.SECONDS,
