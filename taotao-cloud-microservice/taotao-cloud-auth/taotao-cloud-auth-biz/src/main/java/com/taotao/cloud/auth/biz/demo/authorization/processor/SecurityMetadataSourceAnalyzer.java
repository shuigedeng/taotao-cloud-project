...
public class SecurityMetadataSourceAnalyzer {
	...
	public void processSecurityRequestMapping() {
		...
		if (MapUtils.isNotEmpty(requestMappings)) {
			Map<Category, LinkedHashMap<HerodotusRequestMatcher, HerodotusConfigAttribute>> grouping = groupSecurityMatchers(
				requestMappings);

			LinkedHashMap<HerodotusRequestMatcher, HerodotusConfigAttribute> wildcards = grouping.get(
				Category.WILDCARD);
			securityMetadataSourceStorage.addToStorage(wildcards, false);

			LinkedHashMap<HerodotusRequestMatcher, HerodotusConfigAttribute> fullPaths = grouping.get(
				Category.FULL_PATH);
	...
	public void processSecurityMetadata(List<SecurityAttribute> securityAttributes) {
		...
		// 创建一个临时的 Matcher 容器
		LinkedHashMap<HerodotusRequestMatcher, HerodotusConfigAttribute> matchers = new LinkedHashMap<>(
			compatibles);

		// 对分发的 SecurityAttributes 进行分组
		Map<Category, LinkedHashMap<HerodotusRequestMatcher, HerodotusConfigAttribute>> grouping = groupingSecurityAttributes(
			securityAttributes);
		...
		LinkedHashMap<HerodotusRequestMatcher, HerodotusConfigAttribute> placeholders = grouping.get(
	...
	private Map<Category, LinkedHashMap<HerodotusRequestMatcher, HerodotusConfigAttribute>> groupingSecurityAttributes(
		...
		securityAttributes.parallelStream().forEach(securityAttribute -> {
			LinkedHashMap<HerodotusRequestMatcher, HerodotusConfigAttribute> resources = securityMetadataSourceParser.postProcess(
				securityAttribute);
