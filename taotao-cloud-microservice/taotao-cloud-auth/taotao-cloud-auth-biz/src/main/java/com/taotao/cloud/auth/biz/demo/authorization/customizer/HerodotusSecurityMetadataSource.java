...
public class HerodotusSecurityMetadataSource {
	...
	private static final Logger log = LoggerFactory.getLogger(
		HerodotusSecurityMetadataSource.class);
	...
	private HerodotusConfigAttribute findConfigAttribute(String url, String method,
		HttpServletRequest request) {

		log.debug("[Herodotus] |- Current Request is : [{}] - [{}]", url, method);

		HerodotusConfigAttribute configAttribute = this.securityMetadataSourceStorage.getConfigAttribute(
			url, method);
		if (ObjectUtils.isNotEmpty(configAttribute)) {
			log.debug("[Herodotus] |- Get configAttributes from local storage for : [{}] - [{}]",
				url, method);
			...
			if (MapUtils.isNotEmpty(compatible)) {
				...
				for (Map.Entry<HerodotusRequestMatcher, HerodotusConfigAttribute> entry : compatible.entrySet()) {
					if (entry.getKey().matches(request)) {
						log.debug("[Herodotus] |- Request match the wildcard [{}] - [{}]",
