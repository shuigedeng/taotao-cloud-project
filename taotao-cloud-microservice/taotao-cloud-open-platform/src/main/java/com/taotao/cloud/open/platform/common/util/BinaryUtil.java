...
public class BinaryUtil {
	...
	public static boolean isBinaryParam(Object obj) {
		return obj instanceof Binary || TypeUtil.isBinaryArray(obj.getClass())
			|| TypeUtil.isBinaryCollection(obj);
	}
	...
	public static byte[] buildSingleBinaryBytes(Binary binary, String paramStr) {
		...
		return ArrayUtil.addAll(paramLengthBytes, paramBytes, binaryCountBytes, binaryLengthBytes,
			binaryDataBytes);
	}
	...
	public static byte[] buildMultiBinaryBytes(List<Binary> binaries, String paramJsonStr) {
		...
		for (int i = 0; i < binaryCount; i++) {
			bodyBytes = ArrayUtil.addAll(bodyBytes, binaryLengthBytesList.get(i),
				binaryDataBytesList.get(i));
		}
		return bodyBytes;
	}
	...
	public static byte[] getBinaryDataBytes(byte[] bodyBytes, long binaryLengthStartIndex) {
		...
		long binaryLength = ByteUtil.bytesToLong(
			ArrayUtil.sub(bodyBytes, (int) binaryLengthStartIndex,
				(int) (binaryLengthStartIndex + 8)));
		long binaryDataStartIndex = binaryLengthStartIndex + 8;
		byte[] binaryDataBytes = ArrayUtil.sub(bodyBytes, (int) binaryDataStartIndex,
			(int) (binaryDataStartIndex + binaryLength));
	 ...
	 */
	public static long getNextBinaryLengthStartIndex(long binaryLengthStartIndex,
