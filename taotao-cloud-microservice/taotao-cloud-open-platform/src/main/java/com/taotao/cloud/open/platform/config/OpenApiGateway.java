...
import com.taotao.cloud.open.platform.common.constant.Constant;
import com.taotao.cloud.open.platform.common.constant.Header;
...
import com.taotao.cloud.open.platform.common.enums.DataType;
import com.taotao.cloud.open.platform.common.enums.SymmetricCryEnum;
import com.taotao.cloud.open.platform.common.exception.OpenApiServerException;
...
import com.taotao.cloud.open.platform.common.model.Binary;
import com.taotao.cloud.open.platform.common.model.InParams;
import com.taotao.cloud.open.platform.common.model.OutParams;
import com.taotao.cloud.open.platform.common.util.Base64Util;
import com.taotao.cloud.open.platform.common.util.BinaryUtil;
import com.taotao.cloud.open.platform.common.util.CommonUtil;
import com.taotao.cloud.open.platform.common.util.CompressUtil;
import com.taotao.cloud.open.platform.common.util.StrObjectConvert;
import com.taotao.cloud.open.platform.common.util.SymmetricCryUtil;
import com.taotao.cloud.open.platform.common.util.TruncateUtil;
import com.taotao.cloud.open.platform.common.util.TypeUtil;
import com.taotao.cloud.open.platform.model.ApiHandler;
import com.taotao.cloud.open.platform.model.Context;
 ...
 * 对外开放api网关入口
 * <p>
 * 功能 1.负责对外开放接口(基于HTTP对外提供服务) 2.实现接口的参数与返回值的加解密（使用非对称加密：RSA/SM2，或对称加密：AES/SM4）
 * 3.实现接口的验签（服务端会校验客户端的签名，确保调用者身份以及数据不被篡改） 4.实现非对称加密+对称加密联合模式（内容对称加密，对称加密密钥采用非对称加密）
 * <p>
 *
 * @author shuigedeng
 * @version 2022.07
 * @since 2022-07-26 10:12:21
 */
@Slf4j
@RestController
public class OpenApiGateway {
	...
	/**
	 * 定义api处理器映射 key: api_method value: ApiHandler
	 */
	...
	public void init() {
		...
		if (log.isDebugEnabled()) {
			...
			log.debug(
				"OpenApiGateway Init: \nSelfPrivateKey:{},\nAsymmetricCry:{},\nretEncrypt:{},\ncryModeEnum:{},\nSymmetricCry:{},\nenableCompress:{},\nApiHandlers:\n{}",
				selfPrivateKey, asymmetricCryEnum, retEncrypt, cryModeEnum, symmetricCryEnum,
				enableCompress, handlersStr);
	...
	private String getHandlersStr() {
		...
		if (CollUtil.isEmpty(handlers)) {
			handlersStr = new StringBuilder(
				"未找到ApiHandler,请确保注解@OpenApi所声明的bean可被spring扫描到");
		} else {
	...
	private InParams getInParams(HttpServletRequest request) {
		...
		try {
			...
			inParams.setMultiParam(
				Boolean.parseBoolean(request.getHeader(Header.Request.MULTI_PARAM)));
			inParams.setDataType(
				Enum.valueOf(DataType.class, request.getHeader(Header.Request.DATA_TYPE)));
		...
		if (ArrayUtil.isNotEmpty(inParams.getBodyBytes())) {
			...
			try {
				...
				if (inParams.getDataType() == DataType.BINARY) {
					...
					bodyBytes = isEnableCompress(apiHandler) ? CompressUtil.decompress(bodyBytes)
						: bodyBytes;
					...
					paramStr =
						isEnableCompress(apiHandler) ? CompressUtil.decompressToText(bodyBytes)
				...
				if (inParams.isMultiParam()) {
					...
					for (int i = 0; i < list.size(); i++) {
						...
						if (BinaryUtil.isBinaryParam(obj)) {
							binaryLengthStartIndex = this.fillBinaryParam(obj, bodyBytes,
	...
	private long fillBinaryParam(Object obj, byte[] bodyBytes, long binaryLengthStartIndex) {
		if (obj instanceof Binary) {
			binaryLengthStartIndex = this.fillBinaryData((Binary) obj, bodyBytes,
				binaryLengthStartIndex);
		} else if (TypeUtil.isBinaryArray(obj.getClass())) {
			Binary[] binaries = (Binary[]) obj;
			for (Binary binary : binaries) {
				binaryLengthStartIndex = this.fillBinaryData(binary, bodyBytes,
					binaryLengthStartIndex);
			}
	...
	private void verifySign(InParams inParams) {
		...
		log.debug("{}caller({}) publicKey:{}", logPrefix.get(), inParams.getCallerId(),
			callerPublicKey);
		byte[] signContent = CommonUtil.getSignContent(inParams);
		boolean verify = this.asymmetricCryHandler.verifySign(callerPublicKey, signContent,
			inParams.getSign());
	...
	private OutParams doCall(ApiHandler apiHandler, List<Object> paramList, InParams inParams) {
		try {
			...
			log.debug("{}调用API:{},入参：{}", logPrefix.get(), apiHandler,
				TruncateUtil.truncate(paramList));
			Object ret = apiHandler.getMethod().invoke(apiHandler.getBean(), params);
			log.debug("{}调用API:{},出参：{}", logPrefix.get(), apiHandler,
				TruncateUtil.truncate(ret));
			...
			if (ret != null) {
				if (ret instanceof Binary) {
					...
					retBytes = isEnableCompress(apiHandler) ? CompressUtil.compress(retBytes)
						: retBytes;
					...
					if (StrUtil.isNotBlank(retStr)) {
						//转成Byte[]
						retBytes = isEnableCompress(apiHandler) ? CompressUtil.compressText(retStr)
							: retStr.getBytes(StandardCharsets.UTF_8);
					}
				}
			}
	 ...
	 * @return 加密后的返回值
	 */
	private byte[] encryptRet(InParams inParams, byte[] retBytes, OutParams outParams,
	...
	private void logCryMode(CryModeEnum cryModeEnum, OpenApiMethod apiMethod) {
		...
		if (cryModeEnum == CryModeEnum.SYMMETRIC_CRY) {
			log.debug("{}采用非对称加密{}+对称加密{}模式", methodId, asymmetricCryEnum,
				symmetricCryEnum);
		} else if (cryModeEnum == CryModeEnum.ASYMMETRIC_CRY) {
	...
	private void logCostTime(String operate, long startTime) {
		log.debug("{}{}耗时:{}ms", logPrefix.get(), operate,
