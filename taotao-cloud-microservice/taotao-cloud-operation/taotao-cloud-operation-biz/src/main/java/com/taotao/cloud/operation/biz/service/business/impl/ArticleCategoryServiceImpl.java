...
public class ArticleCategoryServiceImpl extends ServiceImpl<ArticleCategoryMapper, ArticleCategory> implements
    ...
    private List<ArticleCategoryVO> initCategory() {
        ...
        articleCategories.forEach(item -> {
            if (item.getLevel() == 0) {
				ArticleCategoryVO articleCategoryVO = ArticleCategoryConvert.INSTANCE.articleCategoryToArticleCategoryVO(item);
    ...
    private void initChild(ArticleCategoryVO tree, List<ArticleCategory> articleCategories) {
        ...
        articleCategories.stream()
                ...
                .forEach(child -> {
					ArticleCategoryVO childTree = ArticleCategoryConvert.INSTANCE.articleCategoryToArticleCategoryVO(child);
