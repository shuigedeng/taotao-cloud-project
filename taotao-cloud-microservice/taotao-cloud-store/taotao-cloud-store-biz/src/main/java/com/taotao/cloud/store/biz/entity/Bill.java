...
public class Bill extends BaseSuperEntity<Bill, Long> {
	 ...
	 * @see BillStatusEnum
	 */
	@Column(name = "bill_status", columnDefinition = "varchar(32) not null comment '状态：OUT(已出账),CHECK(已对账),EXAMINE(已审核),PAY(已付款)'")
	...
	@Column(name = "refund_commission_price", columnDefinition = "decimal(10,2) not null default 0 comment '退单产生退还佣金金额'")
	...
	@Column(name = "distribution_refund_commission", columnDefinition = "decimal(10,2) not null default 0 comment '分销订单退还，返现佣金返还'")
	...
	@Column(name = "site_coupon_refund_commission", columnDefinition = "decimal(10,2) not null default 0 comment '退货平台优惠券补贴返还'")
