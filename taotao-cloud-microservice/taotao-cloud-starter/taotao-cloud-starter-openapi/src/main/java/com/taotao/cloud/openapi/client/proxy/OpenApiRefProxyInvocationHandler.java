...
public class OpenApiRefProxyInvocationHandler implements InvocationHandler {
		...
		if (Object.class.equals(method.getDeclaringClass())) {
			...
			if (method.isAnnotationPresent(OpenApiMethod.class)) {
				...
				if (configDif) {
					...
					apiClient = new OpenApiClientBuilder(config.getBaseUrl(),
						config.getSelfPrivateKey(), config.getRemotePublicKey(),
						config.getCallerId(), api)
	...
	private boolean methodConfigDif(OpenApiMethod openApiMethod) {
		boolean retDecryptDif = StrUtil.isNotBlank(openApiMethod.retDecrypt())
			&& Boolean.parseBoolean(openApiMethod.retDecrypt()) != config.isRetDecrypt();
		if (retDecryptDif) {
			return true;
		}
		boolean cryModeDif = openApiMethod.cryModeEnum() != CryModeEnum.UNKNOWN
			&& openApiMethod.cryModeEnum() != config.getCryModeEnum();
		if (cryModeDif) {
			return true;
		}
		boolean httpConnectionTimeoutDif = openApiMethod.httpConnectionTimeout() != -1
			&& openApiMethod.httpConnectionTimeout() != config.getHttpConnectionTimeout();
		if (httpConnectionTimeoutDif) {
			return true;
		}
		boolean httpReadTimeoutDif = openApiMethod.httpReadTimeout() != -1
			&& openApiMethod.httpReadTimeout() != config.getHttpReadTimeout();
		if (httpReadTimeoutDif) {
			return true;
		}
		boolean enableCompressDif = StrUtil.isNotBlank(openApiMethod.enableCompress())
			&& Boolean.parseBoolean(openApiMethod.enableCompress()) != config.isEnableCompress();
