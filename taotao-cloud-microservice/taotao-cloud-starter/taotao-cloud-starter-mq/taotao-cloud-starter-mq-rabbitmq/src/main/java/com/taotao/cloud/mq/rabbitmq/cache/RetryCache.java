...
import com.taotao.cloud.mq.rabbitmq.common.FastOcpRabbitMqConstants;
				...
				for (Map.Entry<Long, MessageWithTime> entry : map.entrySet()) {
					...
					if (null != messageWithTime) {
						if (messageWithTime.getTime() + 3 * FastOcpRabbitMqConstants.VALID_TIME
							< now) {
							LogUtils.info("send message {} failed after 3 min ", messageWithTime);
							...
							if (!res.isIfSuccess()) {
								LogUtils.info("retry send message failed {} errMsg {}",
									messageWithTime,
									res.getErrMsg());
							}
						}
