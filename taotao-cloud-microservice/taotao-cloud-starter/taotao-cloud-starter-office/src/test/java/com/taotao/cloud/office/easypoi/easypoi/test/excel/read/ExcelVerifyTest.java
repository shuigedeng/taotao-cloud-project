...
import com.taotao.cloud.office.easypoi.easypoi.test.excel.read.hanlder.ExcelVerifyHandlerImpl;
...
import com.taotao.cloud.office.easypoi.test.entity.ExcelVerifyEntityOfMode;
import java.io.File;
	...
	public void basetest() {
		try {
			...
			FileOutputStream fos = new FileOutputStream(
				"D:/home/excel/ExcelVerifyTest.basetest.xlsx");
	...
	public void basetestonlyFail() {
		try {
			...
			ExcelImportResult<ExcelVerifyEntity> result = ExcelImportUtil.importExcelMore(
				...
				ExcelVerifyEntity.class, params);
			FileOutputStream fos = new FileOutputStream(
				"D:/home/excel/ExcelVerifyTest.basetestonlyFail_success.xlsx");
			result.getWorkbook().write(fos);
			fos.close();
			for (int i = 0; i < result.getList().size(); i++) {
				System.out.println(ReflectionToStringBuilder.toString(result.getList().get(i)));
	...
	public void combinedVerificationTest() {
		try {
			...
			ExcelImportResult<ExcelVerifyEntityOfMode> result = ExcelImportUtil.importExcelMore(
				...
				ExcelVerifyEntityOfMode.class, params);
			FileOutputStream fos = new FileOutputStream(
				"D:/home/excel/combinedVerificationTest.xlsx");
			result.getWorkbook().write(fos);
			fos.close();
			for (int i = 0; i < result.getList().size(); i++) {
				System.out.println(ReflectionToStringBuilder.toString(result.getList().get(i)));
