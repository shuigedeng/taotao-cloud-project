...
public class XssUtils {
	...
	private Policy createPolicy() {
		try {
			...
			LogUtils.trace("Antisamy create policy error! {}", e.getMessage());
			return null;
		}
	}
	...
	private String cleanHtml(String taintedHtml) {
		try {
			LogUtils.trace("Before Antisamy Scan, value is: [{}]", taintedHtml);
			// 使用AntiSamy清洗数据
			final CleanResults cleanResults = scan(taintedHtml);
			String result = cleanResults.getCleanHTML();
			LogUtils.trace("After  Antisamy Scan, value is: [{}]", result);
			return result;
		} catch (ScanException | PolicyException e) {
			LogUtils.error("Antisamy scan catch error! {}", e.getMessage());
			return taintedHtml;
		}
	}
	...
	public static String cleaning(String taintedHTML) {
		...
		LogUtils.trace("After  Antisamy Well Formed, value is: [{}]", result);
		return result;
	}
}
