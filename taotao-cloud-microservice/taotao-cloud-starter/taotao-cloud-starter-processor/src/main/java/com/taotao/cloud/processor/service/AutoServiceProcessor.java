...
public class AutoServiceProcessor extends AbstractCloudProcessor {

	/**
	 * AutoService 注解名
	 */
	private static final String AUTO_SERVICE_NAME = com.taotao.cloud.processor.annotation.AutoService.class.getName();
	/**
	...
	private void processAnnotations(Set<? extends TypeElement> annotations,
		...
		for (TypeElement typeElement : typeElementSet) {
			AnnotationMirror annotationMirror = getAnnotation(elementUtils, typeElement,
				AUTO_SERVICE_NAME);
				...
				if (checkImplementer(typeElement, typeMirror)) {
					...
					String message =
						"ServiceProviders must implement their service provider interface. "
							+ providerImplementerName + " does not implement "
	...
	private void generateConfigFiles() {
		...
		for (String providerInterface : providers.keySet()) {
			...
			try {
				...
				// 1. 存在用户手动编写的配置
				try {
					FileObject existingFile = filer.getResource(StandardLocation.SOURCE_OUTPUT, "",
						resourceFile);
					log("Looking for existing resource file at " + existingFile.toUri());
					Set<String> oldServices = ServicesFiles.readServiceFile(existingFile,
						elementUtils);
	...
	/**
	 * Verifies {@link java.util.spi.LocaleServiceProvider} constraints on the concrete provider
	 * class. Note that these constraints are enforced at runtime via the ServiceLoader, we're just
	 * checking them at compile time to be extra nice to our users.
	 */
	...
	public AnnotationValue getAnnotationValue(AnnotationMirror annotationMirror,
		...
		for (Map.Entry<? extends ExecutableElement, ? extends AnnotationValue> entry : elementUtils.getElementValuesWithDefaults(
		...
		throw new IllegalArgumentException(
			String.format("@%s does not define an element %s()", annotationName, elementName));
	}
	...
	public String getType(TypeMirror type) {
		...
		if (type instanceof DeclaredType) {
			...
			if (enclosingElement instanceof TypeElement) {
				return getQualifiedName(enclosingElement) + "$" + declaredType.asElement()
