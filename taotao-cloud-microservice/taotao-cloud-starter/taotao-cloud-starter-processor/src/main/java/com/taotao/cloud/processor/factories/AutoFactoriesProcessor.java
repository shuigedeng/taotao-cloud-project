...
public class AutoFactoriesProcessor extends AbstractCloudProcessor {
	 ...
	 */
	private static final String AUTO_CONFIGURATION_IMPORTS_LOCATION =
		"META-INF/spring/" + AUTO_CONFIGURATION + ".imports";
	/**
	 * 数据承载
	 */
	...
	private void generateAutoConfigurationImportsFiles() {
		...
		try {
			...
			try {
				FileObject existingFactoriesFile = filer.getResource(StandardLocation.SOURCE_OUTPUT,
					"", AUTO_CONFIGURATION_IMPORTS_LOCATION);
				// 查找是否已经存在 spring.factories
				log("Looking for existing AutoConfiguration imports file at "
					+ existingFactoriesFile.toUri());
				Set<String> existingSet = FactoriesFiles.readAutoConfigurationImports(
					existingFactoriesFile);
			...
			FileObject autoConfigurationImportsFile = filer.createResource(
				StandardLocation.CLASS_OUTPUT, "", AUTO_CONFIGURATION_IMPORTS_LOCATION);
