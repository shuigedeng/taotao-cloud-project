...
public class RequeueRedissonListenerErrorHandler implements RedissonListenerErrorHandler {
    ...
    private void requeue(Object payload, Map<String, Object> headers, Throwable throwable) {
        ...
        if (!StringUtils.hasText(queueName)) {
	        LogUtil.warn("message [{}] delivery queue name is empty, abandon it", JsonUtil.toJSONString(payload), throwable);
            return;
        }
        ...
        if (requeueTimes < this.maxRequeueTimes) {
	        ...
	        LogUtil.warn("message [{}] reach the max requeue times, abandon it",JsonUtil.toJSONString(payload), throwable);
            return;
        }
