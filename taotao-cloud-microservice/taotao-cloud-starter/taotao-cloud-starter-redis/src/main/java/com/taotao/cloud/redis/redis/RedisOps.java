package com.taotao.cloud.redis.redis;
...
public class RedisOps {
	...
	public RedisOps(RedisTemplate<String, Object> redisTemplate,
		StringRedisTemplate stringRedisTemplate, boolean defaultCacheNullVal) {
	 ...
	 * <p>
	 * 例子： KEYS * 匹配数据库中所有 key 。 KEYS h?llo 匹配 hello ， hallo 和 hxllo 等。 KEYS h*llo 匹配 hllo 和
	 * heeeeello 等。 KEYS h[ae]llo 匹配 hello 和 hallo ，但不匹配 hillo 。
	 * <p>
	 * 特殊符号用 \ 隔开
	 *
	 * @param pattern 表达式
	...
	public Long hIncrBy(@NonNull CacheHashKey key, long increment) {
		Long hIncrBy = stringRedisTemplate.opsForHash()
	...
	public Double hIncrByFloat(@NonNull CacheHashKey key, double increment) {
		Double hIncrBy = stringRedisTemplate.opsForHash()
			.increment(key.getKey(), key.getField(), increment);
		if (key.getExpire() != null) {
			stringRedisTemplate.expire(key.getKey(), key.getExpire());
		}
	...
	public Set<Object> sInter(@NonNull CacheKey key, Collection<CacheKey> otherKeys) {
		return setOps.intersect(key.getKey(),
	...
	public Long sInterStore(Collection<CacheKey> otherKeys, @NonNull CacheKey destKey) {
		return setOps.intersectAndStore(
			otherKeys.stream().map(CacheKey::getKey).collect(Collectors.toList()),
			destKey.getKey());
	}
	...
	public <V> Set<V> sUnion(@NonNull CacheKey key, Collection<CacheKey> otherKeys) {
		return (Set<V>) setOps.union(key.getKey(),
			otherKeys.stream().map(CacheKey::getKey).collect(Collectors.toList()));
	}
	...
	public Long sDiffStore(Collection<CacheKey> otherKeys, @NonNull CacheKey distKey) {
		return setOps.differenceAndStore(
			otherKeys.stream().map(CacheKey::getKey).collect(Collectors.toList()),
			distKey.getKey());
	}
