...
package com.taotao.cloud.disruptor.disruptor;
...
import com.taotao.cloud.disruptor.disruptor.consumer.QueueConsumer;
import com.taotao.cloud.disruptor.disruptor.consumer.QueueConsumerFactory;
import com.taotao.cloud.disruptor.disruptor.event.DataEvent;
import com.taotao.cloud.disruptor.disruptor.event.DisruptorEventFactory;
import com.taotao.cloud.disruptor.disruptor.event.OrderlyDisruptorEventFactory;
import com.taotao.cloud.disruptor.disruptor.provider.DisruptorProvider;
import com.taotao.cloud.disruptor.disruptor.thread.DisruptorThreadFactory;
import com.taotao.cloud.disruptor.disruptor.thread.OrderlyExecutor;
...
public class DisruptorProviderManage<T> {
    ...
    public void startup(final boolean isOrderly) {
        OrderlyExecutor executor = new OrderlyExecutor(isOrderly, consumerSize, consumerSize, 0, TimeUnit.MILLISECONDS,
                ...
                DisruptorThreadFactory.create("taotao_cloud_disruptor_consumer_", false), new ThreadPoolExecutor.AbortPolicy());
        ...
        Disruptor<DataEvent<T>> disruptor = new Disruptor<>(eventFactory,
                size,
                DisruptorThreadFactory.create("taotao_disruptor_provider_" + consumerFactory.fixName(), false),
                ProducerType.MULTI,
