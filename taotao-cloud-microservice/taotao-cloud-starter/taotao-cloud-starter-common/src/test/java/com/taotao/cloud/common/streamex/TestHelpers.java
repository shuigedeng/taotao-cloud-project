...
public class TestHelpers {
	...
	public static class EntryStreamSupplier<K, V> extends StreamSupplier<Entry<K, V>> {

		public EntryStreamSupplier(Supplier<Stream<Entry<K, V>>> base, Mode mode) {
			super(base, mode);
		}
	...
	public static <K, V> void entryStream(Supplier<Stream<Entry<K, V>>> base,
	...
	public static <T> void checkSpliterator(String msg, List<T> expected, Supplier<Spliterator<T>> supplier) {
		...
		for (int i = 1; i < Math.min(4, expected.size() - 1); i++) {
			...
			sequential = supplier.get();
			for (int j = 0; j < i; j++) {
				assertTrue(msg, sequential.tryAdvance(seq::add));
			}
			sequential.forEachRemaining(seq::add);
			assertEquals(msg, expected, seq);
		}
