...
public class CronTest {
	...
	public void testConvertField() {
		...
		for (int i = 0; i < 6; i++) {
			...
			Assertions.assertEquals(field.getCronPosition().ordinal(), i);
		}
		Assertions.assertEquals("0", cronFields.get(0).getExpress());
		Assertions.assertEquals("0-5", cronFields.get(1).getExpress());
		Assertions.assertEquals("14/2", cronFields.get(2).getExpress());
		Assertions.assertEquals("*", cronFields.get(3).getExpress());
		Assertions.assertEquals("*", cronFields.get(4).getExpress());
		Assertions.assertEquals("*", cronFields.get(5).getExpress());
		List<CronField> fields = CronUtil.convertCronField("0 15 10 ? JAN-NOV MON-FRI");
		for (int i = 0; i < 6; i++) {
			CronField field = fields.get(i);
			Assertions.assertEquals(field.getCronPosition().ordinal(), i);
		}
		Assertions.assertEquals("0", fields.get(0).getExpress());
		Assertions.assertEquals("15", fields.get(1).getExpress());
		Assertions.assertEquals("10", fields.get(2).getExpress());
		Assertions.assertEquals("*", fields.get(3).getExpress());
		Assertions.assertEquals("1-11", fields.get(4).getExpress());
		Assertions.assertEquals("1-5", fields.get(5).getExpress());
	...
	public void testConvertCronField() {
		...
		Assertions.assertEquals(Collections.singletonList(1), cronFields.get(0).points());
		Assertions.assertEquals(Arrays.asList(0, 1, 2, 3, 4, 5), cronFields.get(1).points());
		Assertions.assertEquals(Arrays.asList(1, 4, 7, 10, 13, 16, 19, 22), cronFields.get(2).points());
		Assertions.assertEquals(Arrays.asList(1, 3, 4), cronFields.get(3).points());
		Assertions.assertEquals(Arrays.asList(1, 3, 5, 7, 9, 11), cronFields.get(4).points());
		Assertions.assertEquals(Arrays.asList(0, 1, 2, 3, 4, 5, 6), cronFields.get(5).points());
	}
	...
	public void testCal() {
		...
		Assertions.assertEquals(Collections.singletonList(new TimeOfDay(10, 15, 0)), calculate);
		...
		Assertions.assertEquals(calculate, new DayBasedCronParser(cron).timesOfDay(date));
		...
		calculate = CronUtil.timesOfDay(cron, date);
		Assertions.assertEquals(Collections.emptyList(), calculate);
		Assertions.assertEquals(calculate, new DayBasedCronParser(cron).timesOfDay(date));
	}
