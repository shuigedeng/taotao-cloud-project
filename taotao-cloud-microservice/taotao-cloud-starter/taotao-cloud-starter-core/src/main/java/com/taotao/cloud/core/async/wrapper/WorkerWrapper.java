package com.taotao.cloud.core.async.wrapper;

import com.taotao.cloud.core.async.callback.DefaultCallback;
import com.taotao.cloud.core.async.callback.ICallback;
import com.taotao.cloud.core.async.callback.IWorker;
import com.taotao.cloud.core.async.exception.SkippedException;
import com.taotao.cloud.core.async.executor.timer.SystemClock;
import com.taotao.cloud.core.async.worker.DependWrapper;
import com.taotao.cloud.core.async.worker.ResultState;
...
public class WorkerWrapper<T, V> {
	...
	/**
	 * 依赖的wrappers，有2种情况，1:必须依赖的全部完成后，才能执行自己 2:依赖的任何一个、多个完成了，就可以执行自己 通过must字段来控制是否依赖项必须完成 1 -------3
	 * 2 1、2执行完毕后才能执行3
	 */
	...
	private void beginNext(ExecutorService executorService, long now, long remainTime) {
		...
		if (nextWrappers.size() == 1) {
			nextWrappers.get(0).work(executorService, WorkerWrapper.this, remainTime - costTime,
				forParamUseWrappers);
			return;
		}
		...
		for (int i = 0; i < nextWrappers.size(); i++) {
			...
			futures[i] = CompletableFuture.runAsync(() -> nextWrappers.get(finalI)
				.work(executorService, WorkerWrapper.this, remainTime - costTime,
					forParamUseWrappers), executorService);
		}
		try {
	...
	private synchronized void doDependsJobs(ExecutorService executorService,
		List<DependWrapper> dependWrappers, WorkerWrapper fromWrapper, long now, long remainTime) {
