...
public class AlipayController {
	...
	@Operation(summary = "电脑网站支付", description = "支付宝电脑网站支付接口", responses = {
		@ApiResponse(description = "单位列表", content = @Content(mediaType = "application/json", schema = @Schema(implementation = Result.class)))})
	@Parameters({
	...
	public Result<String> pagePay(@Validated PagePayDetails details) {
		...
		if (details.getMode().equals("2")) {
			response = alipayPaymentTemplate.getProcessor(details.getTradeNo(),
				details.getIdentity()).pagePay(model, alipayPaymentTemplate.getNotifyUrl(),
				alipayPaymentTemplate.getReturnUrl(), details.getAppAuthToken(), HttpMethod.GET);
		} else {
			response = alipayPaymentTemplate.getProcessor(details.getTradeNo(),
				details.getIdentity()).pagePay(model, alipayPaymentTemplate.getNotifyUrl(),
				alipayPaymentTemplate.getReturnUrl(), details.getAppAuthToken());
		}
