package com.taotao.cloud.pay.common.provider;

import com.egzosn.pay.common.api.PayService;
import com.egzosn.pay.common.bean.CertStoreType;
import com.egzosn.pay.common.util.str.StringUtils;
import com.taotao.cloud.pay.common.configuration.UnionPaymentPlatform;
import com.taotao.cloud.pay.common.configurers.PayMessageConfigurer;
import com.taotao.cloud.pay.common.merchant.MerchantNotFoundException;
import com.taotao.cloud.pay.common.merchant.bean.CommonPaymentPlatformMerchantDetails;
import com.taotao.cloud.pay.common.utils.SqlTools;
...
public class JdbcMerchantDetailsManager implements
	...
	private static final List<String> FIELDS = Arrays.asList("appid", "pay_type", "mch_id",
		"cert_store_type", "key_private", "key_cert_pwd", "key_public", "key_cert", "notify_url",
		"return_url", "sign_type", "seller", "sub_app_id", "sub_mch_id", "input_charset",
		"is_test");
	...
	private static final String DEFAULT_BASE_QUERY_SQL = SqlTools.getSelectSQL(
		ID + SqlTools.SEPARATED + SELECT_FIELDS, TABLE);
	private static final String DEFAULT_FIND_BY_ID_SQL =
		DEFAULT_BASE_QUERY_SQL + " where " + ID + "=?";
	private static final String DEFAULT_INSERT_SQL =
		"insert into " + TABLE + " (" + ID + SqlTools.SEPARATED + SELECT_FIELDS + ") values ("
			+ SqlTools.forQuestionMarkSQL(FIELDS.size() + 1) + ")";
	private static final String DEFAULT_UPDATE_SQL = SqlTools.generateUpdateByRowIdString(TABLE,
		FIELDS, ID).toString();
	private static final String DEFAULT_DELETE_SQL = "delete from " + TABLE + " where " + ID + "=?";
	public static final String DEFAULT_EXISTS_SQL =
	...
	public void createMerchant(CommonPaymentPlatformMerchantDetails merchant) {
		...
		Object[] args = new Object[]{merchant.getDetailsId(), merchant.getAppId(),
			merchant.getPayType(), merchant.getMchId(), merchant.getCertStoreType(),
			merchant.getKeyPrivate(), merchant.getKeyPrivateCertPwd(), merchant.getKeyPublic(),
			merchant.getKeyCert(), merchant.getNotifyUrl(), merchant.getReturnUrl(),
			merchant.getSignType(), merchant.getSeller(), merchant.getSubAppId(),
			merchant.getSubMchId(), merchant.getInputCharset(), merchant.isTest()};
	...
	public void updateMerchant(CommonPaymentPlatformMerchantDetails merchant) {
		...
		Object[] args = new Object[]{merchant.getAppId(), merchant.getPayType(),
			merchant.getMchId(), merchant.getCertStoreType(), merchant.getKeyPrivate(),
			merchant.getKeyPrivateCertPwd(), merchant.getKeyPublic(), merchant.getKeyCert(),
			merchant.getNotifyUrl(), merchant.getReturnUrl(), merchant.getSignType(),
			merchant.getSeller(), merchant.getSubAppId(), merchant.getSubMchId(),
			merchant.getInputCharset(), merchant.isTest(), merchant.getDetailsId()};
	...
	public CommonPaymentPlatformMerchantDetails loadMerchantByMerchantId(String merchantId) {
		List<CommonPaymentPlatformMerchantDetails> detailss = jdbcTemplate.query(findByIdSql,
			(ResultSet rs, int i) -> {
	...
	public void setKeyPrivate(CommonPaymentPlatformMerchantDetails details, ResultSet rs)
		throws SQLException {
