package com.taotao.cloud.security.springsecurity.jwt.utils;
...
import com.taotao.cloud.security.springsecurity.jwt.model.ContextConstants;
import com.taotao.cloud.security.springsecurity.jwt.model.ExceptionCode;
import com.taotao.cloud.security.springsecurity.jwt.model.Token;
...
public final class JwtUtil {
	...
	public static String[] getClient(String basicHeader) {
		if (StrUtil.isEmpty(basicHeader) || !basicHeader.startsWith(
			ContextConstants.BASIC_HEADER_PREFIX)) {
			//throw BizException.wrap(ExceptionCode.JWT_BASIC_INVALID);
			throw new BusinessException(ExceptionCode.JWT_BASIC_INVALID.name());
		}

		String decodeBasic = StrUtil.subAfter(basicHeader, ContextConstants.BASIC_HEADER_PREFIX,
			false);
	...
	public static Claims parseJwt(String jsonWebToken, long allowedClockSkewSeconds) {
		try {
			...
			//		e);
			throw new BusinessException(ExceptionCode.JWT_PARSER_TOKEN_FAIL.getCode(),
