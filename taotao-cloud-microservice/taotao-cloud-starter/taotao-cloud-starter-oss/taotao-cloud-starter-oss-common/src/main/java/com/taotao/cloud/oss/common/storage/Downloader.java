...
public class Downloader {
	...
	public void inputStream(Consumer<InputStream> consumer) {
		if (target == TARGET_FILE) {
			//下载文件
			new DownloadAspectChain(aspectList, (fileInfo, fileStorage, con) ->
				fileStorage.download(fileInfo, con)
			...
			//下载缩略图文件
			new DownloadThAspectChain(aspectList, (fileInfo, fileStorage, con) ->
				fileStorage.downloadTh(fileInfo, con)
			).next(fileInfo, fileStorage, in ->
