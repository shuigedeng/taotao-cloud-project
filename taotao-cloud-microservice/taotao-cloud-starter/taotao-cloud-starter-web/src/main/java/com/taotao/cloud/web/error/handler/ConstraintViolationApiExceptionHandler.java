...
import com.taotao.cloud.web.error.ApiParameterError;
...
public class ConstraintViolationApiExceptionHandler extends AbstractApiExceptionHandler {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(
		ConstraintViolationApiExceptionHandler.class);
			...
			.map(constraintViolation -> {
				...
				if (leafNode.isPresent()) {
					...
					if (elementKind == ElementKind.PROPERTY) {
						...
						LOGGER.warn(
							"Unable to convert constraint violation with element kind {}: {}",
	...
	private String getCode(ConstraintViolation<?> constraintViolation) {
		String code = constraintViolation.getConstraintDescriptor().getAnnotation().annotationType()
			.getSimpleName();
