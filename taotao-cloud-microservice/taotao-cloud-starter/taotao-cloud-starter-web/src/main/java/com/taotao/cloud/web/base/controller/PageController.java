...
import com.taotao.cloud.data.mybatis.plus.query.conditions.Wraps;
	...
	default QueryWrap<T> handlerWrapper(QueryDTO params) {
		...
		if (params instanceof BaseQuery baseQuery) {
			...
			Optional.ofNullable(baseQuery.dateTimeBetweenQuery())
				...
				.filter(dateTimeBetweenDTO -> ReflectionUtils.checkField(dateTimeBetweenDTO.getClass(), getEntityClass()))
