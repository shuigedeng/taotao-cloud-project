...
import com.taotao.cloud.websocket.netty.standard.ServerEndpointConfig;
import com.taotao.cloud.websocket.netty.support.AntPathMatcherWrapper;
import com.taotao.cloud.websocket.netty.support.DefaultPathMatcher;
...
import com.taotao.cloud.websocket.netty.support.WsPathMatcher;
...
public class PojoEndpointServer {
	...
	private static final AttributeKey<Object> POJO_KEY = AttributeKey.valueOf(
		"WEBSOCKET_IMPLEMENT");

	public static final AttributeKey<Session> SESSION_KEY = AttributeKey.valueOf(
		"WEBSOCKET_SESSION");

	private static final AttributeKey<String> PATH_KEY = AttributeKey.valueOf("WEBSOCKET_PATH");

	public static final AttributeKey<Map<String, String>> URI_TEMPLATE = AttributeKey.valueOf(
		"WEBSOCKET_URI_TEMPLATE");

	public static final AttributeKey<Map<String, List<String>>> REQUEST_PARAM = AttributeKey.valueOf(
	...
	public void doBeforeHandshake(Channel channel, FullHttpRequest req, String path) {
		...
		if (beforeHandshake != null) {
			try {
				beforeHandshake.invoke(implement,
					methodMapping.getBeforeHandshakeArgs(channel, req));
	...
	public void doOnBinary(Channel channel, WebSocketFrame frame) {
		...
		if (methodMapping.getOnBinary() != null) {
			...
			Object implement = channel.attr(POJO_KEY).get();
			try {
				methodMapping.getOnBinary().invoke(implement,
					methodMapping.getOnBinaryArgs(channel, binaryWebSocketFrame));
			} catch (Throwable t) {
				logger.error(t);
			}
		}
	...
	public void doOnEvent(Channel channel, Object evt) {
		...
		if (methodMapping.getOnEvent() != null) {
			...
			Object implement = channel.attr(POJO_KEY).get();
			try {
				methodMapping.getOnEvent()
					.invoke(implement, methodMapping.getOnEventArgs(channel, evt));
			} catch (Throwable t) {
				logger.error(t);
			}
		}
