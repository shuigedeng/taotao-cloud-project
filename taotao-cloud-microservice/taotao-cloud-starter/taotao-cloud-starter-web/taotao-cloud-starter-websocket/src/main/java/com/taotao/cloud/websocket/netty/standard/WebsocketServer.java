...
import com.taotao.cloud.websocket.netty.util.SslUtils;
...
public class WebsocketServer {
	...
	public WebsocketServer(PojoEndpointServer webSocketServerHandler,
	...
	public void init() throws InterruptedException, SSLException {
		...
		if (config.isUseEventExecutorGroup()) {
			eventExecutorGroup = new DefaultEventExecutorGroup(
				config.getEventExecutorGroupThreads() == 0 ? 16
		...
		bootstrap.group(boss, worker)
			...
			.childHandler(new ChannelInitializer<NioSocketChannel>() {
				...
				protected void initChannel(NioSocketChannel ch) {
					...
					pipeline.addLast(
						new HttpServerHandler(pojoEndpointServer, config, finalEventExecutorGroup,
							corsConfig != null));
				}
			});
