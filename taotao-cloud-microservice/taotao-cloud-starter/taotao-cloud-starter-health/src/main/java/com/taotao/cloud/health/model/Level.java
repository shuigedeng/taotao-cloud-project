...
public enum Level {
	...
	private static final Map<Integer, Level> valueLookup = new ConcurrentHashMap<>(
		values().length);

	static {
		for (Level type : EnumSet.allOf(Level.class)) {
			valueLookup.put(type.level, type);
		}
	}

	public static Level resolve(Integer code) {

		return (code != null ? valueLookup.get(code) : null);
	}

	public static String resolveName(Integer code) {
		Level mode = resolve(code);
