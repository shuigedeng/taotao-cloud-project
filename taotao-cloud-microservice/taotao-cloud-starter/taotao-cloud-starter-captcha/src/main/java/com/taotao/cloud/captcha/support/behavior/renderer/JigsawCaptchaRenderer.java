...
public class JigsawCaptchaRenderer extends AbstractBehaviorRenderer<String, Coordinate> {
	...
	public boolean verify(Verification verification) {
		...
		if (ObjectUtils.isEmpty(verification) || ObjectUtils.isEmpty(
		...
		if (this.isDeflected(real.getX(), store.getX(),
			getCaptchaProperties().getJigsaw().getDeviation()) || real.getY() != store.getY()) {
	...
	private Metadata draw(BufferedImage originalImage, BufferedImage templateImage,
		...
		log.trace("Jigsaw captcha original image width is [{}], height is [{}].",
			originalImageWidth, originalImageHeight);
		log.trace("Jigsaw captcha template image width is [{}], height is [{}].",
			templateImageWidth, templateImageHeight);

		// 随机生成拼图坐标
		Coordinate coordinate = createImageMattingCoordinate(originalImageWidth,
			originalImageHeight, templateImageWidth, templateImageHeight);
		int x = coordinate.getX();
		int y = coordinate.getY();

		// 根据模版抠出新的拼图图像
		BufferedImage jigsawImage = new BufferedImage(templateImageWidth, templateImageHeight,
			templateImage.getType());
		Graphics2D graphics = jigsawImage.createGraphics();

		// 如果需要生成RGB格式，需要做如下配置,Transparency 设置透明
		jigsawImage = graphics.getDeviceConfiguration()
			.createCompatibleImage(templateImageWidth, templateImageHeight,
		...
		// 添加干扰项
		int interferencePosition = createInterferencePosition(originalImageWidth,
			templateImageWidth, x);
		...
		log.trace(" Jigsaw captcha jigsaw image width is [{}], height is [{}].",
			jigsawImage.getWidth(), jigsawImage.getHeight());
	...
	private Coordinate createImageMattingCoordinate(int originalImageWidth, int originalImageHeight,
		...
		log.debug(" Jigsaw captcha image matting coordinate is x: [{}], y: [{}].", x,
			y);
	...
	private int createInterferencePosition(int originalImageWidth, int templateImageWidth, int x) {
		...
		if (interferenceOptions > 0) {
			if (originalImageWidth - x - BOLD > templateImageWidth * 2) {
				// 在原扣图右边插入干扰图
				position = RandomProvider.randomInt(x + templateImageWidth + BOLD,
					originalImageWidth - templateImageWidth);
			} else {
				// 在原扣图左边插入干扰图
	...
	private void addInterference(BufferedImage originalImage, String sliderImageBase64,
		...
		while (true) {
			...
			if (!sliderImageBase64.equals(data)) {
				interferenceByTemplate(originalImage, Objects.requireNonNull(ImgUtil.toImage(data)),
					position, 0);
				break;
			}
		}
