 ...
 */
package com.taotao.cloud.data.mybatis.plus.query.conditions.query;
...
import static com.taotao.cloud.data.mybatis.plus.query.conditions.Wraps.replace;


/**
 * 类似 LambdaQueryWrapper 的增强 Wrapper
 * <p>
 * 相比 LambdaQueryWrapper 的增强如下： 1，new QueryWrapper(T entity)时， 对entity 中的string字段 %和_ 符号进行转义，便于模糊查询
...
public class LbqWrapper<T> extends AbstractLambdaWrapper<T, LbqWrapper<T>>
	...
	protected LbqWrapper<T> instance() {
		return new LbqWrapper<>(getEntity(), getEntityClass(), null, paramNameSeq,
			paramNameValuePairs,
...
//		}
	...
	public LbqWrapper<T> likeLeft(SFunction<T, ?> column, Object val) {
		return super.likeLeft(this.checkCondition(val), column, StrHelper.keywordConvert(
			val.toString()));
	}

	@Override
	public LbqWrapper<T> likeRight(SFunction<T, ?> column, Object val) {
		return super.likeRight(this.checkCondition(val), column, StrHelper.keywordConvert(
			val.toString()));
	}
	...
	public LbqWrapper<T> notLike(SFunction<T, ?> column, Object val) {
		return super.notLike(this.checkCondition(val), column, StrHelper.keywordConvert(
			(String) val));
	}
