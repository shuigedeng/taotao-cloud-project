...
public class AliPayPlugin implements Payment {
	...
	@Override
	public Result<Object> h5pay(HttpServletRequest request, HttpServletResponse response,
		PayParam payParam) {
		...
		//回传数据
		payModel.setPassbackParams(URLEncoder.createAll()
			.encode(BeanUtils.formatKeyValuePair(payParam), StandardCharsets.UTF_8));
		//3分钟超时
		...
		try {
			...
			AliPayRequest.wapPay(response, payModel,
	...
	public Result<Object> appPay(HttpServletRequest request, PayParam payParam) {
		try {
			...
			String orderInfo = AliPayRequest.appPayToResponse(payModel,
				notifyUrl(apiProperties.getBuyer(), PaymentMethodEnum.ALIPAY)).getBody();
	...
	public Result<Object> nativePay(HttpServletRequest request, PayParam payParam) {
		...
		try {
			...
			return Result.success(jsonObject.getJSONObject("alipay_trade_precreate_response")
				.getString("qr_code"));
		} catch (Exception e) {
			LogUtils.error("支付业务异常：", e);
			throw new BusinessException(ResultEnum.PAY_ERROR);
	...
	public void refund(RefundLog refundLog) {
		...
		//交互退款
		try {
			AlipayTradeRefundResponse alipayTradeRefundResponse = AliPayApi.tradeRefundToResponse(
				model);
			LogUtils.error("支付宝退款，参数：{},支付宝响应：{}", JSONUtil.toJsonStr(model),
				JSONUtil.toJsonStr(alipayTradeRefundResponse));
			if (alipayTradeRefundResponse.isSuccess()) {
				refundLog.setIsRefund(true);
				refundLog.setReceivableNo(refundLog.getOutOrderNo());
			} else {
				refundLog.setErrorMessage(String.format("错误码：%s,错误原因：%s",
					alipayTradeRefundResponse.getSubCode(),
	...
	public void cancel(RefundLog refundLog) {
		...
		try {
			//与阿里进行交互
			AlipayTradeCancelResponse alipayTradeCancelResponse = AliPayApi.tradeCancelToResponse(
				model);
	...
	private void callback(HttpServletRequest request) {
		try {
			...
			boolean verifyResult = AlipaySignature.rsaCertCheckV1(map,
				alipayPaymentSetting.getAlipayPublicCertPath(), "UTF-8",
				"RSA2");
			if (verifyResult) {
		...
		try {
			...
			if (verifyResult) {
				...
				PaymentSuccessParams paymentSuccessParams =
					new PaymentSuccessParams(PaymentMethodEnum.ALIPAY.name(), tradeNo,
	...
	private AlipayPaymentSetting alipayPaymentSetting() {
		AlipayPaymentSetting setting = settingApi.getAlipayPaymentSetting(
