package com.taotao.cloud.workflow.api.common.database.sql;
...
//    protected String renameSql = "ALTER TABLE ? RENAME TO ?;";
     ...
     * 增填权限
     */
    public final static String INSERT_AUTHORIZE = "INSERT INTO base_authorize (F_ID, F_ITEMTYPE, F_ITEMID, F_OBJECTTYPE, F_OBJECTID, F_SORTCODE, F_CREATORTIME, F_CREATORUSERID ) VALUES  (?,?,?,?,?,?,?,?)";
    public final static String INSERT_AUTHORIZE2 = "INSERT INTO base_authorize (F_ID, F_ITEMTYPE, F_ITEMID, F_OBJECTTYPE, F_OBJECTID, F_SORTCODE, F_CREATORTIME, F_CREATORUSERID ) VALUES  (?,?,?,?,?,?,TO_DATE(?,'yyyy-mm-dd hh24:mi:ss'),?)";
    public static String getAuthorDelSql(List<String> objectIdAll){
        return "DELETE FROM base_authorize WHERE (F_OBJECTID in( '"+String.join(",",objectIdAll)+"') AND F_ITEMTYPE <> 'portal')";
    }
    ...
    protected void setInstance(String fieldListSql, String tableListSql, String existsTableSql,
                               String fieldListParams, String tableListParams, String existsTableParams){
    ...
    public PreparedStatementDTO getExistsTablePSD(Connection conn, String table, DataSourceMod dbSourceOrDbLink) {
        DataSourceDTO dto = dbSourceOrDbLink.convertDTO();
        return new PreparedStatementDTO(conn, existsTableSql, this.dbBase.getStructParams(existsTableParams, table, dto));
    }
     ...
     * 添加表SQL
     */
    public List<PreparedStatementDTO> getCreateTablePSD(String table, String tableComment, List<DbTableFieldModel> dbTableFieldModelList) throws Exception {
...
//        return new PreparedStatementDTO(null, preparedSql, createSqlDTO.getNewTable(), createSqlDTO.getTableComment());
    ...
    public List<PreparedStatementDTO> getFieldCommentPSD(CreateSqlDTO createSqlDTO){
        ...
        for(DbTableFieldModel fieldModel : createSqlDTO.getFieldModels()){
            String  preparedSql = prepareSql.replace("{column}", fieldModel.getField())
