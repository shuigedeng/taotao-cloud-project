...
public class ContractApprovalSheetController {
	...
	@GetMapping("/{id}")
	public Result<ContractApprovalSheetInfoVO> info(@PathVariable("id") String id,
			...
			if (operator != null) {
				if (StringUtil.isNotEmpty(operator.getDraftData())) {
					vo = JsonUtils.getJsonToBean(operator.getDraftData(),
							ContractApprovalSheetInfoVO.class);
					isData = false;
				}
			}
		}
	...
	public Result create(@RequestBody @Valid ContractApprovalSheetForm contractApprovalSheetForm)
		...
		if (contractApprovalSheetForm.getIncomeAmount() != null && !"".equals(
		...
		if (contractApprovalSheetForm.getTotalExpenditure() != null && !"".equals(
				String.valueOf(contractApprovalSheetForm.getIncomeAmount()))
				&& !RegexUtils.checkDecimals2(
				String.valueOf(contractApprovalSheetForm.getTotalExpenditure()))) {
			return Result.fail("支出金额必须大于0，最多可以输入两位小数点");
		}
		ContractApprovalSheetEntity entity = JsonUtils.getJsonToBean(contractApprovalSheetForm,
				ContractApprovalSheetEntity.class);
		...
		contractApprovalSheetService.submit(entity.getId(), entity,
				contractApprovalSheetForm.getCandidateList());
	...
	@Operation("修改合同申请单表")
	@PutMapping("/{id}")
	public Result update(@RequestBody @Valid ContractApprovalSheetForm contractApprovalSheetForm,
