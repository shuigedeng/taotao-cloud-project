...
public class FlowJsonUtil {
	 ...
	 * 获取下一节点
	 **/
	public static String getNextNode(String nodeId, String data,
		List<ChildNodeList> childNodeListAll, List<ConditionList> conditionListAll) {
	 ...
	 * 下一节点id
	 **/
	private static String nextNodeId(String data, String nodeId,
		List<ChildNodeList> childNodeListAll, List<ConditionList> conditionListAll) {
		String nextId = "";
		boolean flag = false;
		ChildNodeList childNode = childNodeListAll.stream()
			.filter(t -> t.getCustom().getNodeId().equals(nodeId)).findFirst().orElse(null);
	 ...
	 * 获取当前已完成节点
	 **/
	private static void upList(List<FlowTaskNodeEntity> flowTaskNodeList, String node,
		Set<String> upList, String[] tepId) {
		FlowTaskNodeEntity entity = flowTaskNodeList.stream()
			.filter(t -> t.getNodeCode().equals(node)).findFirst().orElse(null);
		if (entity != null) {
			List<String> list = flowTaskNodeList.stream()
				.filter(t -> t.getSortCode() != null && t.getSortCode() < entity.getSortCode())
				.map(FlowTaskNodeEntity::getNodeCode).collect(Collectors.toList());
	 ...
	 * 获取当前未完成节点
	 **/
	private static void nextList(List<FlowTaskNodeEntity> flowTaskNodeList, String node,
		Set<String> nextList, String[] tepId) {
		FlowTaskNodeEntity entity = flowTaskNodeList.stream()
			.filter(t -> t.getNodeCode().equals(node)).findFirst().orElse(null);
		if (entity != null) {
			List<String> list = flowTaskNodeList.stream()
	 ...
	 * 递归条件
	 **/
	private static void getContionNextNode(String data, List<ConditionList> conditionListAll,
		String nodeId, List<String> nextNodeId) {
		List<ConditionList> conditionAll = conditionListAll.stream()
			.filter(t -> t.getPrevId().equals(nodeId)).toList();
		for (ConditionList condition : conditionAll) {
			List<ProperCond> conditions = condition.getConditions();
			boolean flag = nodeConditionDecide(data, conditions, new HashMap<>(100),
				new HashMap<>(100));
			//判断条件是否成立或者其他情况条件
			if (flag || condition.getIsDefault()) {
				String conditionId = condition.getNodeId();
				List<ConditionList> collect = conditionListAll.stream()
					.filter(t -> t.getPrevId().equals(conditionId)).toList();
				if (collect.size() > 0) {
	 ...
	 * 节点条件判断
	 **/
	private static boolean nodeConditionDecide(String formDataJson, List<ProperCond> conditionList,
		Map<String, String> flowKey, Map<String, Object> keyList) {
		boolean flag = false;
		...
		for (int i = 0; i < conditionList.size(); i++) {
			...
			if (">".equals(symbol) || ">=".equals(symbol) || "<=".equals(symbol) || "<".equals(
				symbol)) {
	 ...
	 * 最外层的json
	 **/
	public static void getTemplateAll(ChildNode childNode, List<ChildNodeList> childNodeListAll,
		...
		if (childNode != null) {
			...
			customModel.setChildNode(
				haschildNode ? childNode.getChildNode().getNodeId() : "");
	 ...
	 * 递归子节点的子节点
	 **/
	private static void getchildNode(ChildNode parentChildNodeTest, String firstId,
		List<ChildNodeList> childNodeListAll, List<ConditionList> conditionListAll,
		...
		if (childNode != null) {
			...
			customModel.setType(childNode.getType());
			boolean isFirst =
				chilNodeList.stream().anyMatch(t -> t.getNodeId().equals(nodeId));
	 ...
	 * 条件、分流递归
	 **/
	private static void conditionList(ChildNode childNode,
		String firstId,
		...
		if (conditionNodes.size() > 0) {
			...
			if (childNodeModel != null) {
				...
				ChildNode nodes = chilNodeList.stream()
					.filter(t -> t.getNodeId().equals(childNode.getNodeId())).findFirst()
					.orElse(null);
				if (nodes != null) {
			...
			for (ChildNode node : conditionNodes) {
				...
				if (conditionType) {
					...
					getConditonFlow(node, firstId, childNodeListAll, conditionListAll,
						chilNodeList);
				}
			}
		}
	 ...
	 * 条件递归
	 **/
	private static void getCondition(ChildNode childNode, String firstId,
		List<ChildNodeList> childNodeListAll, List<ConditionList> conditionListAll,
		List<ChildNode> chilNodeList) {
		if (childNode != null) {
			String nodeId = childNode.getNodeId();
			...
			boolean hasconditionNodes = childNode.getConditionNodes() != null;
			boolean isDefault =
				childNode.getProperties().getIsDefault() != null ? childNode.getProperties()
					.getIsDefault() : false;
	 ...
	 * 条件递归
	 **/
	private static void getConditonFlow(ChildNode childNode,
		String firstId,
		List<ChildNodeList> childNodeListAll,
		List<ConditionList> conditionListAll,
		List<ChildNode> chilNodeList) {
