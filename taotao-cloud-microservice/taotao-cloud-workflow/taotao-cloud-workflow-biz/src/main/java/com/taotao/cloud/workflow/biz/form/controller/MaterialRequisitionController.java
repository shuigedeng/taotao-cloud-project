...
public class MaterialRequisitionController {
	...
	@Operation("获取领料单信息")
	@GetMapping("/{id}")
	public Result<MaterialRequisitionInfoVO> info(@PathVariable("id") String id,
			...
			if (operator != null) {
				if (StringUtil.isNotEmpty(operator.getDraftData())) {
					vo = JsonUtils.getJsonToBean(operator.getDraftData(),
							MaterialRequisitionInfoVO.class);
					isData = false;
				}
			}
		}
		if (isData) {
			...
			List<MaterialEntryEntity> entityList = materialRequisitionService.getMaterialEntryList(
					id);
	...
	public Result create(@RequestBody @Valid MaterialRequisitionForm materialRequisitionForm)
			throws WorkFlowException {
		MaterialRequisitionEntity material = JsonUtils.getJsonToBean(materialRequisitionForm,
				MaterialRequisitionEntity.class);
		List<MaterialEntryEntity> materialEntryList = JsonUtils.getJsonToList(
	...
	@Operation("修改领料单")
	@PutMapping("/{id}")
	public Result update(@RequestBody @Valid MaterialRequisitionForm materialRequisitionForm,
