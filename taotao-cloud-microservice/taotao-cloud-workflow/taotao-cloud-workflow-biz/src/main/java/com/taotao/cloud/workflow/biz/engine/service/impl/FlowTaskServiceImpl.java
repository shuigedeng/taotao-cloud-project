...
@Slf4j
@Service
public class FlowTaskServiceImpl extends ServiceImpl<FlowTaskMapper, FlowTaskEntity> implements
	FlowTaskService {
	...
	public List<FlowTaskEntity> getMonitorList(PaginationFlowTask paginationFlowTask) {
		...
		String startTime =
			paginationFlowTask.getStartTime() != null ? paginationFlowTask.getStartTime() : null;
		String endTime =
			paginationFlowTask.getEndTime() != null ? paginationFlowTask.getEndTime() : null;
		if (!StringUtils.isEmpty(startTime) && !StringUtils.isEmpty(endTime)) {
			flag = true;
			Date startTimes = DateUtil.stringToDate(
				DateUtil.daFormatYmd(Long.parseLong(startTime)) + " 00:00:00");
			Date endTimes = DateUtil.stringToDate(
				DateUtil.daFormatYmd(Long.parseLong(endTime)) + " 23:59:59");
			queryWrapper.lambda().ge(FlowTaskEntity::getCreatorTime, startTimes)
		...
		//所属分类
		String flowCategory =
			paginationFlowTask.getFlowCategory() != null ? paginationFlowTask.getFlowCategory()
				: null;
		...
		String creatorUserId =
			paginationFlowTask.getCreatorUserId() != null ? paginationFlowTask.getCreatorUserId()
				: null;
...
//        } else {
		queryWrapper.lambda().orderByAsc(FlowTaskEntity::getSortCode)
			.orderByDesc(FlowTaskEntity::getCreatorTime);
//        }
		if (flag) {
			queryWrapper.lambda().orderByDesc(FlowTaskEntity::getLastModifyTime);
		}
		Page<FlowTaskEntity> page = new Page<>(paginationFlowTask.getCurrentPage(),
			paginationFlowTask.getPageSize());
	...
	public List<FlowTaskEntity> getLaunchList(PaginationFlowTask paginationFlowTask) {
		...
		if ("asc".equals(paginationFlowTask.getSort().toLowerCase())) {
			queryWrapper.lambda().orderByAsc(FlowTaskEntity::getStatus)
		...
		if (!flowTaskEntityPage.getRecords().isEmpty()) {
			List<String> ids = flowTaskEntityPage.getRecords().stream().map(m -> m.getId())
	...
	public List<FlowTaskListModel> getWaitListAll(PaginationFlowTask paginationFlowTask) {
		...
		if (!StringUtils.isEmpty(keyWord)) {
			dbSql.append(
				" AND (t.F_EnCode like '%" + keyWord + "%' or t.F_FullName like '%" + keyWord
					+ "%') ");
		}
		...
		if (!StringUtils.isEmpty(startTime) && !StringUtils.isEmpty(endTime)) {
			if (DbTypeUtil.checkOracle(dataSourceUtil)) {
				...
				dbSql.append(" AND o.F_CreatorTime Between TO_DATE('" + startTimes
					+ "','yyyy-mm-dd HH24:mi:ss') AND TO_DATE('" + endTimes
					+ "','yyyy-mm-dd HH24:mi:ss') ");
			} else {
				String startTimes = DateUtil.daFormatYmd(Long.parseLong(startTime)) + " 00:00:00";
				String endTimes = DateUtil.daFormatYmd(Long.parseLong(endTime)) + " 23:59:59";
				dbSql.append(
					" AND o.F_CreatorTime Between '" + startTimes + "' AND '" + endTimes + "' ");
			}
		}
		//所属流程
		String flowId =
		...
		for (FlowTaskListModel entity : data) {
			List<Date> list =
				StringUtils.isNotEmpty(entity.getDescription()) ? JsonUtils.getJsonToList(
					entity.getDescription(), Date.class) : new ArrayList<>();
			...
			List<FlowDelegateEntity> flowList = flowDelegateList.stream()
				.filter(t -> t.getFlowId().equals(entity.getFlowId())).toList();
			//判断是否有自己审核
			if (!isuser) {
	...
	public IPage<FlowTaskListModel> getTrialList(PaginationFlowTask paginationFlowTask) {
		...
		if (!StringUtils.isEmpty(startTime) && !StringUtils.isEmpty(endTime)) {
			if (DbTypeUtil.checkOracle(dataSourceUtil)) {
				String startTimes = DateUtil.daFormatYmd(Long.parseLong(startTime)) + " 00:00:00";
				String endTimes = DateUtil.daFormatYmd(Long.parseLong(endTime)) + " 23:59:59";
				dbSql.append(" AND r.F_HandleTime Between TO_DATE('" + startTimes
					+ "','yyyy-mm-dd HH24:mi:ss') AND TO_DATE('" + endTimes
					+ "','yyyy-mm-dd HH24:mi:ss') ");
			} else {
				String startTimes = DateUtil.daFormatYmd(Long.parseLong(startTime)) + " 00:00:00";
	...
	public IPage<FlowTaskListModel> getCirculateList(PaginationFlowTask paginationFlowTask) {
		...
		List<String> userRelationList = list.stream().map(u -> u.getObjectId())
			.collect(Collectors.toList());
		...
		if (!StringUtils.isEmpty(keyWord)) {
			dbSql.append(
				" AND (t.F_EnCode like " + " '%" + keyWord + "%' " + " or t.F_FullName like" + " '%"
					+ keyWord + "%') ");
		}
		//日期范围（近7天、近1月、近3月、自定义）
		String startTime =
		...
		if (!StringUtils.isEmpty(startTime) && !StringUtils.isEmpty(endTime)) {
			if (DbTypeUtil.checkOracle(dataSourceUtil)) {
				String startTimes = DateUtil.daFormatYmd(Long.parseLong(startTime)) + " 00:00:00";
				String endTimes = DateUtil.daFormatYmd(Long.parseLong(endTime)) + " 23:59:59";
				dbSql.append(" AND c.F_CreatorTime Between TO_DATE('" + startTimes
					+ "','yyyy-mm-dd HH24:mi:ss') AND TO_DATE('" + endTimes
					+ "','yyyy-mm-dd HH24:mi:ss') ");
			} else {
				String startTimes = DateUtil.daFormatYmd(Long.parseLong(startTime)) + " 00:00:00";
				String endTimes = DateUtil.daFormatYmd(Long.parseLong(endTime)) + " 23:59:59";
				dbSql.append(
					" AND c.F_CreatorTime Between  '" + startTimes + "' AND '" + endTimes + "' ");
			}
		}
		//所属流程
		String flowId =
	...
	public FlowTaskEntity getInfo(String id) {
		...
		if (entity == null) {
			queryWrapper = new QueryWrapper<>();
			queryWrapper.lambda().and(
				t -> t.eq(FlowTaskEntity::getProcessId, id)
			);
	...
	@Override
	public List<FlowTaskEntity> getInfosSubmit(String[] ids,
		SFunction<FlowTaskEntity, ?>... columns) {
	...
	public List<FlowBatchModel> batchFlowSelector() {
		...
		List<String> taskIdList = operatorList.stream().map(FlowTaskOperatorEntity::getTaskId)
			.collect(Collectors.toList());
		...
		for (String key : flowIdList.keySet()) {
			...
			List<String> flowTask = flowTaskList.stream().map(FlowTaskEntity::getId)
				.toList();
			if (flowTaskList.size() > 0) {
				String flowName = flowTaskList.stream().map(FlowTaskEntity::getFlowName).distinct()
					.collect(Collectors.joining(","));
				String flowId = flowTaskList.stream().map(FlowTaskEntity::getFlowId).distinct()
					.collect(Collectors.joining(","));
				Long count = operatorList.stream().filter(t -> flowTask.contains(t.getTaskId()))
					.count();
