...
import com.taotao.cloud.workflow.biz.engine.model.DataModel;
...
public class FlowDataUtil {
	...
	private String getKey(Connection conn, String mainTable) throws SQLException {
		...
		@Cleanup ResultSet primaryKeyResultSet = conn.getMetaData()
			.getPrimaryKeys(catalog, null, mainTable);
	...
	public Map<String, Object> info(List<FieLdsModel> fieLdslist, FlowTaskEntity entity,
		List<TableModel> tableList, boolean convert, DbLinkEntity link) throws WorkFlowException {
		Map<String, Object> data =
			StrUtil.isNotEmpty(entity.getFlowFormContentJson()) ? JsonUtils.stringToMap(
				entity.getFlowFormContentJson()) : new HashMap<>(16);
		DataModel dataModel = new DataModel(data, fieLdslist, tableList, entity.getId(), link,
			convert);
	...
	private Map<String, Object> tableData(DataModel dataModel, List<FormAllModel> formAllModel)
		...
		try {
			...
			//主表数据
			String mastInfo =
				" select " + String.join(",", mastFile) + " from " + mastTableName + " where "
					+ pKeyName + " = '" + mainId + "'";
			...
			for (String key : mastData.keySet()) {
				...
				FormAllModel formAll = mastForm.stream().filter(t -> key.equals(
						t.getFormColumnModel().getFieLdsModel().getVModel().toLowerCase())).findFirst()
					.orElse(null);
				if (formAll != null) {
			...
			for (FormAllModel model : tableForm) {
				...
				String tableInfo =
					"select " + String.join(",", childFile) + " from " + tableName + " where "
						+ table.getTableField() + "='" + mainId + "' order by " + childKey + " asc";
			...
			for (String key : mastTableAll.keySet()) {
				...
				List<String> field = mastTableList.stream()
					.filter(t -> StrUtil.isNotEmpty(t.getFormMastTableModel().getField()))
					.map(t -> t.getFormMastTableModel().getField()).collect(Collectors.toList());
				String mastTableInfo =
					"select " + String.join(",", field) + " from " + table + " where "
						+ tableModel.getTableField() + "='" + mainId + "'";
				...
				for (String mastKey : dataAll.keySet()) {
					...
					FieLdsModel fieLdsModel = mastTableList.stream().filter(
	...
	private Map<String, Object> data(DataModel dataModel, List<FormAllModel> formAllModel) {
		...
		for (String key : dataMap.keySet()) {
			FormAllModel model = mastForm.stream()
				.filter(t -> key.equals(t.getFormColumnModel().getFieLdsModel().getVModel()))
				.findFirst().orElse(null);
			if (model != null) {
				...
				FormAllModel childModel = tableForm.stream()
					.filter(t -> key.equals(t.getChildList().getTableModel())).findFirst()
					.orElse(null);
				if (childModel != null) {
					String childKeyName = childModel.getChildList().getTableModel();
					List<Map<String, Object>> childDataMap = (List<Map<String, Object>>) dataMap.get(
						key);
					...
					for (Map<String, Object> child : childDataMap) {
						...
						for (String childKey : child.keySet()) {
							FormColumnModel columnModel = childModel.getChildList().getChildList()
								.stream()
								.filter(t -> childKey.equals(t.getFieLdsModel().getVModel()))
								.findFirst().orElse(null);
							if (columnModel != null) {
		...
		switch (flowKey) {
				...
				if (ObjectUtil.isNotEmpty(value)) {
					...
					if ("all".equals(showLevel)) {
						if (organizeEntity != null) {
							List<OrganizeEntity> organizeList = serviceUtil.getOrganizeId(
								organizeEntity.getId());
							value = organizeList.stream().map(OrganizeEntity::getFullName)
			...
			case FlowKeyConsts.CASCADER:
				if (value == null) {
					...
					if (isTable) {
						PropsBeanModel propsModel = JsonUtils.toObject(
							fieLdsModel.getProps().getProps(), PropsBeanModel.class);
						if (propsModel.getMultiple()) {
							value = JsonUtils.toObject(String.valueOf(value),
								String[][].class);
						} else {
							value = JsonUtils.toObject(String.valueOf(value), String.class);
						}
	...
	public Map<String, Object> create(Map<String, Object> allDataMap,
		List<FieLdsModel> fieLdsModelList, List<TableModel> tableModelList, String mainId,
		Map<String, String> billData, DbLinkEntity link) throws WorkFlowException {
	...
	private Map<String, Object> createDataList(DataModel dataModel, List<FormAllModel> formAllModel)
		...
		if (tableModelList.size() > 0) {
			DbLinkEntity link = dataModel.getLink();
			boolean isOracle = (DbTypeUtil.checkOracle(dataSourceUtil) || DbTypeUtil.checkPostgre(
				dataSourceUtil));
			if (link != null) {
			...
			//子表
			this.createTable(formAllModel, tableModelList, isOracle, dataNewMap, conn, mainId,
				result);
			//副表
			this.createMastTable(formAllModel, tableModelList, isOracle, dataNewMap, conn, mainId,
				result);
			//主表
			this.createMast(formAllModel, tableModelList, isOracle, dataNewMap, conn, mainId,
				result);
		} else {
			//无表数据处理
	 ...
	 * 子表数据
	 **/
	private void createTable(List<FormAllModel> formAllModel, List<TableModel> tableModelList,
		boolean isOracle, Map<String, Object> allDataMap, Connection conn, String mainId,
		...
		tableForm.forEach(t -> {
			...
			Optional<TableModel> first = tableModelList.stream()
				.filter(k -> k.getTable().equals(tableName)).findFirst();
			if (first.isPresent()) {
		...
		for (String key : childMap.keySet()) {
			...
			List<Map<String, Object>> chidList =
				allDataMap.get(key) != null ? (List<Map<String, Object>>) allDataMap.get(key)
					: new ArrayList<>();
	 ...
	 * 副表数据
	 **/
	private void createMastTable(List<FormAllModel> formAllModel, List<TableModel> tableModelList,
		boolean isOracle, Map<String, Object> allDataMap, Connection conn, String mainId,
		Map<String, Object> result) throws SQLException {
		//副表
		Map<String, List<FormAllModel>> mastTableAll = formAllModel.stream()
		...
		for (String key : mastTableAll.keySet()) {
			...
			//新增sql语句
			String sql =
				"insert into " + tableModelTable + "(" + filedModel + ") values (" + filedValue
					+ ")";
	 ...
	 * 主表数据
	 **/
	private void createMast(List<FormAllModel> formAllModel, List<TableModel> tableModelList,
		boolean isOracle, Map<String, Object> allDataMap, Connection conn, String mainId,
		Map<String, Object> result) throws SQLException {
	 ...
	 * 无表插入数据
	 **/
	private Map<String, Object> createAll(Map<String, Object> dataNewMap,
		...
		for (String key : dataNewMap.keySet()) {
			...
			if (model != null) {
				...
				if (childModel != null) {
					...
					List<Map<String, Object>> childDataMap = (List<Map<String, Object>>) dataNewMap.get(
						key);
					//子表处理的数据
	 ...
	 * 修改数据处理
	 **/
	public Map<String, Object> update(Map<String, Object> allDataMap,
		List<FieLdsModel> fieLdsModelList, List<TableModel> tableModelList, String mainId,
	...
	private Map<String, Object> updateDataList(DataModel dataModel, List<FormAllModel> formAllModel)
		...
		if (tableModelList.size() > 0) {
			...
			conn.setAutoCommit(false);
			//子表
			this.updateTable(formAllModel, tableModelList, isOracle, dataNewMap, conn, mainId,
				result);
			//副表
			this.updateMastTable(formAllModel, tableModelList, isOracle, dataNewMap, conn, mainId,
				result);
			//主表
			this.updateMast(formAllModel, tableModelList, isOracle, dataNewMap, conn, mainId,
				result);
		} else {
			//无表数据处理
	 ...
	 * 子表数据
	 **/
	private void updateTable(List<FormAllModel> formAllModel, List<TableModel> tableModelList,
		boolean isOracle, Map<String, Object> dataNewMap, Connection conn, String mainId,
		Map<String, Object> result) throws SQLException {
		//子表
		List<FormAllModel> tableForm = formAllModel.stream()
			.filter(t -> FormEnum.table.getMessage().equals(t.getFlowKey()))
		...
		for (String key : childMap.keySet()) {
			//子表数据
			List<Map<String, Object>> chidList =
				dataNewMap.get(key) != null ? (List<Map<String, Object>>) dataNewMap.get(key)
					: new ArrayList<>();
			List<FormColumnModel> formColumnModels = childMap.get(key);
			Map<String, FieLdsModel> columMap = new HashMap<>();
			//获取子表对象的类型
			...
			//返回值
			result.put(key, childResultAll);
			String[] del = new String[]{"delete from " + table + " where " + mastKeyName + " = ?",
				mainId};
			String childSql =
				"insert into " + table + "(" + filedModel + ") values (" + filedValue + ")";
			this.sql(childSql, childDataAll, del, conn, false);
	 ...
	 * 副表数据
	 **/
	private void updateMastTable(List<FormAllModel> formAllModel, List<TableModel> tableModelList,
		boolean isOracle, Map<String, Object> dataNewMap, Connection conn, String mainId,
		Map<String, Object> result) throws SQLException {
		//副表
		Map<String, List<FormAllModel>> mastTableAll = formAllModel.stream()
	 ...
	 * 主表数据
	 **/
	private void updateMast(List<FormAllModel> formAllModel, List<TableModel> tableModelList,
		boolean isOracle, Map<String, Object> dataNewMap, Connection conn, String mainId,
		Map<String, Object> result) throws SQLException {
		Optional<TableModel> first = tableModelList.stream().filter(t -> "1".equals(t.getTypeId()))
			.findFirst();
	 ...
	 * 修改无表数据
	 **/
	private Map<String, Object> updateAll(Map<String, Object> dataNewMap,
		List<FormAllModel> formAllModel) {
		//处理好的数据
		Map<String, Object> result = new HashMap<>(16);
		//系统数据
	 ...
	 * 执行sql语句
	 **/
	private void sql(String sql, List<List<Object>> dataAll, String[] del, Connection conn,
		boolean isCommit) throws SQLException {
		try {
			if (del.length > 0) {
