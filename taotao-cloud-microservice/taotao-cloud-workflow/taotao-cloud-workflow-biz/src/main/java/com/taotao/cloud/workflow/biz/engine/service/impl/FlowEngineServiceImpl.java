...
import com.taotao.cloud.common.utils.common.CodeUtil;
 ...
 */
@Service
public class FlowEngineServiceImpl extends
	ServiceImpl<FlowEngineMapper, FlowEngineEntity> implements FlowEngineService {
	...
	public IPage<FlowEngineEntity> getPageList(FlowPagination pagination) {
		...
		List<String> id = engineEntities.stream().map(FlowEngineEntity::getId)
		...
		Page<FlowEngineEntity> page = new Page<>(pagination.getCurrentPage(),
	...
	public List<FlowEngineEntity> getList(PaginationFlowEngine pagination) {
		...
		queryWrapper
			.lambda()
			.select(FlowEngineEntity::getId, FlowEngineEntity::getEnCode,
	...
	public List<FlowEngineEntity> getListAll(FlowPagination pagination, boolean isPage) {
		...
		List<String> formTypeId = flowFormTypeList.stream().map(FlowEngineEntity::getId)
			.toList();
	...
	public boolean updateVisible(String id, FlowEngineEntity entity) throws WorkFlowException {
		...
		BigDecimal b1 = new BigDecimal(
			StrUtil.isEmpty(info.getVersion()) ? "0" : info.getVersion());
	...
	public List<FlowEngineListVO> getTreeList(PaginationFlowEngine pagination, boolean isList) {
		...
		Map<String, List<FlowEngineEntity>> dataList = data.stream()
			.collect(Collectors.groupingBy(FlowEngineEntity::getCategory));
		...
		for (DictionaryDataEntity entity : dictionList) {
			...
			List<FlowEngineEntity> childList =
				dataList.get(entity.getEnCode()) != null ? dataList.get(entity.getEnCode())
					: new ArrayList<>();
	...
	private List<FlowEngineVisibleEntity> visibleList(FlowEngineEntity entity) {
		...
		if (entity.getFlowTemplateJson() != null) {
			ChildNode childNode = JsonUtils.toObject(entity.getFlowTemplateJson(),
				ChildNode.class);
			...
			List<String> roleList =
				properties.getInitiateRole() != null ? properties.getInitiateRole()
					: new ArrayList<>();
	...
	public FlowExportModel exportData(String id) throws WorkFlowException {
		...
		List<FlowEngineVisibleEntity> visibleList = flowEngineVisibleService.getVisibleFlowList(
			entity.getId());
