...
public class ContractApprovalController {
		...
		if (StringUtil.isNotEmpty(taskOperatorId)) {
			...
			if (operator != null) {
				if (StringUtil.isNotEmpty(operator.getDraftData())) {
					vo = JsonUtil.getJsonToBean(operator.getDraftData(),
							ContractApprovalInfoVO.class);
					isData = false;
				}
			}
		}
	...
	public Result create(@RequestBody @Valid ContractApprovalForm contractApprovalForm)
		...
		ContractApprovalEntity entity = JsonUtil.getJsonToBean(contractApprovalForm,
				ContractApprovalEntity.class);
		...
		contractApprovalService.submit(entity.getId(), entity,
	...
	@Operation("修改合同审批")
	@PutMapping("/{id}")
	public Result update(@RequestBody @Valid ContractApprovalForm contractApprovalForm,
