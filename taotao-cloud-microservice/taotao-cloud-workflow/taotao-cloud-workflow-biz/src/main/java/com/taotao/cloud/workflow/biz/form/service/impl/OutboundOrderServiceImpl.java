...
public class OutboundOrderServiceImpl extends ServiceImpl<OutboundOrderMapper, OutboundOrderEntity> implements OutboundOrderService {
    ...
    @Override
    @DSTransactional
    public void submit(String id, OutboundOrderEntity entity, List<OutboundEntryEntity> outboundEntryEntityList, Map<String, List<String>> candidateList) throws WorkFlowException {
        //表单信息
        if (id == null) {
            entity.setId(RandomUtil.uuId());
            for (int i = 0; i < outboundEntryEntityList.size(); i++) {
