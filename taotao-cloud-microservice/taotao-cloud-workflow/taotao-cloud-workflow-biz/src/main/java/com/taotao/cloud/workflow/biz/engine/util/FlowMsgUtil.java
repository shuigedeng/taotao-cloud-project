...
import com.taotao.cloud.workflow.biz.engine.model.flowengine.shuntjson.childnode.FuncConfig;
import com.taotao.cloud.workflow.biz.engine.model.flowengine.shuntjson.childnode.MsgConfig;
import com.taotao.cloud.workflow.biz.engine.model.flowengine.shuntjson.childnode.Properties;
import com.taotao.cloud.workflow.biz.engine.model.flowengine.shuntjson.childnode.TemplateJsonModel;
import com.taotao.cloud.workflow.biz.engine.model.flowengine.shuntjson.nodejson.ChildNodeList;
import com.taotao.cloud.workflow.biz.engine.model.flowmessage.FlowEventModel;
import com.taotao.cloud.workflow.biz.engine.model.flowmessage.FlowMessageModel;
import com.taotao.cloud.workflow.biz.engine.model.flowmessage.FlowMsgModel;
import com.taotao.cloud.workflow.biz.engine.model.flowtask.FlowContModel;
...
public class FlowMsgUtil {
	...
	public void message(FlowMsgModel flowMsgModel) {
		...
		FlowTaskNodeEntity startNode = nodeList.stream()
			.filter(t -> FlowNature.NodeStart.equals(t.getNodeType())).findFirst().orElse(null);
		...
		if (flowMsgModel.isWait()) {
			ChildNodeList childNode = JsonUtils.getJsonToBean(startNode.getNodePropertyJson(),
				ChildNodeList.class);
			Properties properties = childNode.getProperties();
			MsgConfig taskMsgConfig = properties.getWaitMsgConfig();
			Map<String, List<FlowTaskOperatorEntity>> operatorMap = operatorList.stream()
			...
			for (String key : operatorMap.keySet()) {
				...
				for (FlowTaskOperatorEntity operator : taskOperatorList) {
					List<SentMessageForm> delegationMsg = this.delegationMsg(operator, messageModel,
						engine);
		...
		if (flowMsgModel.isApprove()) {
			...
			for (String key : operatorMap.keySet()) {
				recordEntity.setTaskNodeId(key);
				//默认获取当前节点
				FlowTaskNodeEntity taskNode = nodeList.stream().filter(t -> t.getId().equals(key))
					.findFirst().orElse(null);
				ChildNodeList taskChildNode = JsonUtils.getJsonToBean(
		...
		if (flowMsgModel.isCopy()) {
			...
			Map<String, List<FlowTaskCirculateEntity>> circulateMap = circulateList.stream()
			...
			for (String key : circulateMap.keySet()) {
				...
				for (FlowTaskCirculateEntity circulateEntity : circulateMap.get(key)) {
					FlowTaskOperatorEntity operatorEntity = JsonUtils.getJsonToBean(circulateEntity,
	 ...
	 * @param messageModel
	 */
	private void messageModel(List<FlowTaskOperatorEntity> taskOperatorList,
		FlowEngineEntity engine, FlowMessageModel messageModel) {
	 ...
	 * @return
	 */
	private FlowContModel flowMessage(FlowEngineEntity engine, FlowTaskOperatorEntity taskOperator,
	 ...
	 */
	private void setMessageList(List<SentMessageForm> messageList,
		FlowMessageModel flowMessageModel) {
		Map<String, Object> data = flowMessageModel.getData();
		MsgConfig msgConfig =
			flowMessageModel.getMsgConfig() != null ? flowMessageModel.getMsgConfig()
				: new MsgConfig();
		...
		for (TemplateJsonModel templateJsonModel : templateJson) {
			...
			FlowEventModel eventModel = FlowEventModel.builder().dataJson(dataJson)
				.record(recordEntity).relationField(relationField).build();
	...
	public String data(FlowEventModel eventModel) {
		...
		FlowTaskEntity taskEntity = flowTaskService.getInfoSubmit(record.getTaskId(),
			FlowTaskEntity::getFlowId
			, FlowTaskEntity::getFlowName, FlowTaskEntity::getFullName,
		...
		switch (relationField) {
			...
			case "flowLaunchUserName":
				UserEntity createUser =
					taskEntity != null ? serviceUtil.getUserInfo(taskEntity.getCreatorUserId())
						: null;
	 ...
	 * @param record    审批数据
	 */
	public void event(Integer status, ChildNodeList childNode, FlowTaskOperatorRecordEntity record,
		FlowModel flowModel) {
		boolean on = false;
		String interId = "";
		...
		if (on && StrUtil.isNotEmpty(interId)) {
			...
			for (TemplateJsonModel templateJsonModel : templateJsonModelList) {
				...
				String dataJson =
					data.get(relationField) != null ? String.valueOf(data.get(relationField)) : "";
				FlowEventModel eventModel = FlowEventModel.builder().dataJson(dataJson)
					.record(record).relationField(relationField).build();
	 ...
	 * @return
	 */
	private List<SentMessageForm> delegationMsg(FlowTaskOperatorEntity operator,
		FlowMessageModel messageModel, FlowEngineEntity engine) {
		List<SentMessageForm> messageList = new ArrayList<>();
		FlowTaskEntity taskEntity = flowTaskService.getInfoSubmit(operator.getTaskId(),
			FlowTaskEntity::getFlowId);
		List<String> userList = flowDelegateService.getUser(null, taskEntity.getFlowId(),
				operator.getHandleId()).stream().map(t -> t.getFTouserid())
			.collect(Collectors.toList());
		List<FlowTaskOperatorEntity> taskOperatorList = new ArrayList<>();
		for (String user : userList) {
			FlowTaskOperatorEntity delegaOperator = JsonUtils.getJsonToBean(operator,
				FlowTaskOperatorEntity.class);
