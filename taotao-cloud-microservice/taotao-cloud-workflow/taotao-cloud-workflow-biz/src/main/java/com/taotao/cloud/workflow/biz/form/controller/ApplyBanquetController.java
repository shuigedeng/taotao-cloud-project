...
public class ApplyBanquetController {
	...
	public Result create(@RequestBody @Valid ApplyBanquetForm applyBanquetForm)
		...
		if (applyBanquetForm.getBanquetNum() != null && StringUtil.isNotEmpty(
				applyBanquetForm.getBanquetNum()) && !RegexUtils.checkDigit2(
				applyBanquetForm.getBanquetNum())) {
			return Result.fail("宴请人数必须大于0");
		}
		if (applyBanquetForm.getTotal() != null && StringUtil.isNotEmpty(
				applyBanquetForm.getTotal()) && !RegexUtils.checkDigit2(
				applyBanquetForm.getTotal())) {
			return Result.fail("人员总数必须大于0");
		}
		if (applyBanquetForm.getExpectedCost() != null && !RegexUtils.checkDecimals2(
				String.valueOf(applyBanquetForm.getExpectedCost()))) {
			return Result.fail("预计费用必须大于0，最多只能有两位小数");
		}
		ApplyBanquetEntity entity = JsonUtils.getJsonToBean(applyBanquetForm,
				ApplyBanquetEntity.class);
	...
	@Operation("修改宴请申请")
	@PutMapping("/{id}")
	public Result update(@RequestBody @Valid ApplyBanquetForm applyBanquetForm,
