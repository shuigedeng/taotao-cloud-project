...
public class CouponRender implements ICartRenderStep {
	...
	private void checkMemberExistCoupon(TradeDTO tradeDTO, List<MemberCoupon> memberCouponList) {
		if (tradeDTO.getPlatformCoupon() != null
			...
			boolean b = memberCouponList.parallelStream().anyMatch(
				i -> i.getId().equals(tradeDTO.getPlatformCoupon().getMemberCoupon().getId()));
			if (!b) {
				tradeDTO.setPlatformCoupon(null);
			}
		}
		if (!tradeDTO.getStoreCoupons().isEmpty()) {
			for (Map.Entry<String, MemberCouponDTO> entry : tradeDTO.getStoreCoupons().entrySet()) {
				if (entry.getValue().getMemberCoupon() != null && memberCouponList.parallelStream()
					.noneMatch(i -> i.getId().equals(entry.getValue().getMemberCoupon().getId()))) {
	...
	private List<CartSkuVO> filterSkuVo(List<CartSkuVO> cartSkuVOS, MemberCoupon memberCoupon) {
		...
		switch (PromotionsScopeTypeEnum.valueOf(memberCoupon.getScopeType())) {
			...
			case PORTION_GOODS:
				//按照商品过滤
				filterSku = filterSku.stream().filter(cartSkuVO -> memberCoupon.getScopeId()
					.contains(cartSkuVO.getGoodsSku().getId())).collect(Collectors.toList());
				break;
	 ...
	 */
	private List<CartSkuVO> filterPromotionShopCategory(List<CartSkuVO> filterSku,
														MemberCoupon memberCoupon) {
		return filterSku.stream().filter(cartSkuVO -> {
			if (CharSequenceUtil.isNotEmpty(cartSkuVO.getGoodsSku().getStoreCategoryPath())) {
				//获取店铺分类
				String[] storeCategoryPath = cartSkuVO.getGoodsSku().getStoreCategoryPath()
					.split(",");
	 ...
	 */
	private void renderCouponPrice(Map<String, BigDecimal> couponMap, TradeDTO tradeDTO,
								   MemberCoupon coupon, MemberCouponDTO memberCouponDTO) {
		//分发优惠券
	 ...
	 * @param coupon    优惠券信息
	 */
	private void renderCouponDiscount(Map<String, BigDecimal> couponMap, TradeDTO tradeDTO,
									  MemberCoupon coupon) {
		//循环所有优惠券
		for (String skuId : couponMap.keySet()) {
