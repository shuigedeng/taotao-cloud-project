...
public class AfterSaleServiceImpl extends ServiceImpl<AfterSaleMapper, AfterSale> implements
	...
	public Traces deliveryTraces(String afterSaleSn) {
		...
		return logisticsService.getLogistic(afterSale.getMLogisticsCode(),
			afterSale.getMLogisticsNo());
	}

	@Override
	@AfterSaleLogPoint(sn = "#afterSaleSn", description =
	...
	private void updateOrderItemAfterSaleStatus(AfterSale afterSale) {
		...
		AfterSaleStatusEnum afterSaleStatusEnum = AfterSaleStatusEnum.valueOf(
			afterSale.getServiceStatus());
	...
	private void sendAfterSaleMessage(AfterSale afterSale) {
		//发送售后创建消息
		String destination = rocketmqCustomProperties.getAfterSaleTopic() + ":"
			+ AfterSaleTagsEnum.AFTER_SALE_STATUS_CHANGE.name();
		//发送订单变更mq消息
		rocketMQTemplate.asyncSend(destination, JSONUtil.toJsonStr(afterSale),
	...
	private void updateOrderItemGoodsNumber(OrderItem orderItem, List<AfterSale> afterSaleList) {
		...
		List<AfterSale> implementList = afterSaleList.stream()
			.filter(
				afterSale -> afterSale.getServiceStatus().equals(AfterSaleStatusEnum.APPLY.name())
					|| afterSale.getServiceStatus().equals(AfterSaleStatusEnum.PASS.name())
		...
		if (!implementList.isEmpty()) {
			...
			implementList.forEach(
				a -> orderItem.setReturnGoodsNumber(orderItem.getReturnGoodsNumber() + a.getNum()));
		}

		//获取已完成售后订单数量
		List<AfterSale> completeList = afterSaleList.stream()
			.filter(afterSale -> afterSale.getServiceStatus()
