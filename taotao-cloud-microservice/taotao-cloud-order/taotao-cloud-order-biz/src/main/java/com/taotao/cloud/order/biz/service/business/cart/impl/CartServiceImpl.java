...
public class CartServiceImpl implements ICartService {
	...
	public Long getCanUseCoupon(CartTypeEnum checkedWay) {
		...
		if (tradeDTO.getSkuList() != null && !tradeDTO.getSkuList().isEmpty()) {
			...
			for (EsGoodsIndex esGoodsIndex : esGoodsList) {
				if (esGoodsIndex != null) {
					if (esGoodsIndex.getPromotionMap() != null) {
						List<String> couponIds = esGoodsIndex.getPromotionMap().keySet()
							.parallelStream()
							.filter(i -> i.contains(PromotionTypeEnum.COUPON.name()))
							.map(i -> i.substring(i.lastIndexOf("-") + 1))
							.collect(Collectors.toList());
						if (!couponIds.isEmpty()) {
							List<MemberCoupon> currentGoodsCanUse = memberCouponService.getCurrentGoodsCanUse(
			...
			//获取可操作的优惠券集合
			List<MemberCoupon> allScopeMemberCoupon = memberCouponService.getAllScopeMemberCoupon(
				tradeDTO.getMemberId(), storeIds);
			if (allScopeMemberCoupon != null && !allScopeMemberCoupon.isEmpty()) {
				//过滤满足消费门槛
				count += allScopeMemberCoupon.stream()
					.filter(i -> i.getConsumeThreshold() <= totalPrice).count();
			}
		}
		return count;
	}
	...
	public Boolean shippingAddress(String shippingAddressId, String way) {
		...
		MemberAddressVO memberAddress = memberAddressService.getById(shippingAddressId);
		...
		if (cartPrice >= memberCoupon.getConsumeThreshold()) {
			...
			if (memberCoupon.getIsPlatform()) {
				...
				tradeDTO.getStoreCoupons()
					.put(memberCoupon.getStoreId(), new MemberCouponDTO(skuPrice, memberCoupon));
			}
		}
	...
	private void checkKanjia(CartSkuVO cartSkuVO) {
		...
		KanjiaActivity kanjiaActivity = kanjiaActivityService.getKanjiaActivity(
