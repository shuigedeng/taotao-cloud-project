...
public class ChatTopicServiceImpl extends BaseServiceImpl<ChatTopic> implements ChatTopicService {
	...
	public void like(Long topicId) {
		...
		// 点赞操作
		chatTopicLikeService.add(new ChatTopicLike().setTopicId(topicId).setUserId(userId)
		...
		PushParamVo paramVo = new PushParamVo().setToId(topic.getUserId())
	...
	private TopicVo06 replyReply(TopicVo07 topicVo) {
		...
		if (!reply.getUserId().equals(userId)) {
			PushParamVo paramVo = new PushParamVo().setToId(reply.getUserId())
					.setPortrait(fromUser.getPortrait());
			chatPushService.pushNotice(paramVo, PushNoticeTypeEnum.TOPIC_REPLY);
			// 用户推送
	 ...
	 * 执行添加
	 */
	private void addNotice(Long userId, ChatTopic topic, ChatUser fromUser,
			TopicNoticeTypeEnum noticeType, String content) {
		ThreadUtil.execAsync(() -> {
			...
			redisUtils.lLeftPush(ApiConstant.REDIS_TOPIC_REPLY + userId,
					JSONUtil.toJsonStr(topicVo));
		});
	}
	...
	private YesOrNoEnum selfLike(List<TopicVo05> likeList) {
		Long userId = ShiroUtils.getUserId();
		List<Long> userList = likeList.stream().map(TopicVo05::getUserId)
