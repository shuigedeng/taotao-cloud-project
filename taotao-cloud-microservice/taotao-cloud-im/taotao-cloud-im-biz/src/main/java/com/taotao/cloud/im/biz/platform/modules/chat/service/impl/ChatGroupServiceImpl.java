...
public class ChatGroupServiceImpl extends BaseServiceImpl<ChatGroup> implements ChatGroupService {
	...
	public void createGroup(List<Long> list) {
		...
		ChatGroup group = new ChatGroup()
			...
			.setName(StrUtil.format(ApiConstant.GROUP_CREATE_NAME, master.getNickName(),
		...
		for (ChatUser user : userList) {
			infoList.add(new ChatGroupInfo(user.getUserId(), group.getId()).setKeepGroup(
				YesOrNoEnum.YES));
		}
		infoList.add(
			new ChatGroupInfo(master.getUserId(), group.getId()).setKeepGroup(YesOrNoEnum.YES));
		...
		PushParamVo paramVo1 = new PushParamVo()
			...
			.setContent(StrUtil.format(ApiConstant.NOTICE_GROUP_CREATE_MEMBER,
				master.getNickName()));
		// 通知组员
		chatPushService.pushMsg(formatFrom(list, paramVo1), paramVo1, PushMsgTypeEnum.ALERT);
		// 通知群主
		List<String> nickList = userList.stream().map(ChatUser::getNickName)
			.collect(Collectors.toList());
		String content = StrUtil.format(ApiConstant.NOTICE_GROUP_CREATE_MASTER,
			CollUtil.join(nickList, "、"));
	...
	public void invitationGroup(Long groupId, List<Long> list) {
		...
		for (ChatUser chatUser : userList) {
			...
			// 更新
				...
				groupInfoService.updateById(new ChatGroupInfo().setInfoId(groupInfo.getInfoId())
		...
		List<String> nickList = newUserList.stream().map(ChatUser::getNickName)
			.collect(Collectors.toList());
		String content = StrUtil.format(ApiConstant.NOTICE_GROUP_JOIN,
			CollUtil.join(nickList, "、"));
	...
	protected void updMaster(ChatGroup group) {
		...
		groupInfoService.updateById(
			new ChatGroupInfo().setInfoId(userId).setKeepGroup(YesOrNoEnum.YES));
		// 更新申请人
		ChatApply query = new ChatApply().setTargetId(groupId).setApplyType(ApplyTypeEnum.GROUP)
	...
	public void kickedGroup(Long groupId, List<Long> list) {
		...
		PushParamVo paramVo1 = new PushParamVo()
			...
			.setDisturb(groupInfo.getDisturb())
			.setTop(groupInfo.getTop())
			.setContent(StrUtil.format(ApiConstant.NOTICE_GROUP_KICKED_MASTER,
				CollUtil.join(nickList, "、")));
		chatPushService.pushMsg(paramVo1.setToId(group.getMaster()), PushMsgTypeEnum.ALERT);
		// 组员
		List<PushParamVo> paramList = queryGroupPushFrom(groupId, list,
			ApiConstant.NOTICE_GROUP_KICKED_MEMBER);
	...
	public String getGroupQrCode(Long groupId) {
		...
		byte[] data = QrCodeUtil.generatePng(content, ApiConstant.QR_CODE_SIZE,
			ApiConstant.QR_CODE_SIZE);
	...
	public void logoutGroup(Long groupId) {
		...
		if (group.getMaster().equals(userId)) {
			...
			PushParamVo paramVo = new PushParamVo()
				...
				.setTop(groupInfo.getTop())
				.setContent(
					StrUtil.format(ApiConstant.NOTICE_GROUP_LOGOUT, chatUser.getNickName()))
	...
	public List<GroupVo08> groupList() {
		...
		// 查询明细
		List<Long> groupList = groupInfoService.queryList(
				new ChatGroupInfo().setUserId(ShiroUtils.getUserId()).setKicked(YesOrNoEnum.NO))
	...
	public void editGroupName(GroupVo02 groupVo) {
		...
		PushParamVo paramVo = new PushParamVo()
			...
			.setNickName(groupName)
			.setPortrait(group.getPortrait())
			.setContent(StrUtil.format(ApiConstant.NOTICE_GROUP_EDIT, chatUser.getNickName(),
				group.getName()));
		// 通知组员
		List<Long> userList = groupInfoService.queryUserList(groupId);
	...
	public void editGroupNotice(GroupVo03 groupVo) {
		...
		PushParamVo paramVo = new PushParamVo()
			...
			.setContent(StrUtil.format(ApiConstant.NOTICE_GROUP_NOTICE, chatUser.getNickName(),
				group.getNotice()));
		// 通知组员
		List<Long> userList = groupInfoService.queryUserList(groupId);
		chatPushService.pushMsg(formatFrom(userList, paramVo), paramVo, PushMsgTypeEnum.ALERT);
	...
	private String initPortrait(List<ChatUser> userList, ChatUser master) {
		...
		return JSONUtil.toJsonStr(
			CollUtil.sub(newUserList, 0, GROUP_COUNT).stream().map(ChatUser::getPortrait)
				.collect(Collectors.toList()));
	}
