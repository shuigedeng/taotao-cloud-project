...
public class NearServiceImpl implements NearService {
	...
	private void sendNear(NearVo01 nearVo) {
		...
		String userId = NumberUtil.toStr(ShiroUtils.getUserId());
		// 保存坐标
		geoHashUtils.add(ApiConstant.REDIS_NEAR, nearVo.getLongitude(), nearVo.getLatitude(),
				userId);
	}
	...
	private List<NearVo02> getNear() {
		...
		String userId = NumberUtil.toStr(ShiroUtils.getUserId());
		// 100公里内的9999个用户
		List<GeoResult<GeoVo>> geoResults = geoHashUtils.radius(ApiConstant.REDIS_NEAR, userId, 100,
				9999);
		// 过滤
