...
public class Generator {
	...
	public static void main(String[] args) {
		...
		fastAutoGenerator.packageConfig(packageConfigBuilder -> packageConfigBuilder
				...
				.serviceImpl(
					MODULE_NAME.isEmpty() ? "service.impl" : "service." + MODULE_NAME + ".impl")
...
//                .other(MODULE_NAME.isEmpty() ? "pojo.other" : "pojo." + MODULE_NAME + "other")
				.pathInfo(Collections.singletonMap(OutputFile.xml,
					MODULE_NAME.isEmpty() ? "wonder-admin\\src\\main\\resources\\mapper\\business\\"
						: "wonder-admin\\src\\main\\resources\\mapper\\business\\" + MODULE_NAME
							+ "\\"))
		);
		...
		fastAutoGenerator.injectionConfig(injectionConfig -> {
			...
			injectionConfig.beforeOutputFile((tableInfo, biConsumer) -> {
				customFileMap.put(tableInfo.getEntityName() + "Dto.java",
					"/templates/dto.java.ftl");
				customFileMap.put(tableInfo.getEntityName() + "Query.java",
		...
		fastAutoGenerator.strategyConfig(
			strategyConfigBuilder -> strategyConfigBuilder.enableCapitalMode()
				.enableSkipView()
				.disableSqlFilter()
