...
import com.taotao.cloud.generator.generator.entity.FieldInfo;
import com.taotao.cloud.generator.generator.entity.NonCaseString;
...
public class TableParseUtil {
    ...
    public static ClassInfo processTableIntoClassInfo(ParamInfo paramInfo) throws IOException {
        ...
        if (tableName != null
                && StringUtils.isNotNull(
                        MapUtil.getString(paramInfo.getOptions(), "ignorePrefix"))) {
            tableName =
        ...
        if (tableSql.containsAny("comment=", "comment on table")) {
            ...
            String classCommentTmp =
                    (ix > -1)
                            ? tableSql.substring(ix + 8).trim().get()
                            : tableSql.substring(tableSql.lastIndexOf("comment on table") + 17)
                                    .trim()
                                    .get();
        ...
        // 正常( ) 内的一定是字段相关的定义。
        String fieldListTmp =
                tableSql.substring(tableSql.indexOf("(") + 1, tableSql.lastIndexOf(")")).get();
        ...
        if (fieldLineList.length > 0) {
            ...
            for (String columnLine0 : fieldLineList) {
                ...
                if (notSpecialFlag) {
                    ...
                    } else if (ParamInfo.NAME_CASE_TYPE.UPPER_UNDER_SCORE_CASE.equals(
                            nameCaseType)) {
                    ...
                    if (tableSql.contains("comment on column")
                        ...
                        tableSql =
                                tableSql.replaceAll(
                                        ".`" + columnName + "` is", "." + columnName + " is");
                        Matcher columnCommentMatcher =
                                Pattern.compile("\\." + columnName + " is `").matcher(tableSql);
                        ...
                        while (columnCommentMatcher.find()) {
                            ...
                            fieldComment =
                                    tableSql.substring(
                                                    tableSql.indexOf(columnCommentTmp)
                                                            + columnCommentTmp.length())
                                            .trim()
                                            .get();
                            fieldComment =
                                    fieldComment.substring(0, fieldComment.indexOf("`")).trim();
                        }
                    } else if (columnLine.contains(" comment")) {
                        // 20200518 zhengkai 修复包含comment关键字的问题
                        String commentTmp =
                                columnLine
                                        .substring(columnLine.lastIndexOf("comment") + 7)
                                        .trim()
                                        .get();
                        // '用户ID',
                        if (commentTmp.contains("`")
                                || commentTmp.indexOf("`") != commentTmp.lastIndexOf("`")) {
                            commentTmp =
                                    commentTmp.substring(
                                            commentTmp.indexOf("`") + 1,
                                            commentTmp.lastIndexOf("`"));
                        }
                        // 解决最后一句是评论，无主键且连着)的问题:album_id int(3) default '1' null comment '相册id：0
                        // 代表头像 1代表照片墙')
                        if (commentTmp.contains(")")) {
    ...
    private static boolean isNotSpecialColumnLine(NonCaseString columnLine, int lineSeq) {
        return (!columnLine.containsAny(
                ...
                && !(columnLine.contains("primary ")
                        && columnLine.indexOf("storage") + 3 > columnLine.indexOf("("))
    ...
    public static ClassInfo processTableToClassInfoByRegex(ParamInfo paramInfo) {
        ...
        String DDL_PATTEN_STR =
                "\\s*create\\s+table\\s+(?<tableName>\\S+)[^\\(]*\\((?<columnsSQL>[\\s\\S]+)\\)[^\\)]+?(comment\\s*(=|on\\s+table)\\s*'(?<tableComment>.*?)'\\s*;?)?$";

        Pattern DDL_PATTERN = Pattern.compile(DDL_PATTEN_STR, Pattern.CASE_INSENSITIVE);

        // 匹配列sql部分，分别解析每一列的列名 类型 和列注释
        String COL_PATTERN_STR =
                "\\s*(?<fieldName>\\S+)\\s+(?<fieldType>\\w+)\\s*(?:\\([\\s\\d,]+\\))?((?!comment).)*(comment\\s*'(?<fieldComment>.*?)')?\\s*(,|$)";
                ...
                .forEach(
                        jsonField -> {
                            ...
                            if (jsonObject.get(jsonField) instanceof JSONArray) {
                                jsonObject.getJSONArray(jsonField).stream()
                                        .forEach(
                                                arrayObject -> {
                                                    FieldInfo fieldInfo2 = new FieldInfo();
                                                    fieldInfo2.setFieldName(arrayObject.toString());
                                                    fieldInfo2.setColumnName(
                                                            arrayObject.toString());
                                                    fieldInfo2.setFieldClass(
                                                            String.class.getSimpleName());
                                                    fieldInfo2.setFieldComment(
                                                            "children:" + arrayObject.toString());
                                                    fieldList.add(fieldInfo2);
                                                });
                            } else if (jsonObject.get(jsonField) instanceof JSONObject) {
                                jsonObject.getJSONObject(jsonField).keySet().stream()
                                        .forEach(
                                                arrayObject -> {
                                                    FieldInfo fieldInfo2 = new FieldInfo();
                                                    fieldInfo2.setFieldName(arrayObject.toString());
    ...
    public static ClassInfo processInsertSqlToClassInfo(ParamInfo paramInfo) {
        ...
        String valueStr =
                fieldSqlStr
                        .substring(fieldSqlStr.lastIndexOf("values") + 6)
                        .replaceAll(" ", "")
                        .replaceAll("\\(", "")
                        .replaceAll("\\)", "");
        ...
        // String
        // DDL_PATTEN_STR="\\s*create\\s+table\\s+(?<tableName>\\S+)[^\\(]*\\((?<columnsSQL>[\\s\\S]+)\\)[^\\)]+?(comment\\s*(=|on\\s+table)\\s*'(?<tableComment>.*?)'\\s*;?)?$";
        ...
        while (matcher1.find()) {
            ...
            Arrays.stream(columnsSQL.replaceAll(" ", "").split(","))
                    .forEach(
                            column -> {
                                FieldInfo fieldInfo2 = new FieldInfo();
