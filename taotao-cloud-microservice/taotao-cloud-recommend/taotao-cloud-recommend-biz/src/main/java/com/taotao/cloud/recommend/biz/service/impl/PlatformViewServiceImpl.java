package com.taotao.cloud.recommend.biz.service.impl;
...
import com.taotao.cloud.recommend.biz.service.PlatformViewService;
import com.taotao.cloud.recommend.biz.util.StatisticsDateUtil;
import com.taotao.cloud.recommend.biz.util.StatisticsSuffix;
 ...
 */
@Service
public class PlatformViewServiceImpl extends
		ServiceImpl<PlatformViewMapper, PlatformViewData> implements
		PlatformViewService {
	...
	public Long online() {
		...
		Long num = Long.valueOf(
				cache.keys(CachePrefix.ACCESS_TOKEN.getPrefix(UserEnums.MEMBER) + "*").size());
			...
			switch (searchTypeEnum) {
				case TODAY:
					...
					if (StringUtils.isEmpty(queryParam.getStoreId())) {
						...
						today.setUvNum(cache.counter(
										CachePrefix.UV.getPrefix() + StatisticsSuffix.suffix())
								.longValue());
					}
					//店铺流量
					else {
						...
						String pv = cache.getString(
								CachePrefix.STORE_PV.getPrefix() + StatisticsSuffix.suffix(
						...
						today.setPvNum(Long.valueOf(pv));
						today.setUvNum(cache.counter(
								CachePrefix.STORE_UV.getPrefix() + StatisticsSuffix.suffix(
										queryParam.getStoreId())).longValue());
					}
		...
		if (dates[0].equals(calendar.getTime())) {
			if (StringUtils.isNotEmpty(queryParam.getStoreId())) {
				return cache.counter(CachePrefix.UV.getPrefix() + StatisticsSuffix.suffix(
						queryParam.getStoreId())).intValue();
			}
	 ...
	 * @return
	 */
	private List<PlatformViewVO> builderVOS(Date startDate, Date endDate,
			List<PlatformViewData> dataList) {
