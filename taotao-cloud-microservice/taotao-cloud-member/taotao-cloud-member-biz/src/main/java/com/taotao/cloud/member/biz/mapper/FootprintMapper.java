...
public interface FootprintMapper extends BaseSuperMapper<MemberBrowse> {
	 ...
	 */
	@Delete("""
		DELETE FROM tt_foot_print WHERE
		       (SELECT COUNT(b.id) FROM
		            ( SELECT * FROM tt_foot_print WHERE member_id = #{memberId} ) b) >100
		            AND id =(SELECT a.id FROM ( SELECT * FROM tt_foot_print WHERE member_id = #{memberId} ORDER BY create_time ASC LIMIT 1 ) AS a)
		""")
