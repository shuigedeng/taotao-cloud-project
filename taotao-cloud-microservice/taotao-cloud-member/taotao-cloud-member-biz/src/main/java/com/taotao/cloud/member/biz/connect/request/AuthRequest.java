...
public interface AuthRequest {
	 ...
	 * 返回授权url，可自行跳转页面
	 * <p>
	 * 不建议使用该方式获取授权地址，不带{@code state}的授权地址，容易受到csrf攻击。 建议使用{@link BaseAuthRequest#authorize(String)}方法生成授权地址，在回调方法中对{@code
	 * state}进行校验
	 *
	 * @return 返回授权地址
	 */
	...
	/**
	 * 返回带{@code state} 第三方openid 获取方式，这里主要为微信公众号支付获取openid服务，以及根据openid自动登录 授权回调时会带上这个{@code state}
	 * 这里用做uuid
	 *
