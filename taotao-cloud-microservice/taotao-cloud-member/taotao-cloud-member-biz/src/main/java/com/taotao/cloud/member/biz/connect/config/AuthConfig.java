...
public class AuthConfig {
	...
	/**
	 * 是否需要申请unionid，目前只针对qq登录 注：qq授权登录时，获取unionid需要单独发送邮件申请权限。如果个人开发者账号中申请了该权限，可以将该值置为true，在获取openId时就会同步获取unionId
	 ...
	 * BaseAuthRequest#login(AuthCallback)}} 将不会校验 {@code state} 的合法性。
	 * <p>
	 * 使用场景：当且仅当使用自实现 {@code state} 校验逻辑时开启
	 * <p>
	 * 以下场景使用方案仅作参考： 1. 授权、登录为同端，并且全部使用 JustAuth 实现时，该值建议设为 {@code false}; 2. 授权和登录为不同端实现时，比如前端页面拼装
	 * {@code authorizeUrl}，并且前端自行对{@code state}进行校验， 后端只负责使用{@code code}获取用户信息时，该值建议设为 {@code
	 * true};
	 *
