...
public class SeckillApplyServiceImpl extends ServiceImpl<SeckillApplyMapper, SeckillApply>
    ...
    public List<SeckillGoodsVO> getSeckillGoods(Integer timeline) {
        ...
        // 秒杀活动缓存key
        String seckillCacheKey =
                PromotionCacheKeys.getSeckillTimelineKey(
        ...
        if (cacheSeckill == null || cacheSeckill.isEmpty()) {
            // 如缓存中不存在，则单独获取
            List<SeckillTimelineVO> seckillTimelineToCache =
                    getSeckillTimelineToCache(seckillCacheKey);
            Optional<SeckillTimelineVO> first =
                    seckillTimelineToCache.stream()
            ...
            // 如缓存中存在，则取缓存中转为展示的信息
            Optional<SeckillTimelineVO> first =
                    cacheSeckill.stream().filter(i -> i.getTimeLine().equals(timeline)).findFirst();
            if (first.isPresent()) {
                seckillGoodsVoS = first.get().getSeckillGoodsList();
            }
        }
    ...
    public IPage<SeckillApply> getSeckillApply(SeckillPageQuery queryParam, PageVO pageVo) {
        IPage<SeckillApply> seckillApplyPage =
                this.page(PageUtil.initPage(pageVo), queryParam.queryWrapper());
        if (seckillApplyPage != null && !seckillApplyPage.getRecords().isEmpty()) {

            // 获取skuId
            List<String> skuIds =
                    seckillApplyPage.getRecords().stream()
                            .map(SeckillApply::getSkuId)
                            .collect(Collectors.toList());

            // 循环获取 店铺/全平台 参与的促销商品库存进行填充
            if (!skuIds.isEmpty()) {
                List<Integer> skuStock =
                        promotionGoodsService.getPromotionGoodsStock(
    ...
    @Override
    public void addSeckillApply(
            String seckillId, String storeId, List<SeckillApplyVO> seckillApplyList) {
        ...
        // 获取已参与活动的秒杀活动活动申请列表
        List<String> skuIds =
                seckillApplyList.stream().map(SeckillApply::getSkuId).collect(Collectors.toList());
        ...
        for (SeckillApplyVO seckillApply : seckillApplyList) {
            ...
            // 获取秒杀活动时间段
            DateTime startTime =
                    DateUtil.offsetHour(seckill.getStartTime(), seckillApply.getTimeLine());
            // 检测是否可以发布促销商品
        ...
        this.remove(
                new LambdaQueryWrapper<SeckillApply>()
        ...
        if (!promotionGoodsList.isEmpty()) {
            ...
            PromotionTools.promotionGoodsInit(
                    promotionGoodsList, seckill, PromotionTypeEnum.SECKILL);
    ...
    public void removeSeckillApply(String seckillId, String id) {
        ...
        this.promotionGoodsService.deletePromotionGoods(
                seckillId, Collections.singletonList(seckillApply.getSkuId()));
    }
    ...
    private List<SeckillTimelineVO> getSeckillTimelineToCache(String seckillCacheKey) {
        ...
        queryWrapper.between(
                BasePromotions::getStartTime, DateUtil.beginOfDay(now), DateUtil.endOfDay(now));
        ...
        for (Seckill seckill : seckillList) {
            ...
            for (int i = 0; i < hoursSored.length; i++) {
                ...
                if (hoursSoredHour) {
                    ...
                    // 秒杀活动的时刻
                    long timeLine =
                            DateUtil.getDateline(date + " " + hoursSored[i], "yyyy-MM-dd HH");
                    ...
                    tempTimeline.setSeckillGoodsList(
                            wrapperSeckillGoods(hoursSored[i], seckill.getId()));
    ...
    private List<SeckillGoodsVO> wrapperSeckillGoods(Integer startTimeline, String seckillId) {
        ...
        if (!seckillApplyList.isEmpty()) {
            List<SeckillApply> collect =
                    seckillApplyList.stream()
                            .filter(
                                    i ->
                                            i.getTimeLine().equals(startTimeline)
                                                    && i.getPromotionApplyStatus()
                                                            .equals(
                                                                    PromotionsApplyStatusEnum.PASS
                                                                            .name()))
                            .collect(Collectors.toList());
    ...
    private void checkSeckillGoodsSku(
        ...
        if (promotionGoodsService.findInnerOverlapPromotionGoods(
            ...
            throw new BusinessException(
                    "商品[" + goodsSku.getGoodsName() + "]已经在重叠的时间段参加了拼团活动，不能参加秒杀活动");
        }
        // 查询是否在同一时间段参与了秒杀活动活动
        if (promotionGoodsService.findInnerOverlapPromotionGoods(
                ...
                > 0) {
            throw new BusinessException(
                    "商品[" + goodsSku.getGoodsName() + "]已经在重叠的时间段参加了秒杀活动，不能参加秒杀活动活动");
        }
    }
     ...
     * @return 秒杀活动促销商品
     */
    private PromotionGoods setSeckillGoods(
            GoodsSku goodsSku, SeckillApply seckillApply, Seckill seckill) {
        // 设置促销商品默认内容
        ...
        DateTime startTime =
                DateUtil.offsetHour(
                        DateUtil.beginOfDay(seckill.getStartTime()), seckillApply.getTimeLine());
