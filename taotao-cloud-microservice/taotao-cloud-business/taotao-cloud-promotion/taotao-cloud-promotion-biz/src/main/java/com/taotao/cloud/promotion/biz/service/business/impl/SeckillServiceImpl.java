...
public class SeckillServiceImpl extends AbstractPromotionsServiceImpl<SeckillMapper, Seckill>
    ...
    public SeckillVO getSeckillDetail(String id) {
        ...
        seckillVO.setSeckillApplyList(
                this.seckillApplyService.list(
    ...
    public void init() {
        ...
        Result<SettingVO> settingResult =
                settingApi.get(SettingCategoryEnum.SECKILL_SETTING.name());
        SeckillSetting seckillSetting =
                new Gson().fromJson(settingResult.getSettingValue(), SeckillSetting.class);

        for (int i = 1; i <= PRE_CREATION; i++) {
            Seckill seckill =
                    new Seckill(i, seckillSetting.getHours(), seckillSetting.getSeckillRule());
    ...
    public void setSeckillApplyTime(Seckill seckill, SeckillApply seckillApply) {
        // 下一个时间，默认为当天结束时间
        int nextHour =
                PromotionTools.nextHour(seckill.getHours().split(","), seckillApply.getTimeLine());

        String format = DateUtil.format(seckill.getStartTime(), DatePattern.NORM_DATE_PATTERN);
        DateTime parseStartTime =
                DateUtil.parse((format + " " + seckillApply.getTimeLine()), "yyyy-MM-dd HH");
    ...
    public void checkStatus(Seckill promotions) {
        ...
        if (promotions.getStartTime() != null
                && CharSequenceUtil.isNotEmpty(promotions.getHours())) {
        ...
        if (promotions.getStartTime() != null && promotions.getEndTime() != null) {
            // 同一时间段内相同的活动
            QueryWrapper<Seckill> queryWrapper =
