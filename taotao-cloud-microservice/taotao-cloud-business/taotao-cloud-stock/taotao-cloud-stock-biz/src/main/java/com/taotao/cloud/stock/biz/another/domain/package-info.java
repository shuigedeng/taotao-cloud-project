 ...
 *
 * <p>领域对象 VS 业务对象 1.数据对象使用基本类型保持纯净：PlayerEntity 2.业务对象同样会体现业务 最大不同是领域对象采用充血模型聚合业务: PlayerCreateBO
 *
 * <p>领域层 VS 应用层 第一个区别：领域层关注纵向，应用层关注横向。领域层纵向做隔离，本领域业务行为要在本领域内处理完。应用层横向做编排，聚合和编排领域服务。
