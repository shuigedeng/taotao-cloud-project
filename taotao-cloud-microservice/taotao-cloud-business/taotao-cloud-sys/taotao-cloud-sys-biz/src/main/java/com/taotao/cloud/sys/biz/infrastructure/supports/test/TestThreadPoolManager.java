...
package com.taotao.cloud.sys.biz.infrastructure.supports.test;
...
import com.taotao.cloud.sys.biz.supports.test.BusinessThread;
...
public class TestThreadPoolManager implements BeanFactoryAware {
    ...
    final RejectedExecutionHandler handler = ( r, executor ) -> {
        // 订单加入到缓冲队列
        msgQueue.offer(( (com.taotao.cloud.sys.biz.supports.test.BusinessThread) r ).getAcceptStr());
        LogUtils.info(
                "系统任务太忙了,把此订单交给(调度线程池)逐一处理，订单号：" + ( (com.taotao.cloud.sys.biz.supports.test.BusinessThread) r ).getAcceptStr());
    };
    ...
    public void addOrders( String orderId ) {
        ...
        if (cacheMap.get(orderId) == null) {
            ...
            com.taotao.cloud.sys.biz.supports.test.BusinessThread businessThread = new com.taotao.cloud.sys.biz.supports.test.BusinessThread(orderId);
            ...
            () -> {
                ...
                if (!msgQueue.isEmpty()) {
                    ...
                    if (threadPool.getQueue().size() < WORK_QUEUE_SIZE) {
                        ...
                        com.taotao.cloud.sys.biz.supports.test.BusinessThread businessThread = new BusinessThread(orderId);
                        threadPool.execute(businessThread);
