...
import com.taotao.cloud.flowable.biz.bpm.convert.task.BpmTaskConvert;
import com.taotao.cloud.flowable.biz.bpm.dal.dataobject.task.BpmTaskExtDO;
import com.taotao.cloud.flowable.biz.bpm.dal.mysql.task.BpmTaskExtMapper;
...
public class BpmTaskServiceImpl implements BpmTaskService {
    ...
    @Override
    public PageResult<BpmTaskTodoPageItemRespVO> getTodoTaskPage(
            Long userId, BpmTaskTodoPageReqVO pageVO) {
        // 查询待办任务
        TaskQuery taskQuery =
                taskService
                        .createTaskQuery()
    ...
    @Override
    public PageResult<BpmTaskDonePageItemRespVO> getDoneTaskPage(
            Long userId, BpmTaskDonePageReqVO pageVO) {
        // 查询已办任务
        ...
        List<HistoricTaskInstance> tasks =
                taskQuery.listPage(PageUtils.getStart(pageVO), pageVO.getPageSize());
        if (CollUtil.isEmpty(tasks)) {
            return PageResult.empty(taskQuery.count());
        }
                ...
                new TransactionSynchronization() {
                    ...
                    public void afterCommit() {
                        ...
                        if (BpmProcessInstanceResultEnum.isEndResult(taskExt.getResult())) {
                            log.error(
                                    "[updateTaskExtCancel][taskId({}) 处于结果({})，无需进行更新]",
                                    taskId,
                                    taskExt.getResult());
                            return;
                        }
                ...
                new TransactionSynchronization() {
                    ...
                    public void afterCommit() {
                        ProcessInstance processInstance =
                                processInstanceService.getProcessInstance(
