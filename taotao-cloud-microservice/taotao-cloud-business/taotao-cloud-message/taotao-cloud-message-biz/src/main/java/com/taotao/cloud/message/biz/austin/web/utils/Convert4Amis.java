...
/**
 * 【该类的逻辑不用看，没有什么意义】 for Amis!!! amis框架在【表单】回显的时候，不支持嵌套动态语法!! 编写工具类将 List/Object 铺平成 Map 以及相关的格式
...
public class Convert4Amis {
	 ...
	 */
	private static final List<String> PARSE_JSON_OBJ_TO_ARRAY = Arrays.asList(
		"officialAccountParam", "miniProgramParam");

	/**
	 * 钉钉工作消息OA实际的映射
	 */
	private static final List<String> DING_DING_OA_FIELD = Arrays.asList("dingDingOaHead",
		"dingDingOaBody");
	/**
	 * 钉钉OA字段名实际的映射
	 */
				...
				for (String key : jsonObject.keySet()) {
					...
					if (DING_DING_OA_FIELD.contains(key)) {
						...
						for (String objKey : object.keySet()) {
							result.put(DING_DING_OA_NAME_MAPPING.get(objKey),
	 ...
	 * @return
	 */
	public static CommonAmisVo getWxMpTemplateParam(String wxTemplateId,
	...
	public static CommonAmisVo getTestContent(String msgContent) {
		...
		for (String param : placeholderList) {
			CommonAmisVo.ColumnsDTO dto = CommonAmisVo.ColumnsDTO.builder().name(param).label(param)
				.type("input-text").required(true).quickEdit(true).build();
			columnsDtoS.add(dto);
		}
	 ...
	 * @return
	 */
	public static CommonAmisVo getWxMaTemplateParam(String wxTemplateId,
			...
			if (wxTemplateId.equals(templateInfo.getPriTmplId())) {
				...
				for (String datum : data) {
					...
					CommonAmisVo.ColumnsDTO columnsDTO = CommonAmisVo.ColumnsDTO.builder()
						.name(name).type("input-text").required(true).quickEdit(true).label(label)
						.build();
	 ...
	 * 【这个方法不用看】，纯粹为了适配amis前端
	 * <p>
	 * 1、得到微信服务号的【带参数】二维码返回给前端 2、让前端轮询请求 接口看是否已登录
	 *
	 * @return
	 */
	public static CommonAmisVo getWxMpQrCode(String url, String id) {
		CommonAmisVo image = CommonAmisVo.builder().type("static-image").value(url).originalSrc(url)
			.name("image").label("扫描关注").fixedSize(true).fixedSizeClassName(url)
			.fixedSizeClassName("h-32").build();
		...
		CommonAmisVo service = CommonAmisVo.builder().type("service")
			.api(CommonAmisVo.ApiDTO.builder()
				...
				.adaptor(adaptor).requestAdaptor(requestAdaptor).build()).interval(2000)
			.silentPolling(true).build();

		return CommonAmisVo.builder().type("form").title("登录").mode("horizontal")
			.body(Arrays.asList(image, service)).build();
	}
	...
	/**
	 * 【这个方法不用看】，纯粹为了适配amis前端 获取 EchartsVo
	 *
	 * @param anchorResult
	 * @param businessId
	 * @return
	 */
	public static EchartsVo getEchartsVo(Map<Object, Object> anchorResult, String templateName,
		String businessId) {
		...
		if (CollUtil.isNotEmpty(anchorResult)) {
			...
			for (Map.Entry<Object, Object> entry : anchorResult.entrySet()) {
				String description = AnchorState.getDescriptionByCode(
		...
		String title = "【" + templateName + "】在" + DateUtil.format(DateUtil.parse(
				String.valueOf(TaskInfoUtils.getDateFromBusinessId(Long.valueOf(businessId)))),
			DatePattern.CHINESE_DATE_FORMATTER) + "的下发情况：";
		...
		return EchartsVo.builder()
			...
			.series(Arrays.asList(
				EchartsVo.SeriesVO.builder().name("人数").type("bar").data(actualData).build()))
	...
	/**
	 * 【这个方法不用看】，纯粹为了适配amis前端 获取 SmsTimeLineVo
	 *
	 * @return
	 */
		...
		for (Map.Entry<String, List<SmsRecord>> entry : maps.entrySet()) {
			...
			for (SmsRecord smsRecord : entry.getValue()) {
				...
				if (smsRecord.getMessageTemplateId() > 0) {
					...
					itemsVO.setSendTime(
						DateUtil.format(new Date(Long.valueOf(smsRecord.getCreated() * 1000L)),
					...
					itemsVO.setReceiveTime(
						DateUtil.format(new Date(Long.valueOf(smsRecord.getUpdated() * 1000L)),
							DatePattern.NORM_DATETIME_PATTERN));
				}
			}
