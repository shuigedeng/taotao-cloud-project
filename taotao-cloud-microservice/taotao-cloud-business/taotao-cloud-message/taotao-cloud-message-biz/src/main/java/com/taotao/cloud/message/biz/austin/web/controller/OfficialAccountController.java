...
public class OfficialAccountController {
	...
	public List<CommonAmisVo> queryList(Integer id) {
		try {
			...
			for (WxMpTemplate wxMpTemplate : allPrivateTemplate) {
				CommonAmisVo commonAmisVo = CommonAmisVo.builder().label(wxMpTemplate.getTitle())
					.value(wxMpTemplate.getTemplateId()).build();
				result.add(commonAmisVo);
			}
			return result;
		} catch (Exception e) {
			log.error("OfficialAccountController#queryList fail:{}",
	...
	public CommonAmisVo queryDetailList(Integer id, String wxTemplateId) {
		...
		try {
			...
			log.error("OfficialAccountController#queryDetailList fail:{}",
				Throwables.getStackTraceAsString(e));
			throw new CommonException(RespStatusEnum.SERVICE_ERROR);
		}
	}
	...
	public String receiptMessage(HttpServletRequest request) {
		try {
			...
			if (OfficialAccountParamConstant.RAW.equals(encryptType)) {
				...
				WxMpXmlOutMessage outMessage = configService.getWxMpMessageRouter()
					.route(inMessage);
				return outMessage.toXml();
			} else if (OfficialAccountParamConstant.AES.equals(encryptType)) {
				String msgSignature = request.getParameter(
					OfficialAccountParamConstant.MSG_SIGNATURE);
				WxMpXmlMessage inMessage = WxMpXmlMessage.fromEncryptedXml(request.getInputStream(),
					configService.getConfig(), timestamp, nonce, msgSignature);
			...
			log.error("OfficialAccountController#receiptMessage fail:{}",
				Throwables.getStackTraceAsString(e));
	...
	public WxMpUser checkLogin(String sceneId) {
		try {
			...
			log.error("OfficialAccountController#checkLogin fail:{}",
				Throwables.getStackTraceAsString(e));
			return null;
		}
	}
	...
	public void deleteTestUser(HttpServletRequest request) {
		try {
			...
			List<String> openIds = loginUtils.getLoginConfig().getOfficialAccountLoginService()
				.getUserService().userList(null).getOpenids();
			...
			log.error("OfficialAccountController#deleteTestUser fail:{}",
				Throwables.getStackTraceAsString(e));
		}
	}
}
