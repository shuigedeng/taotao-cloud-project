package com.taotao.cloud.message.biz.infrastructure.austin.handler.handler.impl;
...
import com.taotao.cloud.message.biz.infrastructure.austin.common.constant.AustinConstant;
import com.taotao.cloud.message.biz.infrastructure.austin.common.domain.RecallTaskInfo;
import com.taotao.cloud.message.biz.infrastructure.austin.common.domain.TaskInfo;
import com.taotao.cloud.message.biz.infrastructure.austin.common.dto.account.DingDingRobotAccount;
import com.taotao.cloud.message.biz.infrastructure.austin.common.dto.model.DingDingRobotContentModel;
import com.taotao.cloud.message.biz.infrastructure.austin.common.enums.ChannelType;
import com.taotao.cloud.message.biz.infrastructure.austin.common.enums.SendMessageType;
import com.taotao.cloud.message.biz.infrastructure.austin.handler.domain.dingding.DingDingRobotParam;
import com.taotao.cloud.message.biz.infrastructure.austin.handler.domain.dingding.DingDingRobotResult;
import com.taotao.cloud.message.biz.infrastructure.austin.handler.handler.BaseHandler;
import com.taotao.cloud.message.biz.infrastructure.austin.support.utils.AccountUtils;
...
public class DingDingRobotHandler extends BaseHandler {
    ...
    private DingDingRobotParam assembleParam( TaskInfo taskInfo) {
        ...
        if (SendMessageType.LINK.getCode().equals(contentModel.getSendType())) {
            param.setLink(
                    DingDingRobotParam.LinkVO.builder().title(contentModel.getTitle()).text(contentModel.getContent()).messageUrl(contentModel.getUrl()).picUrl(contentModel.getPicUrl()).build());
        }
    ...
    private String assembleSign(long currentTimeMillis, String secret) {
        ...
        try {
            ...
            Mac mac = Mac.getInstance(CommonConstants.HMAC_SHA256_ENCRYPTION_ALGO);
            mac.init(new SecretKeySpec(secret.getBytes(StandardCharsets.UTF_8), CommonConstants.HMAC_SHA256_ENCRYPTION_ALGO));
            byte[] signData = mac.doFinal(stringToSign.getBytes(StandardCharsets.UTF_8));
            sign = URLEncoder.encode(new String(Base64.encodeBase64(signData), StandardCharsets.UTF_8), CommonConstants.CHARSET_UTF_8);
        } catch (Exception e) {
