...
public class DingDingWorkNoticeHandler extends BaseHandler implements Handler {
	...
	public boolean handler(TaskInfo taskInfo) {
		try {
			...
			OapiMessageCorpconversationAsyncsendV2Request request = assembleParam(account,
				taskInfo);
			...
			OapiMessageCorpconversationAsyncsendV2Response response = new DefaultDingTalkClient(
				SEND_URL).execute(request, accessToken);

			// 发送成功后记录TaskId，用于消息撤回(支持当天的)
			if (response.getErrcode() == 0) {
				redisTemplate.opsForList()
					.leftPush(DING_DING_RECALL_KEY_PREFIX + taskInfo.getMessageTemplateId(),
						String.valueOf(response.getTaskId()));
				redisTemplate.expire(DING_DING_RECALL_KEY_PREFIX + taskInfo.getMessageTemplateId(),
			...
			log.error("DingDingWorkNoticeHandler#handler fail!result:{},params:{}",
				JSON.toJSONString(response), JSON.toJSONString(taskInfo));
		} catch (Exception e) {
			log.error("DingDingWorkNoticeHandler#handler fail!{},params:{}",
	...
	private OapiMessageCorpconversationAsyncsendV2Request assembleParam(
		...
		try {
			...
			message.setMsgtype(
				SendMessageType.getDingDingWorkTypeByCode(contentModel.getSendType()));
			...
			if (SendMessageType.ACTION_CARD.getCode().equals(contentModel.getSendType())) {
				...
				actionCard.setBtnJsonList(JSON.parseArray(contentModel.getBtns(),
					OapiMessageCorpconversationAsyncsendV2Request.BtnJsonList.class));
			...
			if (SendMessageType.OA.getCode().equals(contentModel.getSendType())) {
				...
				oa.setHead(JSON.parseObject(contentModel.getDingDingOaHead(),
					OapiMessageCorpconversationAsyncsendV2Request.Head.class));
				oa.setBody(JSON.parseObject(contentModel.getDingDingOaBody(),
					OapiMessageCorpconversationAsyncsendV2Request.Body.class));
				message.setOa(oa);
			}
			req.setMsg(message);
		} catch (Exception e) {
			log.error("assembleParam fail:{},params:{}", Throwables.getStackTraceAsString(e),
	...
	public void recall(MessageTemplate messageTemplate) {
		SupportThreadPoolConfig.getPendingSingleThreadPool().execute(() -> {
			try {
				DingDingWorkNoticeAccount account = accountUtils.getAccountById(
					messageTemplate.getSendAccount(), DingDingWorkNoticeAccount.class);
				String accessToken = redisTemplate.opsForValue().get(
					SendAccountConstant.DING_DING_ACCESS_TOKEN_PREFIX
						+ messageTemplate.getSendAccount());
				while (redisTemplate.opsForList()
					.size(DING_DING_RECALL_KEY_PREFIX + messageTemplate.getId()) > 0) {
					String taskId = redisTemplate.opsForList()
						.leftPop(DING_DING_RECALL_KEY_PREFIX + messageTemplate.getId());
					...
					OapiMessageCorpconversationRecallResponse rsp = client.execute(req,
						accessToken);
					logUtils.print(
						LogParam.builder().bizType(RECALL_BIZ_TYPE).object(JSON.toJSONString(rsp))
							.build());
				}
			} catch (Exception e) {
				log.error("DingDingWorkNoticeHandler#recall fail:{}",
	...
	public void pull(Long accountId) {
		try {
			DingDingWorkNoticeAccount account = accountUtils.getAccountById(accountId.intValue(),
				DingDingWorkNoticeAccount.class);
			String accessToken = redisTemplate.opsForValue()
				.get(SendAccountConstant.DING_DING_ACCESS_TOKEN_PREFIX + accountId);
			...
			log.error("DingDingWorkNoticeHandler#pull fail:{}",
				Throwables.getStackTraceAsString(e));
		}
	}
}
