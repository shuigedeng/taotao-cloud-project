...
public class WeChatLoginConfig {
	...
	@PostConstruct
	private void init() {
		WeChatOfficialAccount account = WeChatOfficialAccount.builder().appId(appId).secret(secret)
				.token(token).build();
	...
	private void initRouter() {
		...
		wxMpMessageRouter.rule().async(false).msgType(WxConsts.XmlMsgType.EVENT)
				...
				.handler(wxMpMessageHandlers.get(OfficialAccountParamConstant.SUBSCRIBE_HANDLER))
				.end();
		wxMpMessageRouter.rule().async(false).msgType(WxConsts.XmlMsgType.EVENT)
		...
		wxMpMessageRouter.rule().async(false).msgType(WxConsts.XmlMsgType.EVENT)
				...
				.handler(wxMpMessageHandlers.get(OfficialAccountParamConstant.UNSUBSCRIBE_HANDLER))
				.end();
	}
