...
public class AssembleAction implements BusinessProcess<SendTaskModel> {
	 ...
	 */
	private List<TaskInfo> assembleTaskInfo(SendTaskModel sendTaskModel,
		...
		for (MessageParam messageParam : messageParamList) {
			TaskInfo taskInfo = TaskInfo.builder()
				...
				.receiver(new HashSet<>(Arrays.asList(
					messageParam.getReceiver().split(String.valueOf(StrUtil.C_COMMA)))))
	...
	private static ContentModel getContentModelValue(MessageTemplate messageTemplate,
		...
		Class<? extends ContentModel> contentModelClass = ChannelType.getChanelModelClassByCode(
			sendChannel);
		...
		for (Field field : fields) {
			...
			if (StrUtil.isNotBlank(originValue)) {
				...
				Object resultObj =
					JSONUtil.isJsonObj(resultValue) ? JSONUtil.toBean(resultValue, field.getType())
						: resultValue;
