...
public class SecurityMetadataSourceStorage {
	...
	public SecurityMetadataSourceStorage() {
		this.compatible = JetCacheUtils.create(
			OAuth2Constants.CACHE_NAME_SECURITY_METADATA_COMPATIBLE, CacheType.LOCAL);
		this.indexable = JetCacheUtils.create(
	...
	private LinkedHashMap<HerodotusRequestMatcher, HerodotusConfigAttribute> readFromCompatible() {
		LinkedHashMap<HerodotusRequestMatcher, HerodotusConfigAttribute> compatible = this.compatible.get(
			KEY_COMPATIBLE);
	 ...
	 * 写入 indexable 缓存
	 *
	 * @param herodotusRequestMatcher  自定义扩展的 AntPathRequestMatchers
	 *                                 {@link HerodotusRequestMatcher}
	 * @param herodotusConfigAttribute 权限配置属性
	 */
	private void writeToIndexable(HerodotusRequestMatcher herodotusRequestMatcher,
	 ...
	 */
	private void appendToCompatible(HerodotusRequestMatcher herodotusRequestMatcher,
...
//        });
		...
		log.trace("[Herodotus] |- Append [{}] to Compatible cache, current size is [{}]",
			herodotusRequestMatcher, compatible.size());
	 ...
	 */
	private void appendToIndexable(HerodotusRequestMatcher herodotusRequestMatcher,
		HerodotusConfigAttribute herodotusConfigAttribute) {
		...
		for (HerodotusRequestMatcher matcher : matchers.keySet()) {
			for (HerodotusRequestMatcher item : securityMetadata.keySet()) {
				if (matcher.matches(item)) {
					result.remove(item);
					log.trace("[Herodotus] |- Pattern [{}] is conflict with [{}], so remove it.",
						item.getPattern(), matcher.getPattern());
				}
			}
		}
