...
public class HttpCryptoProcessor extends AbstractRenderer {
	...
	public String encrypt(String identity, String content)  {
		try {
			...
			log.info("[Herodotus] |- Encrypt content from [{}] to [{}].", content, result);
			return result;
		} catch (StampHasExpiredException e) {
			log.info("[Herodotus] |- Session has expired, need recreate.");
			throw new SessionInvalidException();
		} catch (Exception e) {
			log.info("[Herodotus] |- Symmetric can not Encrypt content [{}], Skip!", content);
			return content;
		}
	}
	...
	public String decrypt(String identity, String content)  {
		try {
			...
			log.info("[Herodotus] |- Decrypt content from [{}] to [{}].", content, result);
			return result;
		} catch (StampHasExpiredException e) {
			log.info("[Herodotus] |- Session has expired, need recreate.");
			throw new SessionInvalidException();
		} catch (Exception e) {
			log.info("[Herodotus] |- Symmetric can not Decrypt content [{}], Skip!", content);
			return content;
		}
	}
	...
	public SecretKey nextStamp(String key) {
		...
		log.info("[Herodotus] |- Generate secret key, value is : [{}]", secretKey);
		return secretKey;
	}
	...
	private SecretKey getSecretKey(String identity)  {
		if (isSessionValid(identity)) {
			...
			if (ObjectUtils.isNotEmpty(secretKey)) {
				log.info("[Herodotus] |- Decrypt Or Encrypt content use param identity [{}], cached identity is [{}].", identity, secretKey.getIdentity());
				return secretKey;
			}
		}
	...
	private String decryptFrontendPublicKey(String content, String privateKey) {
		...
		log.info("[Herodotus] |- Decrypt frontend public key, value is : [{}]", frontendPublicKey);
	...
	private String encryptBackendKey(String symmetricKey, String publicKey) {
		...
		log.info("[Herodotus] |- Encrypt symmetric key use frontend public key, value is : [{}]", encryptedAesKey);
