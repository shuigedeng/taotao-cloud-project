 ...
 * @date : 2021/8/6 12:23
 */
public class RemoteSecurityMetadataSyncListener implements
	ApplicationListener<RemoteSecurityMetadataSyncEvent> {

	private static final Logger log = LoggerFactory.getLogger(
		RemoteSecurityMetadataSyncListener.class);
	...
	public void onApplicationEvent(RemoteSecurityMetadataSyncEvent event) {
		...
		if (!serviceMatcher.isFromSelf(event)) {
			...
			if (StringUtils.isNotBlank(data)) {
				List<SecurityAttribute> securityAttributes = JacksonUtils.toList(data,
					SecurityAttribute.class);

				if (CollectionUtils.isNotEmpty(securityAttributes)) {
					log.debug(
						"[Herodotus] |- Got security attributes from service [{}], current [{}] start to process security attributes.",
