...
public class QrCodeLoginService {
	...
	public String generateUUID() {
		try {
			...
			redisRepository.opsForValue().set(getScanUUID(uuid),
				CodeUtil.getUnusedCodeInfo(), 30 * 1000, TimeUnit.MINUTES);
			return uuid;
		} catch (Exception e) {
	...
	public CodeData scanQrLogin(String uuid, String account) {
		try {
			...
			if (codeStatus == CodeStatusEnum.UNUSED) {
				redisRepository.opsForValue().set(getScanUUID(uuid),
					CodeUtil.getConfirmingCodeInfo(), 30 * 10000, TimeUnit.MINUTES);
				//你的逻辑
