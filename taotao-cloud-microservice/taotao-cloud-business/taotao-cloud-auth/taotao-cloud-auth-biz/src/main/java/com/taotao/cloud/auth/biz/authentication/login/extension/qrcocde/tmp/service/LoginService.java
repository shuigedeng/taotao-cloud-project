...
import com.taotao.cloud.auth.biz.authentication.login.extension.qrcocde.tmp.entity.LoginTicket;
import com.taotao.cloud.auth.biz.authentication.login.extension.qrcocde.tmp.entity.QrCodeStatusEnum;
...
import com.taotao.cloud.auth.biz.authentication.login.extension.qrcocde.tmp.utils.LoginConstant;
...
//                cacheStore.put(CommonUtil.buildOnceTokenKey(onceToken), allowURI, LoginConstant.ONCE_TOKEN_EXPIRE_TIME, TimeUnit.SECONDS);
				cacheStore.setExpire(CommonUtil.buildOnceTokenKey(onceToken), uuid, LoginConstant.ONCE_TOKEN_EXPIRE_TIME, TimeUnit.SECONDS);
	...
	public JSONObject getQrCodeStatus(String uuid, int currentStatus) throws InterruptedException {
		...
		try {
			...
			if (statusEnum == QrCodeStatusEnum.CONFIRMED) {
				...
				cacheStore.setExpire(CommonUtil.buildAccessTokenKey(accessToken), loginTicket.getUserId(), LoginConstant.ACCESS_TOKEN_EXPIRE_TIME, TimeUnit.SECONDS);
