...
public class JustAuthConfigServiceImpl extends
	ServiceImpl<JustAuthConfigMapper, JustAuthConfig> implements IJustAuthConfigService {
	 ...
	 */
	@Override
	public Page<JustAuthConfigDTO> queryJustAuthConfigList(Page<JustAuthConfigDTO> page,
		QueryJustAuthConfigDTO queryJustAuthConfigDTO) {
		Page<JustAuthConfigDTO> justAuthConfigInfoList = justAuthConfigMapper.queryJustAuthConfigList(
	...
	public List<JustAuthConfigDTO> queryJustAuthConfigList(
		QueryJustAuthConfigDTO queryJustAuthConfigDTO) {
		List<JustAuthConfigDTO> justAuthConfigInfoList = justAuthConfigMapper.queryJustAuthConfigList(
	...
	public JustAuthConfigDTO queryJustAuthConfig(QueryJustAuthConfigDTO queryJustAuthConfigDTO) {
		JustAuthConfigDTO justAuthConfigDTO = justAuthConfigMapper.queryJustAuthConfig(
			queryJustAuthConfigDTO);
	...
	public boolean createJustAuthConfig(CreateJustAuthConfigDTO justAuthConfig) {
		JustAuthConfig justAuthConfigEntity = BeanCopierUtils.copyByClass(justAuthConfig,
			JustAuthConfig.class);
		boolean result = this.save(justAuthConfigEntity);
		if (result) {
			// 新增到缓存
			JustAuthConfig justAuthConfigCreate = this.getById(justAuthConfigEntity.getId());
			JustAuthConfigDTO justAuthConfigDTO = BeanCopierUtils.copyByClass(justAuthConfigCreate,
				JustAuthConfigDTO.class);
	...
	public boolean updateJustAuthConfig(UpdateJustAuthConfigDTO justAuthConfig) {
		...
		if (result) {
			...
			JustAuthConfigDTO justAuthConfigDTO = BeanCopierUtils.copyByClass(justAuthConfigUpdate,
				JustAuthConfigDTO.class);
			this.addOrUpdateJustAuthConfigCache(justAuthConfigDTO);
		}
		return result;
	...
	public boolean deleteJustAuthConfig(Long justAuthConfigId) {
		...
		// 从缓存删除
		if (result) {
			JustAuthConfigDTO justAuthConfigDTO = BeanCopierUtils.copyByClass(justAuthConfigDelete,
				JustAuthConfigDTO.class);
	...
	public boolean batchDeleteJustAuthConfig(List<Long> justAuthConfigIds) {
		...
		if (result && !CollectionUtils.isEmpty(justAuthConfigDeleteList)) {
			for (JustAuthConfig justAuthConfigDelete : justAuthConfigDeleteList) {
				JustAuthConfigDTO justAuthConfigDTO = BeanCopierUtils.copyByClass(
					justAuthConfigDelete, JustAuthConfigDTO.class);
				this.deleteJustAuthConfigCache(justAuthConfigDTO);
			}
		}
		return result;
	...
	public void initJustAuthConfigList() {
		...
		List<JustAuthConfigDTO> justAuthSourceInfoList = justAuthConfigMapper.initJustAuthConfigList(
			queryJustAuthConfigDTO);
