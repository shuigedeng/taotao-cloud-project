...
public class MutexLoginController {
    ...
    *
    * 测试步骤：
    	1、在浏览器A上登录账号10001，设备为PC    ---- http://127.0.0.1:8081/mutex/login?userId=10001&device=PC
    		检查是否登录成功，返回true：			---- http://127.0.0.1:8081/mutex/isLogin

    	2、在浏览器B上登录账号10001，设备为APP    ---- http://127.0.0.1:8081/mutex/login?userId=10001&device=APP
    		检查是否登录成功，返回true：			---- http://127.0.0.1:8081/mutex/isLogin

    	3、复查一下览器A上的账号是否登录，发现并没有顶替下去，原因是两个浏览器指定的登录设备不同，允许同时在线
    			---- http://127.0.0.1:8081/mutex/isLogin

    	4、在浏览器C上登录账号10001，设备为PC    ---- http://127.0.0.1:8081/mutex/login?userId=10001&device=PC
    		检查是否登录成功，返回true：			---- http://127.0.0.1:8081/mutex/isLogin
    	...
    	7、此时再从浏览器D上登录账号10001，设备为APP    ---- http://127.0.0.1:8081/mutex/login?userId=10001&device=APP
    		检查是否登录成功，返回true：			---- http://127.0.0.1:8081/mutex/isLogin
    ...
    */
    ...
    // 会话登录接口  ---- http://127.0.0.1:8081/mutex/doLogin?userId=10001&device=PC
    ...
    // 查询当前登录状态  ---- http://127.0.0.1:8081/mutex/isLogin
