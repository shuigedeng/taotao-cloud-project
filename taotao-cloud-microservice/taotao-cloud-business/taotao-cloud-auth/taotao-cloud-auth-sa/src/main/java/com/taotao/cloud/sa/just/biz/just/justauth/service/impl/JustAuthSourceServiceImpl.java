...
import com.taotao.cloud.sa.just.biz.just.justauth.service.IJustAuthSourceService;
...
public class JustAuthSourceServiceImpl extends
	ServiceImpl<JustAuthSourceMapper, JustAuthSource> implements IJustAuthSourceService {
	 ...
	 */
	@Override
	public Page<JustAuthSourceDTO> queryJustAuthSourceList(Page<JustAuthSourceDTO> page,
		QueryJustAuthSourceDTO queryJustAuthSourceDTO) {
		Page<JustAuthSourceDTO> justAuthSourceInfoList = justAuthSourceMapper.queryJustAuthSourceList(
			page, queryJustAuthSourceDTO);
	...
	public List<JustAuthSourceDTO> queryJustAuthSourceList(
		QueryJustAuthSourceDTO queryJustAuthSourceDTO) {
		List<JustAuthSourceDTO> justAuthSourceInfoList = justAuthSourceMapper.queryJustAuthSourceList(
			queryJustAuthSourceDTO);
		return justAuthSourceInfoList;
	}
	...
	public JustAuthSourceDTO queryJustAuthSource(QueryJustAuthSourceDTO queryJustAuthSourceDTO) {
		JustAuthSourceDTO justAuthSourceDTO = justAuthSourceMapper.queryJustAuthSource(
			queryJustAuthSourceDTO);
	...
	public boolean createJustAuthSource(CreateJustAuthSourceDTO justAuthSource) {
		JustAuthSource justAuthSourceEntity = BeanCopierUtils.copyByClass(justAuthSource,
			JustAuthSource.class);
		boolean result = this.save(justAuthSourceEntity);
		if (result) {
			// 新增到缓存
			JustAuthSource justAuthSourceCreate = this.getById(justAuthSourceEntity.getId());
			JustAuthSourceDTO justAuthSourceDTO = BeanCopierUtils.copyByClass(justAuthSourceCreate,
				JustAuthSourceDTO.class);
	...
	public boolean updateJustAuthSource(UpdateJustAuthSourceDTO justAuthSource) {
		...
		if (result) {
			...
			JustAuthSourceDTO justAuthSourceDTO = BeanCopierUtils.copyByClass(justAuthSourceUpdate,
				JustAuthSourceDTO.class);
			this.addOrUpdateJustAuthSourceCache(justAuthSourceDTO);
		}
		return result;
	...
	public boolean deleteJustAuthSource(Long justAuthSourceId) {
		...
		// 从缓存删除
		if (result) {
			JustAuthSourceDTO justAuthSourceDTO = BeanCopierUtils.copyByClass(justAuthSourceDelete,
				JustAuthSourceDTO.class);
	...
	public boolean batchDeleteJustAuthSource(List<Long> justAuthSourceIds) {
		...
		if (result && !CollectionUtils.isEmpty(justAuthSourceDeleteList)) {
			for (JustAuthSource justAuthSourceDelete : justAuthSourceDeleteList) {
				JustAuthSourceDTO justAuthSourceDTO = BeanCopierUtils.copyByClass(
					justAuthSourceDelete, JustAuthSourceDTO.class);
				this.deleteJustAuthSourceCache(justAuthSourceDTO);
			}
		}
	...
	public void initJustAuthSourceList() {
		...
		List<JustAuthSourceDTO> justAuthSourceInfoList = justAuthSourceMapper.initJustAuthSourceList(
			queryJustAuthSourceDTO);
	...
	private void addOrUpdateJustAuthSourceCache(JustAuthSourceDTO justAuthSource) {
		try {
			...
			redisTemplate.opsForHash().put(redisKey, justAuthSource.getSourceName(),
	...
	private void deleteJustAuthSourceCache(JustAuthSourceDTO justAuthSource) {
		try {
			...
			redisTemplate.opsForHash().delete(redisKey, justAuthSource.getSourceName(),
				JsonUtils.objToJson(justAuthSource));
		} catch (Exception e) {
