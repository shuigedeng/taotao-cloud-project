...
/**
 * rabbitMQ消息接收器：仅在vender=rabbitMQ时 && 项目实现IMQReceiver接口时 进行实例化 业务： 更新服务商/商户/商户应用配置信息
 *
 * @author terrfly
...
public class ResetIsvMchAppInfoRabbitMQReceiver implements IMQMsgReceiver {
     ...
     *
     * <p>注意： RabbitMQ的广播模式（fanout）交换机 --》全部的Queue 如果queue包含多个消费者，
    ...
    @RabbitListener(
            bindings = {
                @QueueBinding(
                        ...
                        exchange =
                                @Exchange(
                                        name = RabbitMQConfig.FANOUT_EXCHANGE_NAME_PREFIX
                                                + ResetIsvMchAppInfoConfigMQ.MQ_NAME,
