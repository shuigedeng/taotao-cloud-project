...
import com.taotao.cloud.payment.biz.jeepay.pay.rqrs.payorder.payway.QrCashierOrderRS;
...
public abstract class AbstractPayOrderController extends ApiController {
    ...
    private PayOrder genPayOrder(
        ...
        //        payOrder.setChannelExtra(rq.getChannelExtra()); //特殊渠道发起的附件额外参数,  是否应该删除该字段了？？
        // 比如authCode不应该记录， 只是在传输阶段存在的吧？  之前的为了在payOrder对象需要传参。
    ...
    /** 校验： 商户的支付方式是否可用 返回： 支付接口 * */
    private IPaymentService checkMchWayCodeAndGetService(
            MchAppConfigContext mchAppConfigContext, MchPayPassage mchPayPassage) {
        ...
        if (mchAppConfigContext.getMchType() == MchInfo.TYPE_NORMAL) { // 普通商户
            ...
            if (configContextQueryService.queryNormalMchParams(
                            mchAppConfigContext.getMchNo(), mchAppConfigContext.getAppId(), ifCode)
                    == null) {
                throw new BizException("商户应用参数未配置");
            }
        } else if (mchAppConfigContext.getMchType() == MchInfo.TYPE_ISVSUB) { // 特约商户

            if (configContextQueryService.queryIsvsubMchParams(
                            mchAppConfigContext.getMchNo(), mchAppConfigContext.getAppId(), ifCode)
                    == null) {
                throw new BizException("特约商户参数未配置");
            }

            if (configContextQueryService.queryIsvParams(
                            mchAppConfigContext.getMchInfo().getIsvNo(), ifCode)
                    == null) {
    ...
    /** 处理返回的渠道信息，并更新订单状态 payOrder将对部分信息进行 赋值操作。 * */
