...
public class RefundOrderProcessService {
    ...
    public boolean handleRefundOrder4Channel(ChannelRetMsg channelRetMsg, RefundOrder refundOrder) {
        ...
        if (channelRetMsg.getChannelState() == ChannelRetMsg.ChannelState.CONFIRM_SUCCESS) {
            updateOrderSuccess =
                    refundOrderService.updateIng2Success(
                            refundOrderId, channelRetMsg.getChannelOrderId());
            if (updateOrderSuccess) {
                // 通知商户系统
                if (StringUtils.isNotEmpty(refundOrder.getNotifyUrl())) {
                    payMchNotifyService.refundOrderNotify(
                            refundOrderService.getById(refundOrderId));
                }
            }
            // 确认失败
