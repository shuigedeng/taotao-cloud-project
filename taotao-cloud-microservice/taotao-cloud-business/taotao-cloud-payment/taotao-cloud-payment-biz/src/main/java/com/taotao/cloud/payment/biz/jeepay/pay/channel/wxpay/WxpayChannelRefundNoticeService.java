...
public class WxpayChannelRefundNoticeService extends AbstractChannelRefundNoticeService {
	...
	public MutablePair<String, Object> parseParams(HttpServletRequest request, String urlOrderId,
		...
		try {
			...
			if (CS.MCH_TYPE_NORMAL == mchType) {
				WxpayNormalMchParams normalMchParams = (WxpayNormalMchParams) configContextQueryService.queryNormalMchParams(
						mchAppConfigContext.getMchNo(), mchAppConfigContext.getAppId(),
						getIfCode());
				apiVersion = normalMchParams.getApiVersion();
				wxKey = CS.PAY_IF_VERSION.WX_V2.equals(apiVersion) ? normalMchParams.getKey()
				...
				wxKey = CS.PAY_IF_VERSION.WX_V2.equals(apiVersion) ? wxpayIsvParams.getKey()
			...
			if (CS.PAY_IF_VERSION.WX_V3.equals(apiVersion)) {     // V3接口回调
				// 验签 && 获取订单回调数据
				WxPayRefundNotifyV3Result.DecryptNotifyResult result = parseOrderNotifyV3Result(
	 ...
	 * @return
	 */
	public void verifyWxPay3Params(WxPayRefundNotifyV3Result.DecryptNotifyResult result,
	 ...
	 * @return
	 */
	public void verifyWxPay2Params(WxPayRefundNotifyResult.ReqInfo result,
			RefundOrder refundOrder) {
