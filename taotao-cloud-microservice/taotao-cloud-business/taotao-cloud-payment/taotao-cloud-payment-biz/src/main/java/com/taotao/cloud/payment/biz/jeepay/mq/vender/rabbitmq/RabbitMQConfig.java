...
public class RabbitMQConfig {
	...
	public void init() {
		...
		// 获取到所有的MQ定义
		Set<Class<?>> set = ClassUtil.scanPackageBySuper(ClassUtil.getPackage(AbstractMQ.class),
				AbstractMQ.class);
		...
		for (Class<?> aClass : set) {
			...
			rabbitMQBeanProcessor.beanDefinitionRegistry.registerBeanDefinition(amq.getMQName(),
					BeanDefinitionBuilder.rootBeanDefinition(Queue.class)
							.addConstructorArgValue(amq.getMQName()).getBeanDefinition());
			...
			if (amq.getMQType() == MQSendTypeEnum.BROADCAST) {
				...
				rabbitMQBeanProcessor.beanDefinitionRegistry.registerBeanDefinition(
						...
						BeanDefinitionBuilder.genericBeanDefinition(FanoutExchange.class, () -> {
									...
									return new FanoutExchange(FANOUT_EXCHANGE_NAME_PREFIX + amq.getMQName(),
											true, false);
				...
				rabbitMQBeanProcessor.beanDefinitionRegistry.registerBeanDefinition(
						...
						BeanDefinitionBuilder.genericBeanDefinition(Binding.class, () ->
								BindingBuilder.bind(
										...
										.to(delayedExchange).with(amq.getMQName()).noargs()
