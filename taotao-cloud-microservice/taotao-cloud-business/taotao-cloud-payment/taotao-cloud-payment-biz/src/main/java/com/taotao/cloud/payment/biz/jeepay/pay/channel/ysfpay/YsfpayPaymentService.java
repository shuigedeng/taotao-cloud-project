...
import com.taotao.cloud.payment.biz.jeepay.core.model.params.ysf.YsfpayConfig;
...
public class YsfpayPaymentService extends AbstractPaymentService {
    ...
    /** 封装参数 & 统一请求 * */
    public JSONObject packageParamAndReq(
            String apiUri,
        ...
        YsfpayIsvsubMchParams isvsubMchParams =
        ...
        // 签名
        String isvPrivateCertFile =
                channelCertConfigKitBean.getCertFilePath(isvParams.getIsvPrivateCertFile());
        String isvPrivateCertPwd = isvParams.getIsvPrivateCertPwd();
        reqParams.put(
                "signature",
                YsfSignUtils.signBy256(
                        reqParams, isvPrivateCertFile, isvPrivateCertPwd)); // RSA 签名串

        // 调起上游接口
        log.info("{} reqJSON={}", logPrefix, reqParams);
        String resText =
                YsfHttpUtil.doPostJson(getYsfpayHost4env(isvParams) + apiUri, null, reqParams);
    ...
    public static String getYsfpayHost4env(YsfpayIsvParams isvParams) {
        return CS.YES == isvParams.getSandbox()
                ? YsfpayConfig.SANDBOX_SERVER_URL
                : YsfpayConfig.PROD_SERVER_URL;
    }

    /** 云闪付 jsapi下单请求统一发送参数 * */
    public static void jsapiParamsSet(
            JSONObject reqParams, PayOrder payOrder, String notifyUrl, String returnUrl) {
    ...
    public static void ysfPublicParams(JSONObject reqParams, PayOrder payOrder) {
        ...
        reqParams.put(
                "orderTime",
                DateUtil.format(
                        new Date(), DatePattern.PURE_DATETIME_PATTERN)); // 订单时间 如：20180702142900
