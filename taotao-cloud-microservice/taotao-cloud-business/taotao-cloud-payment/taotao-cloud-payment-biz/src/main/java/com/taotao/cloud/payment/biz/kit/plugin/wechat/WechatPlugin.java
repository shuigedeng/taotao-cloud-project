...
public class WechatPlugin implements Payment {
    ...
    /** 联合登陆 */
    @Autowired private IFeignConnectService connectService;
    /** 联合登陆 */
    @Autowired private IFeignOrderApi orderApi;

    @Override
    public Result<Object> h5pay(
            HttpServletRequest request, HttpServletResponse response1, PayParam payParam) {
        ...
        try {
            ...
            // 过期时间
            String timeExpire =
                    DateTimeZoneUtil.dateToTimeZone(System.currentTimeMillis() + 1000 * 60 * 3);
            ...
            UnifiedOrderModel unifiedOrderModel =
                    new UnifiedOrderModel()
                            ...
                            .setAttach(attach)
                            .setNotify_url(
                                    notifyUrl(apiProperties.getBuyer(), PaymentMethodEnum.WECHAT))
        ...
        try {
            ...
            if (verifySignature) {
                ...
                Map<String, String> map =
                        WxPayKit.jsApiCreateSign(appid, prepayId, setting.getApiclient_key());
        ...
        try {
            ...
            Amount amount =
                    new Amount()
                            ...
                            .setTotal(
                                    CurrencyUtils.fen(
                                            orderApi.getPaymentTotal(refundLog.getOrderSn())));
            ...
            RefundModel refundModel =
                    new RefundModel()
                            ...
                            .setNotify_url(
                                    refundNotifyUrl(
                                            apiProperties.getBuyer(), PaymentMethodEnum.WECHAT));
    ...
    public void refundNotify(HttpServletRequest request) {
        ...
        try { // 校验服务器端响应¬
            ...
            if (("REFUND.SUCCESS").equals(ciphertext.getStr("event_type"))) {
                ...
                RefundLog refundLog =
                        refundLogService.getOne(
                                new LambdaQueryWrapper<RefundLog>()
                                        .eq(RefundLog::getPaymentReceivableNo, transactionId));
                if (refundLog != null) {
    ...
    private X509Certificate getPlatformCert() {
        ...
        String publicCert =
                (String) redisRepository.get(CachePrefix.WECHAT_PLAT_FORM_CERT.getPrefix());
        ...
        try {
            ...
            if (response.getStatus() == 200) {
                ...
                long second =
                        (PayKit.getCertificate(publicCert).getNotAfter().getTime()
                                        - System.currentTimeMillis())
                                / 1000;
                redisRepository.set(
                        CachePrefix.WECHAT_PLAT_FORM_CERT.getPrefix(), publicCert, second);
            } else {
    ...
    private String getPlatformCertStr(String associatedData, String nonce, String cipherText)
        ...
        AesUtil aesUtil =
                new AesUtil(wechatPaymentSetting().getApiKey3().getBytes(StandardCharsets.UTF_8));
        // 平台证书密文解密
