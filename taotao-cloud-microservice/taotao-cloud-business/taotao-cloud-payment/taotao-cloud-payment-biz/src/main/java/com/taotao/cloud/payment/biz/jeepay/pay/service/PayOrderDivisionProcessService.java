...
import com.taotao.cloud.payment.biz.jeepay.service.impl.PayOrderDivisionRecordService;
...
public class PayOrderDivisionProcessService {
     ...
     * @date 2021/8/27 9:44
     */
    public ChannelRetMsg processPayOrderDivision(
            String payOrderId,
        ...
        // 分账状态不正确
        if (payOrder.getDivisionState() != PayOrder.DIVISION_STATE_WAIT_TASK
        ...
        if (isResend) {
            ...
            // 查询&过滤 所有的分账接收对象
            List<MchDivisionReceiver> allReceiver =
                    this.queryReceiver(useSysAutoDivisionReceivers, payOrder, receiverList);
        ...
        try {
            ...
            if (channelRetMsg.getChannelState() == ChannelRetMsg.ChannelState.CONFIRM_SUCCESS) {
                ...
                payOrderDivisionRecordService.updateRecordSuccessOrFail(
                        recordList,
                        PayOrderDivisionRecord.STATE_SUCCESS,
                        channelRetMsg.getChannelOrderId(),
                        channelRetMsg.getChannelOriginResponse());
            ...
            payOrderDivisionRecordService.updateRecordSuccessOrFail(
                    recordList, PayOrderDivisionRecord.STATE_FAIL, null, "系统异常：" + e.getMessage());
    ...
    /** 生成对象信息 * */
    private PayOrderDivisionRecord genRecord(
            String batchOrderId,
            PayOrder payOrder,
            MchDivisionReceiver receiver,
    ...
    private List<MchDivisionReceiver> queryReceiver(
            Byte useSysAutoDivisionReceivers,
            PayOrder payOrder,
            List<PayOrderDivisionMQ.CustomerDivisionReceiver> customerDivisionReceiverList) {
        ...
        if (useSysAutoDivisionReceivers == CS.YES) {
            ...
            queryWrapper.eq(
                    MchDivisionReceiver::getReceiverGroupId, groups.get(0).getReceiverGroupId());
        }
