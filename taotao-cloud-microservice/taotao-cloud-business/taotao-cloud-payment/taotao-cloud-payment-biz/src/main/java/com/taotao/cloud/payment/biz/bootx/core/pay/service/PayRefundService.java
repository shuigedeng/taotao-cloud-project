...
public class PayRefundService {
	...
	public void refund(RefundParam refundParam) {
		Optional<Payment> paymentOptional = Optional.ofNullable(
				paymentService.getAndCheckPaymentByBusinessId(refundParam.getBusinessId()));
		paymentOptional.ifPresent(
				payment -> this.refundPayment(payment, refundParam.getRefundModeParams()));
	}
	...
	public void refundByBusinessId(String businessId) {
		Optional<Payment> paymentOptional = Optional.ofNullable(
				paymentService.getAndCheckPaymentByBusinessId(businessId));
		paymentOptional.ifPresent(
				payment -> this.refundPayment(payment, payment.getRefundableInfoList().stream()
	...
	private void refundPayment(Payment payment, List<RefundModeParam> refundModeParams) {
		// 删除退款金额为0的支付通道参数
		refundModeParams.removeIf(refundModeParam ->
				BigDecimalUtil.compareTo(refundModeParam.getAmount(), BigDecimal.ZERO) == 0);
		// 获取 paymentParam
		...
		List<PayModeParam> payModeParams = refundModeParams.stream()
				.map(RefundModeParam::toPayModeParam).collect(Collectors.toList());
		...
		this.doHandler(payment, paymentStrategyList, (strategyList, paymentObj) -> {
			...
			try {
				...
				// 记录退款失败的记录
				BigDecimal amount = refundModeParams.stream().map(RefundModeParam::getAmount)
	 ...
	 */
	private void payModeCheck(List<RefundModeParam> refundModeParams,
		...
		Map<Integer, RefundableInfo> payModeMap = refundableInfos.stream()
				.collect(Collectors.toMap(RefundableInfo::getPayChannel, o -> o));
	...
	public void payModeCheck(RefundModeParam refundModeParam, RefundableInfo refundableInfo) {
		...
		// 退款金额大于可退款金额
		if (BigDecimalUtil.compareTo(refundModeParam.getAmount(), refundableInfo.getAmount())
				== 1) {
	...
	public void saveRefund(Payment payment, BigDecimal amount,
			List<RefundModeParam> refundModeParams) {
		List<RefundableInfo> refundableInfos = refundModeParams.stream()
				.map(RefundModeParam::toRefundableInfo).collect(Collectors.toList());
		...
		RefundRecord refundRecord = new RefundRecord()
				...
				.setRefundStatus(Objects.isNull(AsyncRefundLocal.getErrorCode())
						? PayStatusCode.REFUND_PROCESS_SUCCESS : PayStatusCode.REFUND_PROCESS_FAIL);
