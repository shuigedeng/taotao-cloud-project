package com.taotao.cloud.payment.biz.infrastructure.daxpay.channel.union.sdk.api;
...
import com.taotao.cloud.payment.biz.daxpay.unisdk.common.bean.*;
import com.taotao.cloud.payment.biz.daxpay.unisdk.common.bean.outbuilder.PayTextOutMessage;
...
import com.taotao.cloud.payment.biz.daxpay.unisdk.common.http.HttpConfigStorage;
import com.taotao.cloud.payment.biz.daxpay.unisdk.common.http.UriVariables;
import com.taotao.cloud.payment.biz.daxpay.unisdk.common.util.Util;
import com.taotao.cloud.payment.biz.daxpay.unisdk.common.util.sign.CertDescriptor;
import com.taotao.cloud.payment.biz.daxpay.unisdk.common.util.sign.SignTextUtils;
import com.taotao.cloud.payment.biz.daxpay.unisdk.common.util.sign.SignUtils;
import com.taotao.cloud.payment.biz.daxpay.unisdk.common.util.sign.encrypt.RSA;
import com.taotao.cloud.payment.biz.daxpay.unisdk.common.util.sign.encrypt.RSA2;
import com.taotao.cloud.payment.biz.infrastructure.daxpay.channel.union.sdk.bean.*;
...
public class UnionPayKit extends BasePayService<UnionPayConfigStorage> {
     ...
     */
    @Override
    public UnionPayKit setPayConfigStorage(
            UnionPayConfigStorage payConfigStorage) {
    ...
    public String getQrPay(UniOrder order) {
        ...
        if (this.verify(new NoticeParams(response))) {
            ...
            throw new PayErrorException(new PayException((String) response.get(SDKConstants.param_respCode), (String) response.get(
                    SDKConstants.param_respMsg), response.toJSONString()));
        }
...
//                String tn =  (String)response.get(SDKConstants.param_tn);
    ...
    public UnionRefundResult refund(UniRefundOrder refundOrder) {
        ...
        if (this.verify(new NoticeParams(response))) {
            ...
            throw new PayErrorException(new PayException(response.getString(SDKConstants.param_respCode), response.getString(
                    SDKConstants.param_respMsg), response.toJSONString()));
        }
        throw new PayErrorException(new PayException("failure", "验证签名失败", response.toJSONString()));
    }
