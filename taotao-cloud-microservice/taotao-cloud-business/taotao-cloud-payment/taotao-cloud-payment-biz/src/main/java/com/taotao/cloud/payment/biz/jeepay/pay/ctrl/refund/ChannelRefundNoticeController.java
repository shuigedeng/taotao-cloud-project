...
import com.taotao.cloud.payment.biz.jeepay.pay.channel.IChannelRefundNoticeService;
...
import com.taotao.cloud.payment.biz.jeepay.pay.service.RefundOrderProcessService;
...
public class ChannelRefundNoticeController extends AbstractCtrl {
	 ...
	 **/
	@ResponseBody
	@RequestMapping(value = {"/api/refund/notify/{ifCode}",
			"/api/refund/notify/{ifCode}/{refundOrderId}"})
	public ResponseEntity doNotify(HttpServletRequest request,
			@PathVariable("ifCode") String ifCode,
			@PathVariable(value = "refundOrderId", required = false) String urlOrderId) {

		String refundOrderId = null;
		String logPrefix =
				"进入[" + ifCode + "]退款回调：urlOrderId：[" + StringUtils.defaultIfEmpty(urlOrderId,
						"") + "] ";
		log.info("===== {} =====", logPrefix);
		...
		try {
			...
			//查询退款接口是否存在
			IChannelRefundNoticeService refundNotifyService = SpringBeansUtil.getBean(
					ifCode + "ChannelRefundNoticeService", IChannelRefundNoticeService.class);
			...
			// 解析订单号 和 请求参数
			MutablePair<String, Object> mutablePair = refundNotifyService.parseParams(request,
					urlOrderId, IChannelRefundNoticeService.NoticeTypeEnum.DO_NOTIFY);
			...
			log.info("{}, 解析数据为：refundOrderId:{}, params:{}", logPrefix, refundOrderId,
					mutablePair.getRight());

			if (StringUtils.isNotEmpty(urlOrderId) && !urlOrderId.equals(refundOrderId)) {
				log.error("{}, 订单号不匹配. urlOrderId={}, refundOrderId={} ", logPrefix,
						urlOrderId, refundOrderId);
			...
			//调起接口的回调判断
			ChannelRetMsg notifyResult = refundNotifyService.doNotice(request,
					mutablePair.getRight(), refundOrder, mchAppConfigContext,
					IChannelRefundNoticeService.NoticeTypeEnum.DO_NOTIFY);

			// 返回null 表明出现异常， 无需处理通知下游等操作。
			if (notifyResult == null || notifyResult.getChannelState() == null
					|| notifyResult.getResponseEntity() == null) {
				log.error("{}, 处理回调事件异常  notifyResult data error, notifyResult ={} ",
						logPrefix, notifyResult);
				throw new BizException(
						"处理回调事件异常！"); //需要实现类自行抛出ResponseException, 不应该在这抛此异常。
			}
			// 处理退款订单
			boolean updateOrderSuccess = refundOrderProcessService.handleRefundOrder4Channel(
					notifyResult, refundOrder);
			...
			log.info("===== {}, 订单通知完成。 refundOrderId={}, parseState = {} =====", logPrefix,
					refundOrderId, notifyResult.getChannelState());
