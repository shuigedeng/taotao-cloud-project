...
public class MemberController {
	...
	public String homeIndex(@RequestParam(required = false) String userId,
		...
		// 获取Banner轮播图信息
		CompletableFuture<List<BannerVO>> future1 = CompletableFuture.supplyAsync(
			() -> this.buildBanners(userId, lang), asyncThreadPoolExecutor);
		// 获取用户message通知信息
		CompletableFuture<List<NotificationVO>> future2 = CompletableFuture.supplyAsync(
			() -> this.buildNotifications(userId, lang), asyncThreadPoolExecutor);
		// 获取用户权益信息
		CompletableFuture<List<BenefitVO>> future3 = CompletableFuture.supplyAsync(
