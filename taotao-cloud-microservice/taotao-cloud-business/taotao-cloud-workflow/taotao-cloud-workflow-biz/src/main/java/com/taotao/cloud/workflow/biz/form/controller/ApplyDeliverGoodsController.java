...
public class ApplyDeliverGoodsController {
    ...
    @Autowired private ApplyDeliverGoodsService applyDeliverGoodsService;
    ...
    @Operation("获取发货申请单信息")
    @GetMapping("/{id}")
    public Result<ApplyDeliverGoodsInfoVO> info(
                ...
                if (StringUtil.isNotEmpty(operator.getDraftData())) {
                    vo =
                            JsonUtils.getJsonToBean(
                                    operator.getDraftData(), ApplyDeliverGoodsInfoVO.class);
                    isData = false;
                }
            }
        }
        if (isData) {
            ...
            List<ApplyDeliverGoodsEntryEntity> entityList =
                    applyDeliverGoodsService.getDeliverEntryList(id);
    ...
    public Result create(@RequestBody @Valid ApplyDeliverGoodsForm applyDeliverGoodsForm)
            throws WorkFlowException {
        ApplyDeliverGoodsEntity deliver =
                JsonUtils.getJsonToBean(applyDeliverGoodsForm, ApplyDeliverGoodsEntity.class);
    ...
    public Result update(
            @RequestBody @Valid ApplyDeliverGoodsForm applyDeliverGoodsForm,
        ...
        applyDeliverGoodsService.submit(
                id, deliver, deliverEntryList, applyDeliverGoodsForm.getCandidateList());
        return Result.success(MsgCode.SU006.get());
    }
}
