...
public class VisualDevTableCre {
	...
	public void fieldsTableName(JSONArray jsonArray, List<TableModel> tableModels) {
		for (int i = 0; i < jsonArray.size(); i++) {
			...
			if (FormEnum.card.getMessage().equals(flowKey) || FormEnum.row.getMessage()
				.equals(flowKey) || FormEnum.tab.getMessage().equals(flowKey)
				|| FormEnum.collapse.getMessage().equals(flowKey) || StrUtil.isEmpty(flowKey)) {
				JSONArray childArray = jsonObject.getJSONObject("__config__")
	...
	private TableModel fieldsModel(JSONObject jsonObject, List<TableModel> tableModels) {
		...
		String relationField =
			StrUtil.isNotEmpty(jsonObject.getString("relationField")) ? jsonObject.getString(
				"relationField") : "";
		String flowKey = jsonObject.getJSONObject("__config__").getString("flowKey");
		TableModel tableName = tableModels.stream().filter(t -> "1".equals(t.getTypeId()))
		...
		List<TableModel> childTableAll = tableModels.stream().filter(t -> "0".equals(t.getTypeId()))
			.toList();
		TableModel childTableaa = childTableAll.stream().filter(
				t -> t.getFields().stream().anyMatch(k -> k.getField().equals(vModel)))
			.findFirst().orElse(null);
			...
			if (StrUtil.isNotEmpty(relationField)) {
				boolean isSubTable =
					jsonObject.getJSONObject("__config__").getBoolean("isSubTable") != null
						? jsonObject.getJSONObject("__config__").getBoolean("isSubTable") : false;
	 ...
	 * @return
	 */
	public List<TableModel> tableList(JSONArray jsonArray, List<FormAllModel> formAllModel,
		String table, String linkId) throws WorkFlowException {
			...
			for (FormAllModel model : formAllModel) {
				if (FormEnum.mast.getMessage().equals(model.getFlowKey())) {
					...
					List<FieLdsModel> fieldsList = fieLdsModel.getChildList().stream()
						.map(t -> t.getFieLdsModel()).collect(Collectors.toList());
	 ...
	 * @param tableList
	 */
	private void fieldList(FieLdsModel fieLdsModel, String table,
		List<DbTableFieldModel> tableList) {
		...
		if (StrUtil.isNotEmpty(vmodel)) {
			...
			if (StrUtil.isNotEmpty(fieLdsModel.getVModel())) {
				if (FlowKeyConsts.UPLOADIMG.equals(flowKey) || FlowKeyConsts.UPLOADFZ.equals(
					flowKey) || FlowKeyConsts.ADDRESS.equals(flowKey)) {
					fieldForm.setDataType("text");
				}
				if (FlowKeyConsts.MODIFYTIME.equals(flowKey) || FlowKeyConsts.CREATETIME.equals(
					flowKey) || FlowKeyConsts.DATE.equals(flowKey)) {
	 ...
	 * @param isforeign
	 */
	private void tableModel(List<TableModel> tableModelList, List<DbTableFieldModel> dbtable,
		String table, String mastTable, boolean isforeign) {
	 ...
	 * @return
	 */
	private DbTableCreate dbTable(String linkId, String tableName,
		List<DbTableFieldModel> tableFieldList, boolean isforeign) {
