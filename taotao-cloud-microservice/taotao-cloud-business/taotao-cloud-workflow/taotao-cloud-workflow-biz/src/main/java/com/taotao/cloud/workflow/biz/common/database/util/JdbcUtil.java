...
public class JdbcUtil {
     ...
     * @param DTOs
     */
    public static List<Boolean> creUpDeBatch(Connection conn, List<PreparedStatementDTO> DTOs)
    ...
    private static List<Boolean> creUpDeBase(Connection conn, List<PreparedStatementDTO> DTOs)
            throws SQLException {
        // 执行结果标识
     ...
     * @throws DataException
     */
    public static int[] creUpDeBatchOneSql(
            Connection conn, String sql, List<PreparedStatementDTO> DTOs) throws DataException {
        try {
    ...
    /** 通用：多条查询 */
    public static List<Map<String, Object>> queryList(PreparedStatementDTO statementDTO)
            throws DataException {
    ...
    /** 通用：多条查询（查询别名） */
    public static List<Map<String, Object>> queryListAlias(PreparedStatementDTO statementDTO)
            throws DataException {
        JdbcDTO<JdbcGetMod> jdbcDTO = new JdbcDTO<>();
        jdbcDTO.setReturnType(DbConst.MAP_MOD);
    ...
    /** 通用：多条查询（开启小写） */
    public static List<Map<String, Object>> queryListLowercase(PreparedStatementDTO statementDTO)
            throws DataException {
        JdbcDTO<JdbcGetMod> jdbcDTO = new JdbcDTO<>();
        jdbcDTO.setReturnType(DbConst.MAP_MOD);
        // 开启小写模式
    ...
    /** 通用：单条查询 */
    public static Map<String, Object> queryOne(PreparedStatementDTO statementDTO)
            throws DataException {
    ...
    /** 查单条Int类型返回值 */
    public static Integer queryOneInt(PreparedStatementDTO statementDTO, String keyWord)
            throws DataException {
        List<Map<String, Object>> mapList = queryList(statementDTO);
        if (mapList.size() > 0) {
    ...
    public static List<List<DbFieldMod>> queryIncludeFieldMods(PreparedStatementDTO statementDTO)
            throws DataException {
        JdbcDTO<JdbcGetMod> jdbcDTO = new JdbcDTO<>();
    ...
    /** 专用：打印模板使用 */
    public static List<List<DbFieldMod>> queryTableFields(PreparedStatementDTO statementDTO)
            throws DataException {
        JdbcDTO<JdbcGetMod> jdbcDTO = new JdbcDTO<>();
    ...
    /** 自定义末班查询 */
    public static <T extends JdbcGetMod> List<T> queryCustomMods(
            PreparedStatementDTO statementDTO, Class<T> modType) throws DataException {
    ...
    public static JdbcPageMod queryPage(
            PreparedStatementDTO statementDTO,
            String sortType,
            Integer currentPage,
            Integer pageSize,
            JdbcDTO jdbcDTO)
        ...
        String[] sqlArray =
                sqlBase.getPageSql(statementDTO.getPrepareSql(), sortType, currentPage, pageSize);
        ...
        try {
            ...
            statementDTO.setPrepareSql(selectSql);
            pageModel.setTotalRecord(
                    queryOneInt(statementDTO, DbAliasEnum.TOTAL_RECORD.asByDb(db)));
     ...
     */
    private static <T extends JdbcGetMod> JdbcDTO<T> query(
            PreparedStatementDTO statementDTO, JdbcDTO<T> jdbcDTO) throws DataException {
        try {
            String sql = statementDTO.getPrepareSql();
    ...
    private static <T extends JdbcGetMod> JdbcDTO<T> getList(ResultSet result, JdbcDTO<T> jdbcDTO)
            throws Exception {
        // 自定义jdbc模型对象
     ...
     */
    private static <T> Map<String, Object> getModel0(ResultSet rs, JdbcDTO<T> jdbcDTO)
            throws SQLException {
     ...
     * @throws SQLException ignore
     */
    private static <T> List<DbFieldMod> getModel1(ResultSet rs, JdbcDTO<T> jdbcDTO)
            throws SQLException {
        ResultSetMetaData md = rs.getMetaData();
        int columnCount = md.getColumnCount();
     ...
     * @throws SQLException ignore
     */
    private static <T> List<DbFieldMod> getModel3(ResultSet rs, JdbcDTO<T> jdbcDTO)
            throws SQLException {
        ResultSetMetaData md = rs.getMetaData();
     ...
     * @throws SQLException
     */
    private static <T> Map<String, Object> getModel4(ResultSet rs, JdbcDTO<T> jdbcDTO)
            throws SQLException {
        ResultSetMetaData md = rs.getMetaData();
        Map<String, Object> map = new HashMap<>();
        // 获取字段集合信息
