...
public class SqlSQLServer extends SqlBase {
    ...
    protected void init() {
        String fieldListSql =
                "SELECT cast(a.name as varchar(50)) "
        ...
        String tableListSql =
                "SET NOCOUNT ON DECLARE @TABLEINFO TABLE ( NAME VARCHAR(50) , SUMROWS VARCHAR(11) ,"
    ...
    public static List<PreparedStatementDTO> getTableComment(
        ...
        for (DbTableFieldModel model : models) {
            PSDs.add(
                    insertFieldCommentPSD(
                            model.getFieldName(), "dbo", newTableName, model.getField()));
        }
        return PSDs;
    }

    /** 添加表注释 */
    private static PreparedStatementDTO insertTableCommentPSD(
            String comment, String schema, String table) {
