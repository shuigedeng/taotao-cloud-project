package com.taotao.cloud.report.biz.strategy;
...
public class InitSystemContext {
	...
	private InitSystemStrategy getInitSystemStrategy() {
		InitSystemStrategy initSystemStrategy = initSystemStrategyMap.get(
			databaseProperties.getMultiTenantType().name());
		ArgumentAssert.notNull(initSystemStrategy, "您配置的租户模式:{}不可用",
			databaseProperties.getMultiTenantType().name());
