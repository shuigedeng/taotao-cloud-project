 ...
 * <p>
 * NONE("非租户模式"): 不存在租户的概念 COLUMN("字段模式"): 在sql中拼接 tenant_code 字段 SCHEMA("独立schema模式"): 在sql中拼接 数据库
 * schema
 * <p>
 * COLUMN和SCHEMA模式的实现 参考下面的 @see 中的3个类
 *
 * @author zuihou
 * @date 2017-11-18 0:34 断点查看原理：👇👇👇
...
public class TenantDatabaseAutoConfiguration extends BaseMasterDatabaseConfiguration {
	...
	public TenantDatabaseAutoConfiguration(MybatisPlusProperties properties,
		...
		super(properties, databaseProperties, interceptorsProvider, typeHandlersProvider,
			...
			applicationContext);
		log.debug(
			"检测到 lamp.database.multiTenantType!=DATASOURCE，加载了 AuthorityDatabaseAutoConfiguration");
	}
