 ...
 * 初始化系统
 * <p>
 * 初始化规则： 表结构： lamp-tenant-server/src/main/resources/schema/{数据库类型}/{数据库前缀}.sql 租户初始数据：
 * lamp-tenant-server/src/main/resources/data/{数据库类型}/{数据库前缀}.sql
 * <p>
 * 不支持ORACLE！
 *
 * @author zuihou
...
public class SchemaInitSystemStrategy implements InitSystemStrategy {
	...
	public void initDatabase(String tenant) {
		databaseProperties.getInitDatabasePrefix().forEach(
			database -> this.initDbMapper.createDatabase(
				StrUtil.join(StrUtil.UNDERLINE, database, tenant)));
	}
	...
	private void runScript(ScriptRunner runner, String tenant, String path) {
		try {
			for (String database : databaseProperties.getInitDatabasePrefix()) {
				String useDb = StrUtil.format("use {};",
					StrUtil.join(StrUtil.UNDERLINE, database, tenant));
				...
				Reader reader = Resources.getResourceAsReader(
					StrUtil.format(path, getDbType().getDb(), database));
...
//            runner.setDelimiter(databaseProperties.getDelimiter());
	...
	public boolean delete(List<Long> ids, List<String> tenantCodeList) {
		...
		databaseProperties.getInitDatabasePrefix()
			.forEach(prefix -> tenantCodeList.forEach(tenant -> {
