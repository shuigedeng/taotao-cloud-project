...
public abstract class SliceUploadTemplate implements SliceUploadStrategy {
	...
	public FileUpload sliceUpload(FileUploadRequest param) {
		...
		if (isOk) {
			...
			FileUpload fileUploadDTO = this.saveAndFileUploadDTO(
				param.getFile().getOriginalFilename(), tmpFile);
	...
	public boolean checkAndSetUploadProgress(FileUploadRequest param, String uploadDirPath) {
		...
		boolean isOk = setUploadProgress2Redis(param, uploadDirPath, fileName, confFile,
			isComplete);
		return isOk;
	}
	...
	public FileUpload saveAndFileUploadDTO(String fileName, File tmpFile) {
		...
		try {
			...
			if (fileUploadDTO.isUploadComplete()) {
				System.out
					.println("upload complete !!" + fileUploadDTO.isUploadComplete() + " name="
						+ fileName);
				//TODO 保存文件信息到数据库
