...
public class DateUtil {
	...
	public static String dateToStrByType(Date date, DateType dateType) {
		...
		switch (dateType) {
			...
			case YEAR_WEEK:
				dateStr =
					dateToStr(date, "yyyy") + "|" + String.format("%02d", getWhichWeekOfYear(date));
				break;
			case MONTH_WEEK:
	...
	public static Date getYearWeekDateByInterval(Date date, int interval) {
		if (interval < 0) {
			...
			int weekOfYear = (int) Math.ceil(
				(c.get(Calendar.DAY_OF_YEAR) + wholeDayOfWeek - getDayOfWeek(c.getTime()))
			...
			int maxWeekOfYear = (int) Math.ceil(
				(temp.get(Calendar.DAY_OF_YEAR) + wholeDayOfWeek - getDayOfWeek(temp.getTime()))
					/ (double) wholeDayOfWeek);
