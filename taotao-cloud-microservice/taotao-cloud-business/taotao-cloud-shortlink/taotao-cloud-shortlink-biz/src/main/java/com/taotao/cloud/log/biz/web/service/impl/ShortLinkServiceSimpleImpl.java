package com.taotao.cloud.log.biz.web.service.impl;

import com.alibaba.fastjson.JSONObject;
import com.taotao.cloud.log.api.api.dto.ShortLinkDTO;
import com.taotao.cloud.log.biz.web.biz.ShortLinkBiz;
...
public class ShortLinkServiceSimpleImpl implements IShortLinkService {
	...
	public Optional<ShortLinkDTO> getShortLinkFromCache(String shortLinkCode) {
		RBucket<String> cacheBucket = redissonClient.getBucket(
			String.format(CACHE_KEY_SHORT_CODE, shortLinkCode));
	...
	public Optional<String> parseShortLinkCode(String shortLinkCode) {
		RBucket<String> codeBucket = redissonClient.getBucket(
			String.format(CACHE_KEY_SHORT_CODE, shortLinkCode));
...
//        codeBucket.set(JSONObject.toJSONString(shortLinkDTO), getShortLinkCodeCacheTime(), TimeUnit.MILLISECONDS);
//        return Optional.ofNullable(shortLinkDTO.getOriginUrl());

		// TIPS: 双写一致性的解决：引入分布式锁，to see ShortLinkServiceSimpleImpl.updateShortLinkCode
		// TIPS: 极端情况：突发流量请求的是冷门数据，导致缓存击穿，大量请求都囤积在这里，发生严重锁竞争,并发变成串行(load db + write cache)，即使有double check
		...
		try {
			if (lock.tryLock(1000L, TimeUnit.MILLISECONDS)) {
				...
				if (StringUtils.isNoneBlank(cacheJson) || !CACHE_EMPTY.equals(cacheJson)) {
					ShortLinkDTO shortLinkDTO = JSONObject.parseObject(cacheJson,
						ShortLinkDTO.class);
					if (shortLinkBiz.checkShortLinkCodeValid(shortLinkDTO)) {
						codeBucket.expire(Duration.ofMillis(getShortLinkCodeCacheTime()));
						log.info("解析短链：double check缓存，拿到缓存数据，code -> {}",
							shortLinkDTO.getCode());
						return Optional.ofNullable(shortLinkDTO.getOriginUrl());
					}
				}

				Optional<ShortLinkDTO> shortLinkCodeOpt = shortLinkBiz.getShortLinkCodeDto(
					shortLinkCode);
				if (!shortLinkCodeOpt.isPresent()) {
	...
	public Boolean updateShortLinkCode(ShortLinkUpdateRequest request) {
		...
		if (lock.tryLock()) {
			try {
				Boolean result = shortLinkBiz.updateShortLinkCode(
					ShortLinkDTO.builder().id(request.getId())
						.code(request.getCode())
						.originUrl(request.getOriginUrl()).build());
				...
				// 写缓存
				ShortLinkDTO shortLinkDTO = shortLinkCodeOpt.get();
				redissonClient.getBucket(String.format(CACHE_KEY_SHORT_CODE, request.getCode()))
					.set(JSONObject.toJSONString(shortLinkDTO), getShortLinkCodeCacheTime(),
						TimeUnit.MILLISECONDS);
