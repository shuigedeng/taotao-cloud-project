...
public class WeChatTemplateService {
    ...
    /** 分页 */
    public PageResult<WeChatTemplateDto> page(
            PageQuery PageQuery, WeChatTemplateParam weChatTemplateParam) {
        return MpUtil.convert2DtoPageResult(
                weChatTemplateManager.page(PageQuery, weChatTemplateParam));
    }

    /** 获取单条 */
    public WeChatTemplateDto findById(Long id) {
        return weChatTemplateManager
                .findById(id)
                .map(WeChatTemplate::toDto)
                .orElseThrow(DataNotExistException::new);
    }
    ...
    public void sync() {
        ...
        List<WxMpTemplate> wxTemplates =
                templateMsgService.getAllPrivateTemplate().stream()
        ...
        List<Long> deleteIds =
                weChatTemplates.stream()
                        ...
                        .collect(Collectors.toList());
        // 新增 服务端有且本地没有
        List<WeChatTemplate> saveTemplate =
                wxTemplates.stream()
        ...
        SecurityUtil.getCurrentUser()
                .ifPresent(
                        userDetail ->
                                userWsNoticeService.sendMessageByUser(
