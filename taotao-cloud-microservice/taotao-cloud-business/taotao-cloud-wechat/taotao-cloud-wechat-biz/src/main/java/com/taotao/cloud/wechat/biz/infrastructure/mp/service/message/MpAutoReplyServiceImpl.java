...
package com.taotao.cloud.wechat.biz.infrastructure.mp.service.message;
...
public class MpAutoReplyServiceImpl implements MpAutoReplyService {
    ...
    public Long createAutoReply(MpAutoReplyCreateReqVO createReqVO) {
        ...
        validateAutoReplyConflict(
                null,
                createReqVO.getAccountId(),
                createReqVO.getType(),
                createReqVO.getRequestKeyword(),
                createReqVO.getRequestMessageType());

        // 第二步，插入数据
        MpAccountDO account = mpAccountService.getRequiredAccount(createReqVO.getAccountId());
        MpAutoReplyDO autoReply =
                MpAutoReplyConvert.INSTANCE.convert(createReqVO).setAppId(account.getAppId());
    ...
    public void updateAutoReply(MpAutoReplyUpdateReqVO updateReqVO) {
        ...
        validateAutoReplyConflict(
                updateReqVO.getId(),
                autoReply.getAccountId(),
                updateReqVO.getType(),
                updateReqVO.getRequestKeyword(),
     ...
     * 校验自动回复是否冲突
     *
     * <p>不同的 type，会有不同的逻辑： 1. type = SUBSCRIBE 时，不允许有其他的自动回复 2. type = MESSAGE 时，校验
     * requestMessageType 已经存在自动回复 3. type = KEYWORD 时，校验 keyword 已经存在自动回复
     *
     * @param id 自动回复编号
    ...
    private MpAutoReplyDO buildDefaultSubscribeAutoReply(String appId) {
        ...
        return new MpAutoReplyDO()
                ...
                .setResponseMessageType(WxConsts.XmlMsgType.TEXT)
                .setResponseContent("感谢关注");
    }
}
