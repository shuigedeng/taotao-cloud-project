...
public class GrayConfiguration {
	...
	public ReactiveLoadBalancerClientFilter gatewayLoadBalancerClientFilter(
		...
		return new GrayReactiveLoadBalancerClientFilter(clientFactory,
	...
	public static class GrayReactiveLoadBalancerClientFilter extends
		...
		public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
			...
			if (url != null && ("grayLb".equals(url.getScheme()) || "grayLb"
				.equals(schemePrefix))) {
		...
		private Mono<Response<ServiceInstance>> choose(ServerWebExchange exchange) {
			...
			if (Objects.nonNull(uri)) {
				GrayLoadBalancer grayLoadBalancer = new GrayLoadBalancer(uri.getHost(),
					clientFactory
						.getLazyProvider(uri.getHost(), ServiceInstanceListSupplier.class));
	...
	public static class GrayLoadBalancer implements ReactorServiceInstanceLoadBalancer {
		...
		private Response<ServiceInstance> getServiceInstanceResponseWithGray(
			...
			String reqVersion = headers
				.getFirst(CommonConstant.TAOTAO_CLOUD_REQUEST_VERSION_HEADER);
			...
			if (StrUtil.isNotBlank(reqVersion)) {
				...
				final Set<Entry<String, String>> attributes = Collections
					.unmodifiableSet(versionMap.entrySet());
