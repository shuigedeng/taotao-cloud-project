...
public class SqlInjectionFilter implements GlobalFilter, Ordered {
	...
	private Mono<Void> sqlInjectionResponse(ServerWebExchange exchange, URI uri) {
		LogUtils.error("Paramters of Request [" + uri.getRawPath() + uri.getRawQuery()
			+ "] contain illegal SQL keyword!");
		return WebFluxUtil.writeJsonResponse(exchange.getResponse(),
			Result.fail(ResultEnum.SQL_INJECTION_REQUEST));
	}
