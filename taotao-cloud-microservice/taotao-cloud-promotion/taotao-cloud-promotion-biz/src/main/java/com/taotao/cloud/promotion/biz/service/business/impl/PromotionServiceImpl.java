...
public class PromotionServiceImpl implements IPromotionService {
	...
	public Map<String, Object> getCurrentPromotion() {
		...
		//获取当前进行的满优惠活动
		List<FullDiscount> fullDiscountList = fullDiscountService.listFindAll(
	...
	public Map<String, Object> getGoodsCurrentPromotionMap(EsGoodsIndex index) {
		...
		fullDiscountSearchParams.setPromotionStatus(PromotionsStatusEnum.START.name());
		List<FullDiscount> fullDiscountVOS = this.fullDiscountService.listFindAll(
			fullDiscountSearchParams);
		...
		for (Coupon coupon : couponVOS) {
			if (("platform").equals(coupon.getStoreId()) || index.getStoreId()
				.equals(coupon.getStoreId())) {
		...
		List<PromotionGoods> promotionGoodsList = promotionGoodsService.listFindAll(
		...
		for (PromotionGoods promotionGoods : promotionGoodsList) {
			...
			switch (PromotionTypeEnum.valueOf(promotionGoods.getPromotionType())) {
				...
				case PINTUAN:
					...
					break;
				case FULL_DISCOUNT:
					FullDiscount fullDiscount = fullDiscountService.getById(
				...
				case SECKILL:
					...
					break;
				case POINTS_GOODS:
					PointsGoods pointsGoods = pointsGoodsService.getById(
						promotionGoods.getPromotionId());
	...
	private void getGoodsCurrentSeckill(PromotionGoods promotionGoods,
		Map<String, Object> promotionMap, EsGoodsIndex index) {
