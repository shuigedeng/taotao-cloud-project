...
public class SeckillServiceImpl extends
	AbstractPromotionsServiceImpl<SeckillMapper, Seckill> implements
	ISeckillService {
	...
	public void init() {
		...
		for (Seckill seckill : seckillList) {
			...
			this.esGoodsIndexApi.updateEsGoodsIndexAllByList(seckill,
				PromotionTypeEnum.SECKILL.name() + "-" + seckill.getId());
		}
		this.remove(new QueryWrapper<>());

		Result<SettingVO> settingResult = settingApi.get(
			SettingCategoryEnum.SECKILL_SETTING.name());
		SeckillSetting seckillSetting = new Gson().fromJson(settingResult.getSettingValue(),
			SeckillSetting.class);
		...
		if (seckill != null) {
			...
			updateWrapper.set(Seckill::getGoodsNum,
				this.seckillApplyService.count(
					new LambdaQueryWrapper<SeckillApply>().eq(SeckillApply::getSeckillId,
						seckillId)));
	...
	public void updateEsGoodsSeckill(Seckill seckill, List<SeckillApply> seckillApplies) {
		if (seckillApplies != null && !seckillApplies.isEmpty()) {
			...
			for (SeckillApply seckillApply : seckillApplies) {
				if (seckillApply.getPromotionApplyStatus()
					.equals(PromotionsApplyStatusEnum.PASS.name())) {
					this.setSeckillApplyTime(seckill, seckillApply);
					log.info("更新限时抢购商品状态:{}", seckill);
					String promotionKey =
						PromotionTypeEnum.SECKILL.name() + "-" + seckillApply.getTimeLine();
					this.esGoodsIndexApi.updateEsGoodsIndexPromotions(seckillApply.getSkuId(),
						seckill, promotionKey, seckillApply.getPrice());
				}
			}
		}
	}
	...
	public void setSeckillApplyTime(Seckill seckill, SeckillApply seckillApply) {
		//下一个时间，默认为当天结束时间
		int nextHour = PromotionTools.nextHour(seckill.getHours().split(","),
			seckillApply.getTimeLine());

		String format = DateUtil.format(seckill.getStartTime(), DatePattern.NORM_DATE_PATTERN);
		DateTime parseStartTime = DateUtil.parse((format + " " + seckillApply.getTimeLine()),
			"yyyy-MM-dd HH");
		...
		if (nextHour == seckillApply.getTimeLine()) {
			parseEndTime = DateUtil.parse((format + " " + nextHour + ":59:59"),
	...
	public void checkStatus(Seckill promotions) {
		...
		if (promotions.getStartTime() != null && CharSequenceUtil.isNotEmpty(
			promotions.getHours())) {
			String[] split = promotions.getHours().split(",");
			Arrays.sort(split);
			String startTimeStr =
				DateUtil.format(promotions.getStartTime(), DatePattern.NORM_DATE_PATTERN) + " "
					+ split[0] + ":00";
			promotions.setStartTime(
				DateUtil.parse(startTimeStr, DatePattern.NORM_DATETIME_MINUTE_PATTERN));
			promotions.setEndTime(DateUtil.endOfDay(promotions.getStartTime()));
		}
		if (promotions.getStartTime() != null && promotions.getEndTime() != null) {
			//同一时间段内相同的活动
			QueryWrapper<Seckill> queryWrapper = PromotionTools.checkActiveTime(
				promotions.getStartTime(), promotions.getEndTime(), PromotionTypeEnum.SECKILL, null,
				promotions.getId());
