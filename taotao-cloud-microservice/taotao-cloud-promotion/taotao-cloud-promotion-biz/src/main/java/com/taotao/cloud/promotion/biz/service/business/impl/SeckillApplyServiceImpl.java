...
public class SeckillApplyServiceImpl extends
	ServiceImpl<SeckillApplyMapper, SeckillApply> implements
	...
	public List<SeckillGoodsVO> getSeckillGoods(Integer timeline) {
		...
		if (cacheSeckill == null || cacheSeckill.isEmpty()) {
			//如缓存中不存在，则单独获取
			List<SeckillTimelineVO> seckillTimelineToCache = getSeckillTimelineToCache(
				seckillCacheKey);
			...
			//如缓存中存在，则取缓存中转为展示的信息
			Optional<SeckillTimelineVO> first = cacheSeckill.stream()
				.filter(i -> i.getTimeLine().equals(timeline)).findFirst();
			if (first.isPresent()) {
				seckillGoodsVoS = first.get().getSeckillGoodsList();
			}
	...
	public IPage<SeckillApply> getSeckillApply(SeckillPageQuery queryParam, PageVO pageVo) {
		IPage<SeckillApply> seckillApplyPage = this.page(PageUtil.initPage(pageVo),
	...
	@Override
	public void addSeckillApply(String seckillId, String storeId,
		List<SeckillApplyVO> seckillApplyList) {
		...
		//获取已参与活动的秒杀活动活动申请列表
		List<String> skuIds = seckillApplyList.stream().map(SeckillApply::getSkuId)
		...
		for (SeckillApplyVO seckillApply : seckillApplyList) {
			...
			//获取秒杀活动时间段
			DateTime startTime = DateUtil.offsetHour(seckill.getStartTime(),
		...
		if (!promotionGoodsList.isEmpty()) {
			...
			searchParams.setSkuIds(promotionGoodsList.stream().map(PromotionGoods::getSkuId)
				.collect(Collectors.toList()));
			promotionGoodsService.deletePromotionGoods(searchParams);
			//初始化促销商品
			PromotionTools.promotionGoodsInit(promotionGoodsList, seckill,
	...
	public void removeSeckillApply(String seckillId, String id) {
		...
		//删除促销商品
		this.promotionGoodsService.deletePromotionGoods(seckillId,
			Collections.singletonList(seckillApply.getSkuId()));
	}
		...
		for (SeckillApplyVO seckillApply : seckillApplyList) {
			...
			if (existSku.contains(seckillApply.getSkuId())) {
				throw new BusinessException(
					seckillApply.getGoodsName() + "该商品不能同时参加多个时间段的活动");
			} else {
	...
	private List<SeckillTimelineVO> getSeckillTimelineToCache(String seckillCacheKey) {
		...
		//查询当天时间段内的秒杀活动活动
		Date now = new Date();
		queryWrapper.between(BasePromotions::getStartTime, DateUtil.beginOfDay(now),
			DateUtil.endOfDay(now));
		...
		for (Seckill seckill : seckillList) {
			...
			for (int i = 0; i < hoursSored.length; i++) {
				...
				if (hoursSoredHour) {
					...
					//秒杀活动的时刻
					long timeLine = DateUtil.getDateline(date + " " + hoursSored[i],
						"yyyy-MM-dd HH");
	...
	private List<SeckillGoodsVO> wrapperSeckillGoods(Integer startTimeline, String seckillId) {
		...
		if (!seckillApplyList.isEmpty()) {
			List<SeckillApply> collect = seckillApplyList.stream().filter(
				i -> i.getTimeLine().equals(startTimeline) && i.getPromotionApplyStatus()
					.equals(PromotionsApplyStatusEnum.PASS.name())).collect(Collectors.toList());
	 ...
	 */
	private void checkSeckillGoodsSku(Seckill seckill, SeckillApplyVO seckillApply,
		GoodsSku goodsSku, DateTime startTime) {
		//活动库存不能大于商品库存
		...
		if (promotionGoodsService.findInnerOverlapPromotionGoods(PromotionTypeEnum.PINTUAN.name(),
			...
			throw new BusinessException("商品[" + goodsSku.getGoodsName()
				+ "]已经在重叠的时间段参加了拼团活动，不能参加秒杀活动");
		}
		//查询是否在同一时间段参与了秒杀活动活动
		if (promotionGoodsService.findInnerOverlapPromotionGoods(PromotionTypeEnum.SECKILL.name(),
			goodsSku.getId(), startTime, seckill.getEndTime(), seckill.getId()) > 0) {
			throw new BusinessException("商品[" + goodsSku.getGoodsName()
				+ "]已经在重叠的时间段参加了秒杀活动，不能参加秒杀活动活动");
		}
	}
	 ...
	 * @return 秒杀活动促销商品
	 */
	private PromotionGoods setSeckillGoods(GoodsSku goodsSku, SeckillApply seckillApply,
		Seckill seckill) {
		//设置促销商品默认内容
