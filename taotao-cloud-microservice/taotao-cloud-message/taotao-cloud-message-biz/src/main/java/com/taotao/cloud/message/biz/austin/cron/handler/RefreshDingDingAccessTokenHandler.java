...
public class RefreshDingDingAccessTokenHandler {
	...
	public void execute() {
		...
		SupportThreadPoolConfig.getPendingSingleThreadPool().execute(() -> {
			...
			for (ChannelAccount channelAccount : accountList) {
				DingDingWorkNoticeAccount account = JSON.parseObject(
						channelAccount.getAccountConfig(), DingDingWorkNoticeAccount.class);
				String accessToken = getAccessToken(account);
				if (StrUtil.isNotBlank(accessToken)) {
					redisTemplate.opsForValue()
							.set(SendAccountConstant.DING_DING_ACCESS_TOKEN_PREFIX
	...
	private String getAccessToken(DingDingWorkNoticeAccount account) {
		...
		try {
			...
			log.error("RefreshDingDingAccessTokenHandler#getAccessToken fail:{}",
