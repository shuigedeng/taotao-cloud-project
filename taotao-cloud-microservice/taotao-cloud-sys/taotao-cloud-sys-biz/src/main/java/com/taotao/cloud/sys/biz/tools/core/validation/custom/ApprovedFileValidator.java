                    ...
                    try {
                       ...
                       char[] chars = Hex.encodeHex(buffer28);
                        ...
                        if(!startsWith){
                            ...
                            LogUtil.error("魔术数字检查失败，当前文件 {} 的魔术数字 {} 不是目标类型 {} ",fileName,new String(chars),key);
                        }
                        return startsWith;
                    }catch (IOException e){ LogUtil.error("读前 28 位流出错:"+e.getMessage(),e);}
                    finally {IOUtils.closeQuietly(inputStream);}
                }
            }

            //可能当前文件类型不受解析支持，但有可能是正确的
	        LogUtil.warn("可能当前文件类型[{}]不受解析支持，但有可能是正确的",fileMimeType);
            return true;
        } catch (IOException e) {
	        LogUtil.error("解析文件类型时读流出错:"+e.getMessage(),e);
        }
