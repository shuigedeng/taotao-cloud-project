				...
				if (keywordArray.length != 1) {
					for (int i = 0; i < keywordArray.length; i++) {
						if (i == keywordArray.length - 1) {
							if (!"AND".equalsIgnoreCase(keywordArray[i]) && !"OR".equalsIgnoreCase(
								keywordArray[i]) && !"OR".equalsIgnoreCase(keywordArray[i])) {
								if (keywordArray[i].startsWith("(")) {
									if (keywordArray[i].contains("NOT") || keywordArray[i].contains(
										"not")) {
										handleKeyWords.append(keywordArray[i].toUpperCase());
										continue;
									}
									int frontIndex = keywordArray[i].lastIndexOf("(");
									handleKeyWords.append(keywordArray[i], 0, frontIndex + 1)
										.append("\"")
										.append(keywordArray[i].substring(frontIndex + 1))
										.append("\"");
								} else if (keywordArray[i].endsWith(")")) {
									int backIndex = keywordArray[i].indexOf(")");
									handleKeyWords.append("\"")
										.append(keywordArray[i], 0, backIndex).append("\"")
										.append(keywordArray[i].substring(backIndex));
								} else {
									handleKeyWords.append("\"")
										.append(keywordArray[i].toUpperCase()).append("\"");
								}
							} else {
							...
							if (!"AND".equalsIgnoreCase(keywordArray[i]) && !"OR".equalsIgnoreCase(
								keywordArray[i]) && !"NOT".equalsIgnoreCase(keywordArray[i])) {
								if (keywordArray[i].startsWith("(")) {
									if (keywordArray[i].contains("NOT") || keywordArray[i].contains(
										"not")) {
										handleKeyWords.append(keywordArray[i].toUpperCase())
									...
									int backIndex = keywordArray[i].indexOf(")");
									handleKeyWords.append("\"").append(keywordArray[i], 0,
											backIndex).append("\"")
										.append(keywordArray[i].substring(backIndex)).append(" ");
								} else {
									handleKeyWords.append("\"").append(keywordArray[i])
										.append("\" ");
								}
							} else {
