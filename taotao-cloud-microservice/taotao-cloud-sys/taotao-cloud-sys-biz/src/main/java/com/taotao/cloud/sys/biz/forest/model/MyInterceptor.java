...
import com.taotao.cloud.sys.biz.forest.TelClient;
...
@Component
public class MyInterceptor<T> implements Interceptor<T> {

	@Autowired
	private TelClient telClient;
	@Autowired
	...
	public boolean beforeExecute(ForestRequest req) {
		...
		//获取accesstoken
		Object accessToken = repository.get("gitee_access_token");
		if (accessToken == null) {
			String token = telClient.getAccessToken("sldfjasldf");
			repository.set("gitee_access_token", token);
			accessToken = token;
		}

		log.info("invoke Simple beforeExecute");
		// 执行在发送请求之前处理的代码
		req.addHeader("accessToken", accessToken);  // 添加Header
