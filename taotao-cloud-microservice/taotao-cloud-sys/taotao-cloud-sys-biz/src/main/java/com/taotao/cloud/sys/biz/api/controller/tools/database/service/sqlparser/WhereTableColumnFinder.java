...
public class WhereTableColumnFinder implements ExpressionVisitor {
    ...
    private MultiValueMap<String, Column> tableColumnsMap = new LinkedMultiValueMap<>();

    public WhereTableColumnFinder(Map<String, ExtendFindTable> tableAliasMap) {
        this.tableAliasMap = tableAliasMap;
    }

    public MultiValueMap<String, Column> getTableColumnsMap(Expression where){
        where.accept(this);
    ...
    public void visit(Column tableColumn) {
        ...
        if (tableAliasMap.size() == 1){
            ...
            Column findColumn = findTableColumn(tableColumn, extendFindTable);
            if (findColumn == null) {
                return;
            }
        ...
        if (StringUtils.isNotBlank(table.getName())) {
           ...
           Column findColumn = findTableColumn(tableColumn, extendFindTable);
            ...
            A:while (iterator.hasNext()){
                ...
                for (Column column : extendFindTable.getTableMeta().getColumns()) {
    ...
    public Column findTableColumn(Column tableColumn, ExtendFindTable extendFindTable) {
