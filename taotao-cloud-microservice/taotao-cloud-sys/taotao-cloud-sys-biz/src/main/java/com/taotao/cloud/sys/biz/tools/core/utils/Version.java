...
public final class Version implements Serializable {
				...
				if (isNumber(c)) {
					...
					if (i == 0) {
						throw new IllegalArgumentException(
							"The version number string " + stringValue
			...
			if (extraInfoTmp != null) {
				...
				if (firstChar == '.' || firstChar == '-' || firstChar == '_') {
					...
					if (extraInfoTmp.length() == 0) {
						throw new IllegalArgumentException(
							"The version number string " + stringValue
								+ " has an extra info section opened with \"" + firstChar
								+ "\", but it's empty.");
					}
				}
			}
	...
	/**
	 * Creates an object based on the {@code int} value that uses the same kind of encoding as
	 * {@link #intValue()}.
	 *
	 * @since 2.3.24
	 */
	...
	public Version(int major, int minor, int micro, String extraInfo, Boolean gaeCompatible,
		Date buildDate) {
		this.major = major;
		this.minor = minor;
		this.micro = micro;
	 ...
	 * like "RC03" in "2.4.0-RC03". This is usually a qualifier (RC, SNAPHOST, nightly, beta, etc)
	 * and sometimes build info (like date).
	 */
