...
public class UniqueVisitApp {
	...
	public static void main(String[] args) throws Exception {
		...
		SingleOutputStreamOperator<JSONObject> filteredDS = keybyWithMidDS.filter(
			new RichFilterFunction<JSONObject>() {
				...
				public boolean filter(JSONObject jsonObj) throws Exception {
					...
					//用当前页面的访问时间和状态时间进行对比
					if (lastVisitDate != null && lastVisitDate.length() > 0 && lastVisitDate
						.equals(logDate)) {
						System.out.println(
							"已访问：lastVisitDate-" + lastVisitDate + ",||logDate:" + logDate);
						return false;
					} else {
						System.out.println(
							"未访问：lastVisitDate-" + lastVisitDate + ",||logDate:" + logDate);
		...
		//6.1 json->string
		SingleOutputStreamOperator<String> kafkaDS = filteredDS
