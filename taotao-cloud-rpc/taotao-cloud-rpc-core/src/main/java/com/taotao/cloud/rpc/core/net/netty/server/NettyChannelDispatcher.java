...
public class NettyChannelDispatcher {
	 ...
	 */
	private static ExecutorService operationExecutorService = ThreadPoolFactory.createDefaultThreadPool(
		"operation-executor-pool");

	/**
	 * Lettuce 分布式缓存采用 HESSIAN 序列化方式
	 */
	private static CommonSerializer serializer = CommonSerializer.getByCode(
	...
	static {
		...
		try (BufferedReader bufferedReader = new BufferedReader(
			...
			if ("jedis".equals(redisServerWay) || "default".equals(redisServerWay)
				...
				try {
					redisServerAsync = configResource.getString(
						PropertiesConstants.REDIS_SERVER_ASYNC);

					if ("false".equals(redisServerAsync) || "default".equals(redisServerAsync)
			...
			try {
				...
				if ("jedis".equals(redisServerWay) || "default".equals(redisServerWay)
					...
					//LRedisHelper.preLoad();
					try {
						redisServerAsync = resource.getString(
							PropertiesConstants.REDIS_SERVER_ASYNC);
				...
				try {
					...
					if ("jedis".equals(redisServerWay) || "default".equals(redisServerWay)
						...
						if (!JRedisHelper.existsRetryResult(msg.getRequestId())) {
							...
							if (result != null) {
								JRedisHelper.setRetryRequestResult(msg.getRequestId(),
						...
						if (LRedisHelper.existsRetryResult(msg.getRequestId()) == 0L) {
							...
							if (result != null) {
								result = serializer.deserialize((byte[]) result,
									msg.getReturnType());
							}
							log.info("Previous results:{} ", result);
							log.info(
					...
					if (ctx.channel().isActive() && ctx.channel().isWritable()) {
						...
						if (result != null) {
							try {
								checkCode = new String(
									DigestUtils.md5(result.toString().getBytes("UTF-8")));
						...
						RpcResponse rpcResponse = RpcResponse.success(result, msg.getRequestId(),
							checkCode);
						log.info(String.format(
							"server send back response package {requestId: %s, message: %s, statusCode: %s ]}",
							rpcResponse.getRequestId(), rpcResponse.getMessage(),
						...
						log.info("channel status [active: {}, writable: {}]",
