...
public class RpcClientProxy implements InvocationHandler {
	 ...
	 */
	private static UnprocessedRequests unprocessedRequests = SingleFactory.getInstance(
	...
	public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
		...
		if (rpcClient instanceof NettyClient) {
			 ...
			 */
			for (Field field : fields) {
				if (field.isAnnotationPresent(Reference.class) && method.getDeclaringClass()
					.getName().equals(field.getType().getName())) {
			...
			if (!useRetry || asyncTime <= 0) {
				...
				if (timeout >= asyncTime) {
					log.error("asyncTime [ {} ] should be greater than timeout [ {} ]", asyncTime,
						timeout);
				...
				for (int i = 0; i < retries; i++) {
					...
					if (handleTime >= timeout) {
						// 超时重试
						log.warn("invoke service timeout and retry to invoke [ rms: {}, tms: {} ]",
							handleTime, timeout);
						...
						if (checkPass) {
							log.info("client call success counts {} [ rms: {}, tms: {} ]",
								sucRes.incrementAndGet(), handleTime, timeout);
							return rpcResponse.getData();
						}
						// 包被 劫持触发 超时重发机制 保护重发
					}
				...
				log.info("client call failed counts {} [ rms: {}, tms: {} ]",
					errRes.incrementAndGet(), handleTime, timeout);
