...
public class NacosUtils {
	...
	static {
		...
		try (BufferedReader bufferedReader = new BufferedReader(
			...
			if ("false".equals(useCluster) || "default".equals(useCluster) || StringUtils.isBlank(
				useCluster)) {
				...
				try {
					...
					log.info("nacos property attribute is missing: {}",
						loadBalancerException.getMessage());
					log.info("use default loadBalancer : " + balancer);
				}
				try {
					nodes = configResource.getString(PropertiesConstants.NACOS_CLUSTER_NODES)
						.split("[;,|]");
			...
			nodes = new String[1];
			try {
				String nacosRegisterAddr = configResource.getString(
					PropertiesConstants.NACOS_REGISTER_ADDR);
			...
			try {
				...
				try {
					...
					if ("false".equals(useCluster) || "default".equals(useCluster)
						|| StringUtils.isBlank(useCluster)) {
						log.info("begin start with default single mode");
						nodes = new String[1];
						try {
							String nacosRegisterAddr = resource.getString(
								PropertiesConstants.NACOS_REGISTER_ADDR);
							nodes[0] = StringUtils.isBlank(nacosRegisterAddr) ? SERVER_ADDR
								: nacosRegisterAddr;
						} catch (MissingResourceException registerAddException) {
							nodes[0] = SERVER_ADDR;
							log.warn("nacos register address attribute is missing");
						...
						try {
							nodes = resource.getString(PropertiesConstants.NACOS_CLUSTER_NODES)
								.split("[;,|]");
						} catch (MissingResourceException clusterNodesException) {
							log.error("nacos cluster nodes attribute is missing: ",
	...
	public static void registerService(String serviceName, InetSocketAddress address)
		...
		log.info("host[{}], service[{}] has been registered on Register Center",
			address.getHostName(), serviceName);
	 ...
	 * @throws NacosException
	 */
	public static void registerService(String serviceName, String groupName,
		InetSocketAddress address) throws NacosException {
		namingService.registerInstance(serviceName, groupName, address.getHostName(),
			address.getPort());
		log.info("host[{}], service[{}] has been registered on Register Center",
			address.getHostName(), serviceName);
		inetSocketAddress = address;
