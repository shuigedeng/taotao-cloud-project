...
//    protected long epoch = 1387886498127L; // 2013-12-24 20:01:38.127
	 ...
	 */
	private ExecutorService timeStampClearExecutorService = ThreadPoolFactory.createDefaultThreadPool(
		"timestamp-clear-pool");
	...
	public IdWorker(long workerId) {
		...
		logger.debug("worker starting. timestamp left shift {}, worker id {}", timestampLeftShift,
			workerId);
	}
	...
	private long checkWorkerId(long workerId) {
		...
		if (workerId > maxWorkerId || workerId < 0) {
			...
			logger.warn("worker Id can't be greater than {} or less than 0, use a random {}",
				maxWorkerId, rand);
			return rand;
		}
	...
	public synchronized long nextId() {
		...
		if (timestamp < lastMillis) {
			...
			if (diff > 1000L) {
				...
				logger.error("clock is moving backwards.  Rejecting requests until {}.",
					lastMillis);
