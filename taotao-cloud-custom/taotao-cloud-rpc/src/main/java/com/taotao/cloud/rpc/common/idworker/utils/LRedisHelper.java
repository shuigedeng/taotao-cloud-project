...
public class LRedisHelper {
	...
	static {
		...
		try (BufferedReader bufferedReader = new BufferedReader(
			...
			if ("false".equals(redisAuth) || "default".equals(redisAuth) || StringUtils.isBlank(
				redisAuth)) {
				...
				try {
					...
					log.info("redis server address property attribute is missing: {}",
			...
			try {
				String redisAddress = configResource.getString(
					PropertiesConstants.REDIS_SERVER_ADDR);
			...
			try {
				...
				try {
					...
					if ("false".equals(redisAuth) || "default".equals(redisAuth)
						...
						try {
							...
							log.error("redis password attribute is missing: ",
...
//				.timeoutOptions(TimeoutOptions.enabled(Duration.ofSeconds(5)))	//命令超时
	...
	public static void asyncSetRetryRequestResult(String retryRequestId, byte[] result) {
		...
		strToByteAsyncCommand.set(retryReqIds + ":" + retryRequestId, result,
			new SetArgs().nx().ex(60));
	}
	...
	public static void syncSetRetryRequestResult(String retryRequestId, byte[] result) {
		...
		strToByteSyncCommand.set(retryReqIds + ":" + retryRequestId, result,
			new SetArgs().nx().ex(60));
	}
