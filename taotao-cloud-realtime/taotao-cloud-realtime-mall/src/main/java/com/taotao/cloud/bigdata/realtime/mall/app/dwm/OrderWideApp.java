package com.taotao.cloud.bigdata.realtime.mall.app.dwm;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONObject;
import com.taotao.cloud.bigdata.realtime.mall.app.func.DimAsyncFunction;
import com.taotao.cloud.bigdata.realtime.mall.bean.OrderDetail;
import com.taotao.cloud.bigdata.realtime.mall.bean.OrderInfo;
...
public class OrderWideApp {
	...
	public static void main(String[] args) throws Exception {
		...
		//2.2 读取订单主题数据
		FlinkKafkaConsumer<String> orderInfoSource = MyKafkaUtil
			.getKafkaSource(orderInfoSourceTopic, groupId);
		DataStreamSource<String> orderInfoJsonStrDS = env.addSource(orderInfoSource);

		//2.3 读取订单明细数据
		FlinkKafkaConsumer<String> orderDetailSource = MyKafkaUtil
			.getKafkaSource(orderDetailSourceTopic, groupId);
		...
		//TODO 4. 指定事件时间字段
		//4.1 订单指定事件时间字段
		SingleOutputStreamOperator<OrderInfo> orderInfoWithTsDS = orderInfoDS
		...
		//4.2 订单明细指定事件时间字段
		SingleOutputStreamOperator<OrderDetail> orderDetailWithTsDS = orderDetailDS
			.assignTimestampsAndWatermarks(
		...
		KeyedStream<OrderDetail, Long> orderDetailKeyedDS = orderDetailWithTsDS
		...
		SingleOutputStreamOperator<OrderWide> orderWideDS = orderInfoKeyedDS
			...
			.process(
				new ProcessJoinFunction<OrderInfo, OrderDetail, OrderWide>() {
					@Override
					public void processElement(OrderInfo orderInfo, OrderDetail orderDetail,
